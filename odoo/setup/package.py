...
def _rpc_count_modules(addr="http://127.0.0.1", port=8069, dbname="mycompany"):
    time.sleep(5)
    uid = xmlrpclib.ServerProxy(f"{addr}:{port}/xmlrpc/2/common").authenticate(
        dbname, "admin", "admin", {}
    )
    modules = xmlrpclib.ServerProxy(f"{addr}:{port}/xmlrpc/2/object").execute(
        dbname,
        uid,
        "admin",
        "ir.module.module",
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob(f"{args.build_dir}/odoo_*.{extension}")
        if release:
        ...
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                ...
                if addon_path.startswith(args.build_dir) and os.path.isdir(addon_path):
                    logging.info(f"Removing '{addon_path}'")
                    try:
                        shutil.rmtree(addon_path)
                    except shutil.Error as rm_error:
                        logging.warning(f"Cannot remove '{addon_path}': {rm_error}")
...
class Docker:
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = f"odoo-{DOCKERVERSION}-{self.arch}-nightly-tests"
    ...
    def run(
        ...
        if exposed_port:
            docker_cmd.extend(["-p", f"127.0.0.1:{exposed_port}:{exposed_port}"])
            self.exposed_port = exposed_port
        if detach:
            docker_cmd.append("-d")
            # preserve logs in case of detached docker container
            cmd = f"({cmd}) > {self.test_log_file} 2>&1"
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        os.rename(
            ...
            f"{self.args.build_dir}/odoo_{VERSION}.{TSTAMP}.tar.gz",
        )
        os.rename(
            glob("%s/dist/odoo-*.zip" % self.args.build_dir)[0],
            f"{self.args.build_dir}/odoo_{VERSION}.{TSTAMP}.zip",
        )
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f"pip3 install /data/src/odoo_{VERSION}.{TSTAMP}.tar.gz",
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        self.run(
            "python3 setup.py --quiet bdist_rpm",
            self.args.build_dir,
            "odoo-rpm-build-%s" % TSTAMP,
        )
        os.rename(
            glob("%s/dist/odoo-*.noarch.rpm" % self.args.build_dir)[0],
            f"{self.args.build_dir}/odoo_{VERSION}.{TSTAMP}.rpm",
        )
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f"dnf install -d 0 -e 0 /data/src/odoo_{VERSION}.{TSTAMP}.rpm -y",
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        shutil.copytree(
            os.path.join(temp_path, "repodata"),
            os.path.join(args.pub, "rpm", "repodata"),
        )
...
class KVM:
    ...
    def timeout(self, signum, frame):
        logging.warning(f"vm timeout kill (pid: {self.kvm_proc.pid})")
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        with open(
            os.path.join(self.args.build_dir, "setup/win32/Makefile.version"), "w"
        ) as f:
            f.write(
                "VERSION={}.{}\n".format(
        ...
        with open(
            os.path.join(self.args.build_dir, "setup/win32/Makefile.python"), "w"
        ) as f:
            f.write("PYTHON_VERSION=%s\n" % self.args.vm_winxp_python_version)
        with open(
            os.path.join(self.args.build_dir, "setup/win32/Makefile.servicename"), "w"
        ) as f:
        ...
        self.rsync(
            [
                ...
                f"{self.login}@127.0.0.1:{remote_build_dir}",
            ]
        )
        self.ssh(f"cd {remote_build_dir}setup/win32;time make allinone;")
        self.rsync(
            [
                f"{self.login}@127.0.0.1:{remote_build_dir}setup/win32/release/",
                "%s/" % self.args.build_dir,
            ]
        )
