...
class Field(MetaField("DummyField", (object,), {})):
    ...
    def __str__(self):
        if self.name is None:
            return "<{}.{}>".format(__name__, type(self).__name__)
    ...
    def mapped(self, records):
        ...
        while len(vals) < len(records):
            ...
            remaining = records._browse(
                records.env, records[len(vals) :]._ids, records._prefetch_ids
            )
...
class Monetary(Field):
    ...
    def setup_nonrelated(self, model):
        ...
        ), "Field {} with unknown currency_field {!r}".format(
            self,
    ...
    def write(self, records, value):
        ...
        if update_column:
            ...
            if self.translate is True and cache_value:
                tname = f"{records._name},{self.name}"
    ...
    def _setup_attrs(self, model_class, name):
        ...
        for field in self._base_fields:
            ...
            if "selection_add" in field.args:
                ...
                assert (
                    values is not None
                ), "{}: selection_add={!r} on non-list selection {!r}".format(
                    self,
                    selection_add,
                    self.selection,
                )
...
class Many2many(_RelationalMulti):
    ...
    def update_db(self, model, columns):
        ...
        if not sql.table_exists(cr, self.relation):
            ...
            cr.execute(query, [f"RELATION BETWEEN {model._table} AND {comodel._table}"])
            _schema.debug(
    ...
    def write_real(self, records_commands_list, create=False):
        ...
        if pairs:
            ...
            if self.store:
                ...
                COND = f"{self.column1} IN %s AND {self.column2} IN %s"
