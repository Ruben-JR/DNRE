...
def html_keep_url(text):
    ...
    for item in re.finditer(link_tags, text):
        ...
        final += '<a href="{}" target="_blank" rel="noreferrer noopener">{}</a>'.format(
            item.group(0),
            item.group(0),
        )
        idx = item.end()
...
def append_content_to_html(
    ...
    html = re.sub(
        r"(</?)(\w+)([ >])",
        lambda m: "{}{}{}".format(m.group(1), m.group(2).lower(), m.group(3)),
        html,
    )
    ...
    return markupsafe.Markup(
        "{}{}{}".format(html[:insert_location], content, html[insert_location:])
    )
