...
def exec_pg_command(name, *args):
    ...
    with open(os.devnull) as dn:
        ...
        if rc:
            raise Exception(f"Postgres subprocess {args2} error {rc}")
    ...
    class PatchedXlsxWorkbook(xlsxwriter.Workbook):
        ...
        def add_worksheet(self, name=None, **kw):
            if name:
                ...
                # maximum size is 31 characters
                name = name[:31]
            return super().add_worksheet(name, **kw)
...
def logged(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        for key, value in kwargs.items():
            vector.append("  kwarg {:>10}: {}".format(key, pformat(value)))
...
DEFAULT_SERVER_DATETIME_FORMAT = "{} {}".format(
...
def format_decimalized_number(number, decimal=1):
    ...
    for unit in ["", "k", "M", "G"]:
        if abs(number) < 1000.0:
            return "{:g}{}".format(round(number, decimal), unit)
        number /= 1000.0
    return "{:g}{}".format(round(number, decimal), "T")
...
def format_decimalized_amount(amount, currency=None):
    ...
    if currency.position == "before":
        return "{}{}".format(currency.symbol or "", formated_amount)

    return "{} {}".format(formated_amount, currency.symbol or "")
...
def _consteq(str1, str2):
    ...
    return (
        len(str1) == len(str2) and sum(ord(x) ^ ord(y) for x, y in zip(str1, str2)) == 0
    )
...
def get_diff(data_from, data_to, custom_style=False):
    ...
    def handle_style(html_diff, custom_style):
        ...
        for old, new in to_append.items():
            html_diff = html_diff.replace(old, f"{old} {new}")
