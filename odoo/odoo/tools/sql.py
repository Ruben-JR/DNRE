...
def create_model_table(cr, tablename, comment=None, columns=()):
    ...
    queries, params = [], []
    if comment:
        queries.append(f'COMMENT ON TABLE "{tablename}" IS %s')
        params.append(comment)
    for columnname, columntype, columncomment in columns:
        queries.append(f'COMMENT ON COLUMN "{tablename}"."{columnname}" IS %s')
...
def drop_not_null(cr, tablename, columnname):
    ...
    cr.execute(f'ALTER TABLE "{tablename}" ALTER COLUMN "{columnname}" DROP NOT NULL')
    _schema.debug(
...
def create_index(cr, indexname, tablename, expressions):
    ...
    cr.execute(f'CREATE INDEX "{indexname}" ON "{tablename}" ({args})')
...
def create_unique_index(cr, indexname, tablename, expressions):
    ...
    if index_exists(cr, indexname):
        return
    args = ", ".join(expressions)
    cr.execute(f'CREATE UNIQUE INDEX "{indexname}" ON "{tablename}" ({args})')
    _schema.debug("Table %r: created index %r (%s)", tablename, indexname, args)
...
def drop_view_if_exists(cr, viewname):
    cr.execute(f"DROP view IF EXISTS {viewname} CASCADE")
...
def reverse_order(order):
    ...
    for item in order.split(","):
        ...
        items.append("{} {}".format(item[0], direction))
    return ", ".join(items)
...
def increment_field_skiplock(record, field):
    ...
    """.format(
        table=record._table, field=field
    )
