...
def try_report(
    ...
    if res_format == "pdf":
        if res_data[:5] != b"%PDF-":
            raise ValueError(
                "Report {} produced a non-pdf header, {!r}".format(rname, res_data[:10])
            )
        res_text = False
        try:
...
def try_report_action(
    ...
    if isinstance(action_id, str):
        if "." in action_id:
            ...
            act_xmlid = action_id
            action_id = f"{our_module}.{action_id}"
    ...
    assert action, f"Could not read action {act_model}[{act_id}]"
    loop = 0
    while action:
        loop += 1
