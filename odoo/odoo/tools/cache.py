...
class ormcache:
    ...
    def determine_key(self):
        ...
        if self.skiparg is None:
            ...
            if self.args:
                code = "lambda {}: ({},)".format(args, ", ".join(self.args))
            else:
                code = f"lambda {args}: ()"
...
class ormcache_context(ormcache):
    ...
    def determine_key(self):
        ...
        keys_expr = f"tuple({cont_expr}.get(k) for k in {self.keys!r})"
        if self.args:
            code = "lambda {}: ({}, {})".format(args, ", ".join(self.args), keys_expr)
        else:
            code = f"lambda {args}: ({keys_expr},)"
        self.key = unsafe_eval(code)
...
class ormcache_multi(ormcache):
    ...
    def determine_key(self):
        ...
        sign = signature(self.method)
        args = str(sign)[1:-1]
        code_multi = f"lambda {args}: {self.multi}"
