    ...
    if node.tag in ("field", "value"):
        ...
        if t == "file":
            ...
            if not module.get_module_resource(self.module, path):
                raise OSError(f"No such file or directory: '{path}' in {self.module}")
...
class xml_import:
    ...
    def _test_xml_id(self, xml_id):
        if "." in xml_id:
            ...
            assert (
                "." not in id
            ), """The ID reference "{}" must contain
maximum one dot. They are used to refer to other modules ID, in the
form: module.record_id""".format(
                xml_id,
            )
            if module != self.module:
                ...
                assert (
                    modcnt == 1
                ), f"""The ID "{xml_id}" refers to an uninstalled module"""
    ...
    def _tag_report(self, rec):
        ...
        for dest, f in (
            ...
            res[dest] = rec.get(f)
            assert res[dest], f"Attribute {f} of report is empty !"
        for field, dest in (
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        if rec.get("action"):
            ...
            if "." not in a_action:
                a_action = f"{self.module}.{a_action}"
    ...
    def _tag_template(self, el):
        ...
        if "." not in full_tpl_id:
            full_tpl_id = f"{self.module}.{tpl_id}"
