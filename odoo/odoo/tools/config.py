...
class configmanager:
    def __init__(self, fname=None):
        ...
        group.add_option(
            ...
            my_default=8072,
            help="Listen port for the longpolling HTTP service",
            type="int",
            metavar="PORT",
        )
        group.add_option(
        ...
        group.add_option(
            ...
            action="append_const",
            dest="log_handler",
            const="odoo.http.rpc.request:DEBUG",
        ...
        group.add_option(
            ...
            action="append_const",
            dest="log_handler",
            const="odoo.http.rpc.response:DEBUG",
        ...
        group.add_option(
            ...
            choices=levels,
            my_default="info",
            help=f"specify the level of the logging. Accepted values: {levels}.",
        )
        ...
        for path in value.split(","):
            ...
            if not os.path.isdir(res):
                raise optparse.OptionValueError(
                    f"option {opt}: no such directory: {res!r}"
                )
    ...
    def _check_upgrade_path(self, option, opt, value, parser):
        ...
        for path in value.split(","):
            ...
            if not os.path.isdir(res):
                raise optparse.OptionValueError(
                    f"option {opt}: no such directory: {path!r}"
                )
