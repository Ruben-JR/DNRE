...
class TestTermCount(common.TransactionCase):
    def test_count_term(self):
        ...
        odoo.tools.trans_load(
            self.cr, "test_translation_import/i18n/fr.po", "fr_FR", verbose=False
        )
        ...
        self.assertEqual(
            translations[0].name, "addons/test_translation_import/models.py"
        )
    ...
    def test_lang_with_base(self):
        ...
        odoo.tools.trans_load(
            self.cr, "test_translation_import/i18n/fr.po", "fr_BE", verbose=False
        )
        odoo.tools.trans_load(
            self.cr,
            "test_translation_import/i18n/fr_BE.po",
            "fr_BE",
            verbose=False,
            overwrite=True,
        )
        ...
        translations = self.env["ir.translation"].search(
            [
                ...
                (
                    "src",
                    "=",
                    "Test translation with a code type but different line number in pot",
                ),
                (
                    "value",
                    "=",
                    "Test traduction avec un type code mais différent numéro de ligne dans le pot",
                ),
            ]
        )
        self.assertEqual(len(translations), 1)
    ...
    def test_export_empty_string(self):
        ...
        def update_translations(create_empty_translation=False):
            ...
            with closing(io.BytesIO()) as bufferobj:
                odoo.tools.trans_export(
                    "fr_FR", ["test_translation_import"], bufferobj, "po", self.cr
                )
                bufferobj.name = "test_translation_import/i18n/fr.po"
                odoo.tools.trans_load_data(
                    self.cr,
                    bufferobj,
                    "po",
                    "fr_FR",
        ...
        update_translations()
        translation = self.env["ir.translation"].search_count(
            [("src", "=", "Efgh"), ("value", "=", "")]
        )
        ...
        self.assertEqual(
            ...
            "New Name",
            'The translation of "New Name" should be "New Name"',
        )
        ...
        self.assertEqual(
            menu.with_context(lang="fr_FR").name,
            "Nouveau nom",
            'The translation of "New Name" should be "Nouveau nom"',
        )
    ...
    def test_import_from_po_file(self):
        ...
        self.assertEqual(
            _("Klingon"), "tlhIngan", "The code translation was not applied"
        )
    ...
    def test_lazy_translation(self):
        ...
        context = {"lang": "tlh"}
        self.assertEqual(
            _("Klingon"), "tlhIngan", "The direct code translation was not applied"
        )
        context = None
        ...
        self.assertEqual(
            str(TRANSLATED_TERM), "Klingon", "The translation should not be applied yet"
        )
    ...
    def test_import_from_csv_file(self):
        ...
        self.assertEqual(
            _("Accounting"), "samva", "The code translation was not applied"
        )
    ...
    def test_export_pollution(self):
        ...
        reader = TranslationFileReader(
            base64.b64decode(po_file).decode(), fileformat="po"
        )
        for row in reader:
            if row["value"]:
