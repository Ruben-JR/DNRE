...
class TestFields(TransactionCaseWithUserDemo):
    ...
    def test_10_computed_custom_invalid_transitive_depends(self):
        ...
        self.assertEqual(
            sum(
                invalid_depends in field_triggers.get(None, [])
                for field_triggers in triggers.values()
            ),
            0,
        )
        ...
        self.assertEqual(
            sum(
                invalid_transitive_depends in field_triggers.get(None, [])
                for field_triggers in triggers.values()
            ),
            1,
        )
    ...
    def test_11_stored(self):
        ...
        def check_stored(disc):
            ...
            for msg in disc.messages:
                self.assertEqual(msg.name, f"[{disc.name}] {msg.author.name}")
    ...
    def test_40_new_defaults(self):
        ...
        self.assertEqual(new_msg.name, f"[{discussion.name}] {user.name}")
