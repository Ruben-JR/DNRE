...
class TestOnChange(SavepointCaseWithUserDemo):
    ...
    def test_onchange(self):
        ...
        self.assertEqual(result["value"]["name"], f"[{discussion.name}] {USER.name}")
    ...
    def test_onchange_one2many(self):
        ...
        self.assertEqual(message1.name, "[{}] {}".format("", USER.name))
    ...
    def test_onchange_one2many_value(self):
        ...
        lines = [f"{m.name}:{m.body}" for m in discussion.messages]
        lines[0] = "{}:{}".format(discussion.messages[0].name, "test onchange")
        values = {
            "name": discussion.name,
            "moderator": demo.id,
            "categories": [Command.link(cat.id) for cat in discussion.categories],
    ...
    def test_onchange_many2one_one2many(self):
        ...
        values = {
            "discussion": discussion.id,
            "name": "[{}] {}".format("", self.env.user.name),
            "body": False,
            "author": self.env.uid,
            "size": 0,
        }
