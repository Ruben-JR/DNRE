...
class Discussion(models.Model):
    ...
    def _onchange_messages(self):
        self.message_concat = "\n".join([f"{m.name}:{m.body}" for m in self.messages])
...
class Message(models.Model):
    ...
    def _compute_name(self):
        for message in self:
            message.name = self._context.get(
                "compute_name",
                "[{}] {}".format(
                    message.discussion.name or "", message.author.name or ""
                ),
            )
    ...
    def _search_size(self, operator, value):
        ...
        self.flush(["body"])
        query = """SELECT id FROM "{}" WHERE char_length("body") {} %s""".format(
            self._table,
            operator,
        )
...
class ComputeRecursiveTree(models.Model):
    ...
    def _compute_display_name(self):
        for rec in self:
            ...
            rec.display_name = "{}({})".format(rec.name, ", ".join(children_names))
