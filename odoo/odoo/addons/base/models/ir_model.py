...
def model_xmlid(module, model_name):
    ...
    return "{}.model_{}".format(module, model_name.replace(".", "_"))


def field_xmlid(module, model_name, field_name):
    """Return the XML id of the given field."""
    return "{}.field_{}__{}".format(module, model_name.replace(".", "_"), field_name)
...
class IrModelFields(models.Model):
    ...
    def name_get(self):
        res = []
        for field in self:
            res.append((field.id, f"{field.field_description} ({field.model})"))
        return res
...
class IrModelData(models.Model):
    ...
    def check_object_reference(self, module, xml_id, raise_on_access_error=False):
        ...
        model, res_id = self._xmlid_lookup(f"{module}.{xml_id}")[1:3]
