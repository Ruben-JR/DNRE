...
class QWebException(Exception):
    def __init__(
        self, message, qweb, options, error=None, template=None, path=None, code=None
    ):
        self.error = error
        self.name = template
...
class QWeb:
    ...
    _available_objects = {
        k: v for k, v in vars(builtins).items() if not k.startswith("_")
    }
    _allowed_keyword = [
        "False",
        "None",
    ...
    def _compile(self, template, options):
        ...
        options["ref_xml"] = (
            document if isinstance(document, str) else str(document, "utf-8")
        )
        ...
        try:
            ...
            raise QWebException(
                ...
                error=e,
                template=template,
                path=_options.get("last_path_node"),
            )
        try:
            code = "\n".join(code_lines)
        except QWebException as e:
    ...
    def _get_template(self, template, options):
        ...
        if isinstance(template, etree._Element):
            ...
            try:
                ...
                raise QWebException(
                    "load could not load template", self, options, e, template
                )
    ...
    def _is_static_node(self, el, options):
        ...
        attributes.
        """
        return el.tag != "t" and not any(
            att.startswith("t-") and att not in ["t-tag", "t-content"]
    ...
    def _compile_static_node(self, el, options, indent):
        ...
        if unqualified_el_tag != "t":
            attributes = "".join(
                f' {str(name)}="{str(escape(str(value)))}"'
    ...
    def _compile_static_attributes(self, el, options, indent):
        ...
        # the values the prefixes in order to get back the right prefix and restore it.
        nsprefixmap = {
            v: k for k, v in chain(options["nsmap"].items(), el.nsmap.items())
        }
    ...
    def _compile_dynamic_attributes(self, el, options, indent):
        ...
        for name, value in el.attrib.items():
            ...
            if name.startswith("t-attf-"):
                code.append(
                    self._indent(
                        f"attrs[{repr(name[7:])}] = {self._compile_format(value)}",
                        indent,
                    )
                )
                ...
                code.append(
                    self._indent(
                        f"attrs[{repr(name[6:])}] = {self._compile_expr(value)}", indent
                    )
                )
            elif name == "t-att":
                code.append(
    ...
    def _compile_tag_open(self, el, options, indent, attr_already_created=False):
        ...
        if unqualified_el_tag != "t":
            attributes = "".join(
                f' {str(name)}="{str(escape(self._compile_to_str(value)))}"'
                for name, value in extra_attrib.items()
            )
            self._appendText(f"<{el_tag}{attributes}", options)
            code.extend(
                self._compile_all_attributes(el, options, indent, attr_already_created)
            )
            if unqualified_el_tag in self._void_elements:
                self._appendText("/>", options)
            else:
    ...
    def _compile_directive_debug(self, el, options, indent):
        ...
        if options.get("dev_mode"):
            code.append(
                self._indent(
                    f"self._debug_trace({repr(debugger)}, compile_options)", indent
                )
            )
        else:
            _logger.warning(
                "@t-debug in template is only available in qweb dev mode options"
            )
    ...
    def _compile_directive_options(self, el, options, indent):
        ...
        if t_options and dict_arg:
            code.append(
                self._indent(
                    f"{varname} = {{**{self._compile_expr(t_options)}, {', '.join(dict_arg)}}}",
                    indent,
                )
            )
        elif dict_arg:
            ...
            code.append(
                self._indent(f"{varname} = {self._compile_expr(t_options)}", indent)
            )
    ...
    def _compile_directive_tag(self, el, options, indent):
        ...
        if el.nsmap:
            code.extend(
                self._compile_directives(el, dict(options, nsmap=el.nsmap), indent)
            )
        else:
            code.extend(self._compile_directives(el, options, indent))
    ...
    def _compile_directive_set(self, el, options, indent):
        ...
        * whose value is the content of the tag (being Markup safe).
        ...
        if "t-value" in el.attrib:
            ...
            def_name = (
                f"qweb_t_set_{re.sub(_VARNAME_REGEX, '_', options['last_path_node'])}"
            )
            content = self._compile_directive_content(
                el, options, indent + 1
    ...
    def _compile_directive_if(self, el, options, indent):
        ...
        code = self._flushText(options, indent)
        content_if = self._compile_directives(
            el, options, indent + 1
        ) + self._flushText(options, indent + 1)
        ...
        if next_el is not None and {"t-else", "t-elif"} & set(next_el.attrib):
            ...
            el.tail = None
            orelse = self._compile_node(
                next_el, dict(options, t_if=True), indent + 1
            ) + self._flushText(options, indent + 1)
    ...
    def _compile_directive_foreach(self, el, options, indent):
        ...
        code.append(
            self._indent(
                f'log["last_path_node"] = {repr(options["root"].getpath(el))} ',
                indent + 1,
            )
        )
    ...
    def _compile_directive_out(self, el, options, indent):
        ...
        if expr == "0":
            if code_options:
                code.append(
                    self._indent(
                        "content = Markup(''.join(values.get('0', [])))", indent
                    )
                )
            else:
        ...
        if code_options:
            code.append(
                self._indent(
                    f"attrs, content, force_display = self._get_widget(content, {repr(expr)}, {repr(el.tag)}, t_out_t_options, compile_options, values)",
                    indent,
                )
            )
        else:
        ...
        code.extend(
            self._compile_widget_value(
                el, options, indent, without_attributes=not code_options
            )
        )
        return code
    ...
    def _compile_directive_field(self, el, options, indent):
        ...
        code_options = self._compile_directive(el, options, "options", indent) or [
            self._indent("t_field_t_options = {}", indent)
        ]
        code.extend(code_options)
        code.append(
            self._indent(
                f"attrs, content, force_display = self._get_field({self._compile_expr(record, raise_on_missing=True)}, {repr(field_name)}, {repr(expression)}, {repr(tagName)}, t_field_t_options, compile_options, values)",
                indent,
            )
        )
    ...
    def _compile_widget_value(self, el, options, indent=0, without_attributes=False):
        ...
        code = self._flushText(options, indent)
        code.append(
            self._indent("if content is not None and content is not False:", indent)
        )
        code.extend(
            self._compile_tag_open(el, options, indent + 1, not without_attributes)
        )
    ...
    def _compile_directive_call(self, el, options, indent):
        ...
        code_options = self._compile_directive(el, options, "options", indent) or [
            self._indent("t_call_t_options = {}", indent)
        ]
        code.extend(code_options)
        ...
        if content:
            ...
            code.append(
                self._indent(
                    f"t_call_values['0'] = Markup(''.join({def_name}(self, t_call_values, log)))",
                    indent,
                )
            )
        else:
        ...
        if nsmap:
            ...
            code.append(
                self._indent(
                    f"t_call_options.update(nsmap={{{', '.join(nsmap)}}})", indent
                )
            )
        ...
        if code_options:
            ...
            code.append(
                self._indent(
                    f"yield from self._compile({template}, t_call_options)(self, t_call_values)",
                    indent,
                )
            )
