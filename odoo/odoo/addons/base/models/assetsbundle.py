...
class AssetsBundle:
    ...
    def last_modified(self):
        ...
        assets = [
            ...
            for f in self.files
            if f["atype"]
            in ["text/sass", "text/scss", "text/less", "text/css", "text/javascript"]
        ]
        return max(
            itertools.chain(
    ...
    def checksum(self):
        ...
        """
        check = "{}{}".format(
            json.dumps(self.files, sort_keys=True), self.last_modified
        )
    ...
    def _unlink_attachments(self, attachments):
        ...
        self.env.cr.execute(
            f"DELETE FROM {attachments._table} WHERE id IN %s", [tuple(attachments.ids)]
        )
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            line = '@import "{}"{}'.format(ref, matchobj.group(3))
            if (
                "." not in ref
...
class WebAsset:
    ...
    def name(self):
        ...
        return f"{name} defined in bundle '{self.bundle.name}'"
...
class StylesheetAsset(WebAsset):
    ...
    def __init__(self, *args, **kw):
        ...
        if self.direction == "rtl" and self.url:
            ...
            self.html_url_format = "%s/{}/{}.%s".format("rtl", self.bundle.name)
    ...
    def content(self):
        content = super().content
        if self.media:
            content = f"@media {self.media} {{ {content} }}"
        return content
...
class PreprocessedCSS(StylesheetAsset):
    ...
    def __init__(self, *args, **kw):
        ...
        self.html_url_format = "%s/{}{}/%s.css".format(
