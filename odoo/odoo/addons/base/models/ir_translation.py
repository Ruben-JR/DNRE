...
class IrTranslationImport:
    ...
    def finish(self):
        ...
        _logger.debug(
            "ir.translation.cursor: We have %d entries to process", len(self._rows)
        )
...
class IrTranslation(models.Model):
    ...
    def _modified(self):
        ...
        for trans in self:
            if (
                trans.type != "model"
                or (trans.name.split(",")[0] in self.CACHED_MODELS)
                or (trans.comments and "openerp-web" in trans.comments)
            ):  # clear get_web_trans_hash
                self.clear_caches()
                break
    ...
    def _get_terms_query(self, field, records):
        ...
        query = """ SELECT * FROM ir_translation
                    WHERE lang=%s AND type=%s AND name=%s AND res_id IN %s """
        name = f"{field.model_name},{field.name}"
    ...
    def _sync_terms_translations(self, field, records):
        ...
        if discarded:
            ...
            self.env.cr.execute(
                "DELETE FROM ir_translation WHERE id IN %s", [discarded._ids]
            )
    ...
    def _search(
        ...
        if any(
            self.env[model]._fields[field].translate
            for model, ids in self.env.all.towrite.items()
            for record_id, fields in ids.items()
            for field in fields
        ):
            self.flush()
    ...
    def _upsert_translations(self, vals_list):
        ...
        if rows_by_type["model_terms"]:
            query = """
                ...
                WHERE EXCLUDED.value IS NOT NULL AND EXCLUDED.value != '';
            """.format(
                ", ".join(["%s"] * len(rows_by_type["model_terms"]))
            )
    ...
    def _update_translations(self, vals_list):
        ...
        for vals in vals_list:
            ...
            grouped_rows.setdefault(
                key, [vals["value"], vals["src"], vals["state"], []]
            )
    ...
    def translate_fields(self, model, id, field=None):
        ...
        def make_domain(fld, rec):
            name = f"{fld.model_name},{fld.name}"
        ...
        if field:
            ...
            if not fld.related:
                action["context"] = {
                    "search_default_name": f"{fld.model_name},{fld.name}",
                }
            else:
                rec = record
                try:
            ...
            action["target"] = "new"
            action["context"]["translation_type"] = (
                "text" if fld.type in ["text", "html"] else "char"
            )
            action["context"]["translation_show_src"] = False
            if callable(fld.translate):
                action["view_id"] = (
                    self.env.ref("base.view_translation_lang_src_value_tree").id,
                )
                action["context"]["translation_show_src"] = True
            else:
                action["view_id"] = (
                    self.env.ref("base.view_translation_lang_value_tree").id,
                )
    ...
    def get_technical_translations(self, model_name):
        ...
        selection_ids = tools.flatten(
            [field.selection_ids.ids for field in fields if field.ttype == "selection"]
        )
        view = (
