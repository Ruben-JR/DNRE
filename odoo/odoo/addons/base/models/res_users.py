    ...
    def _compute_full_name(self):
        ...
        for group, group1 in zip(self, self.sudo()):
            if group1.category_id:
                group.full_name = f"{group1.category_id.name} / {group1.name}"
            else:
...
class Users(models.Model):
    ...
    def context_get(self):
        ...
        return frozendict({key: values[name] for name, key in name_to_key.items()})
    ...
    def _compute_session_token(self, sid):
        ...
        # generate hmac key
        key = (f"{data_fields}").encode("utf-8")
        # hmac the session id
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        ip = request.httprequest.environ["REMOTE_ADDR"] if request else "n/a"
        _logger.info(
            "Password change for '%s' (#%s) from %s",
            self.env.user.login,
            self.env.uid,
            ip,
        )
...
class GroupsImplied(models.Model):
    ...
    def _remove_group(self, implied_group):
        ...
        if implied_group in self.implied_ids:
            ...
            implied_group.write(
                {"users": [Command.unlink(user.id) for user in self.users]}
            )
...
class UsersImplied(models.Model):
    ...
    def write(self, values):
        ...
        res = super().write(values)
        demoted_users = users_before.filtered(
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        for app, kind, gs, category_name in (
            ...
            if kind == "selection":
                ...
                tips = [f"{g.name}: {g.comment}" for g in gs if g.comment]
                res[field_name] = {
                    "type": "selection",
