...
* if False, the view currently does not extend its parent but can be enabled
    ...
    def _compute_arch(self):
        def resolve_external_ids(arch_fs, view_xml_id):
            def replacer(m):
                xmlid = m.group("xmlid")
                if "." not in xmlid:
                    xmlid = "{}.{}".format(view_xml_id.split(".")[0], xmlid)
    ...
    def _compute_xml_id(self):
        ...
        for data in (
            ...
            xml_ids[data["res_id"]].append("{}.{}".format(data["module"], data["name"]))
        for view in self:
    ...
    def create(self, vals_list):
        for values in vals_list:
            ...
            if not values.get("name"):
                values["name"] = "{} {}".format(values.get("model"), values["type"])
    ...
    def _get_inheriting_views(self):
        ...
        if any(row[3] for row in rows):
            ...
            rows = [
                row for row in rows if not (row[3] and user_groups.isdisjoint(row[3]))
            ]
    ...
    def _postprocess_access_rights(self, node, model):
        ...
        is_base_model = (
            self.env.context.get("base_model_name", model._name) == model._name
        )
        ...
        for attr, expr in node.items():
            if attr in ("class", "t-att-class", "t-attf-class"):
                ...
                if not aria_control and not node.get("t-attf-aria-controls"):
                    msg = (
                        'tab link (data-toggle="tab") must have "aria_control" defined'
                    )
                    self._log_view_warning(msg, node)
    ...
    def get_view_id(self, template):
        ...
        return (
            view
            and view.id
            or self.env["ir.model.data"]._xmlid_to_res_id(
    ...
    def get_view_xmlid(self):
        ...
        xmlid = (
            self.env["ir.model.data"].sudo().search_read(domain, ["module", "name"])[0]
        )
        return "{}.{}".format(xmlid["module"], xmlid["name"])
