    ...
    * generates the root result node itself through :meth:`~.render_element`
    ...
    def record_to_html(self, record, field_name, options):
        ...
        return (
            False
            if value is False
            else record.env[self._name].value_to_html(value, options=options)
        )
    ...
    def value_to_html(self, value, options):
        ...
        for element in body.iter():
            if element.attrib:
                ...
                attrib = irQweb._post_processing_att(
                    element.tag, attrib, options.get("template_options")
                )
...
class MonetaryConverter(models.AbstractModel):
    ...
    def value_to_html(self, value, options):
        ...
        fmt = f"%.{display_currency.decimal_places}f"
