...
class TestViews(ViewCase):
    ...
    def _insert_view(self, **kw):
        ...
        keys = sorted(kw)
        fields = ",".join('"{}"'.format(k.replace('"', r"\"")) for k in keys)
        params = ",".join(f"%({k})s" for k in keys)

        query = f"INSERT INTO ir_ui_view({fields}) VALUES({params}) RETURNING id"
    ...
    def test_view_inheritance(self):
        ...
        view = self.View.with_context(
            check_view_ids=[view2.id, view3.id]
    ...
    def test_view_inheritance_text_inside(self):
        ...
        view = self.View.with_context(check_view_ids=view2.ids).fields_view_get(
            view1.id
        )
        self.assertEqual(view["type"], "form")
        self.assertEqual(
    ...
    def test_modifiers(self):
        def _test_modifiers(what, expected):
            ...
            assert modifiers == expected, f"{modifiers} != {expected}"
    ...
    def test_valid_simili_tab(self):
        self.assertValid(
            '<form><a data-toggle="tab" role="tab" aria-controls="test"/></form>'
        )
