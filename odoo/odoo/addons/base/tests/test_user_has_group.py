...
class TestHasGroup(TransactionCase):
            ...
            portal_user = self.env["res.users"].create(
                {
                    ...
                    "sel_groups_%s_%s"
                    % (
                        grp_test_internal1.id,
                        grp_test_internal2.id,
                    ): grp_test_internal2.id,
                }
            )
    ...
    def test_implied_groups(self):
        ...
        self.assertEqual(
            user_a.groups_id, (group_AA + group_A + group_user + group_no_one)
        )
        ...
        self.assertEqual(
            user_b.groups_id, (group_AA + group_A + group_BB + group_B + group_portal)
        )
