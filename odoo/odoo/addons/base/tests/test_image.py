...
class TestImage(TransactionCase):
    ...
    def test_00_base64_to_image(self):
        ...
        self.assertEqual(
            type(image), PngImagePlugin.PngImageFile, "base64 as string, correct format"
        )
    ...
    def test_10_image_process_base64_source(self):
        ...
        self.assertFalse(
            tools.image_process(False), "return False if base64_source is falsy"
        )
        self.assertEqual(
            tools.image_process(self.base64_svg),
            self.base64_svg,
    ...
    def test_14_image_process_crop(self):
        ...
        for test in tests:
            ...
            self.assertEqual(
                image.getpixel(px),
                test[4][0],
                "{} - color top ({}, {})".format(test[5], px[0], px[1]),
            )
            # verify bottom
            ...
            self.assertEqual(
                image.getpixel(px),
                test[4][1],
                "{} - color bottom ({}, {})".format(test[5], px[0], px[1]),
            )
            # verify left
            px = (left, half_height)
            self.assertEqual(
                image.getpixel(px),
                test[4][2],
                "{} - color left ({}, {})".format(test[5], px[0], px[1]),
            )
            # verify right
            ...
            self.assertEqual(
                image.getpixel(px),
                test[4][3],
                "{} - color right ({}, {})".format(test[5], px[0], px[1]),
            )
    ...
    def test_16_image_process_format(self):
        ...
        image = tools.base64_to_image(
            tools.image_process(self.base64_1920x1080_jpeg, output_format="BMP")
        )
        ...
        image = tools.base64_to_image(
            tools.image_process(self.base64_image_1080_1920_rgba, output_format="jpeg")
        )
    ...
    def test_20_image_data_uri(self):
        ...
        self.assertEqual(
            ...
            "data:image/png;base64," + self.base64_1x1_png.decode("ascii"),
        )
