    ...
    def test_ir_http_attachment_access(self):
        ...
        def test_access(**kwargs):
            ...
            status, _, _ = (
                ...
                .binary_content(**dict(defaults, **kwargs))
            )
            return status
        ...
        self.assertEqual(
            status, 403, "no access if access token for attachment without access token"
        )
        ...
        status = test_access(access_token="Wrong")
        self.assertEqual(
            status, 403, "no access for wrong access token for attachment with access"
        )
