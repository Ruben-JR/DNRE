...
class expression:
    ...
    def parse(self):
        ...
        while stack:
            ...
            if not field:
                raise ValueError(
                    "Invalid field {}.{} in leaf {}".format(
                        model._name, path[0], str(leaf)
                    )
                )
            ...
            # ----------------------------------------
                ...
                subquery, subparams = query.select(
                    f'"{comodel._table}"."{field.inverse_name}"'
                )
                push(
            ...
            # -------------------------------------------------
                ...
                if field.type == "datetime" and right:
                    ...
                    left = unaccent(
                        model._generate_translated_field(alias, left, self.query)
                    )
                    instr = unaccent("%s")
    ...
    def __leaf_to_sql(self, leaf, model, alias):
        ...
        if leaf == TRUE_LEAF:
            ...
            query = '({}."{}" in ({}))'.format(table_alias, left, right[0])
            params = list(right[1])

        elif operator == "not inselect":
            query = '({}."{}" not in ({}))'.format(table_alias, left, right[0])
            params = list(right[1])
            ...
            if isinstance(right, bool):
                ...
                if (operator == "in" and right) or (operator == "not in" and not right):
                    query = f'({table_alias}."{left}" IS NOT NULL)'
                else:
                    query = f'({table_alias}."{left}" IS NULL)'
                params = []
            elif isinstance(right, Query):
                subquery, subparams = right.subselect()
                query = f'({table_alias}."{left}" {operator} ({subquery}))'
                params = subparams
                ...
                if params:
                    ...
                    query = f'({table_alias}."{left}" {operator} ({instr}))'
                else:
                ...
                if (operator == "in" and check_null) or (
                    ...
                    query = f'({query} OR {table_alias}."{left}" IS NULL)'
                elif operator == "not in" and check_null:
                    query = '({} AND {}."{}" IS NOT NULL)'.format(
                        query,
                        table_alias,
                        left,
            ...
            query = '({}."{}" IS NOT NULL and {}."{}" != false)'.format(
                table_alias,
                left,
                table_alias,
                left,
            ...
            column = "{}.{}".format(table_alias, _quote(left))
            query = "({} {} {})".format(
