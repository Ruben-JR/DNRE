...
def new_test_user(env, login="", groups="base.group_user", context=None, **kwargs):
    ...
    if "email" not in create_values:
        if single_email_re.match(login):
            ...
            create_values["email"] = "{}.{}@example.com".format(login[0], login[0])
...
class OdooSuite(unittest.suite.TestSuite):
    ...
    if sys.version_info < (3, 8):
        ...
        def _addClassOrModuleLevelException(
            self, result, exception, errorName, info=None
        ):
...
class BaseCase(unittest.TestCase, metaclass=MetaCase):
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Profiler(
            description="%s %s %s"
            % (test_method, self.env.user.name, "warm" if self.warm else "cold"),
...
class ChromeBrowser:
        ...
        if pid != 0:
            ...
            for _ in range(100):
                ...
                raise unittest.SkipTest(
                    "Failed to detect chrome devtools port after 2.5s."
                )
            return pid
        else:
    ...
    def _chrome_start(self):
        ...
        cmd = [self.executable]
        cmd += [f"{k}={v}" if v else k for k, v in switches.items()]
        url = "about:blank"
        cmd.append(url)
        try:
    ...
    def _json_command(self, command, timeout=3, get_key=None):
        ...
        url = werkzeug.urls.url_join(f"http://{HOST}:{self.devtools_port}/", command)
        ...
        while timeout > 0:
            ...
            delay = delay * 1.5
            tries += 1
        self._logger.error(f"{message} after {tries} tries")
        if failure_info:
    ...
    def take_screenshot(self, prefix="sc_", suffix=None):
        ...
        if not base_png:
            self._logger.warning(
                "Couldn't capture screenshot: expected image data, got %s", res
            )
            return
    ...
    def _save_screencast(self, prefix="failed"):
        ...
        fname = f"{prefix}_screencast_{timestamp}.mp4"
    ...
    def _wait_ready(self, ready_code, timeout=60):
        ...
        ready_id = self._websocket_send(
            "Runtime.evaluate", params={"expression": ready_code}
        )
        promise_id = None
        last_bad_res = ""
        start_time = time.time()
        ...
        while tdiff < timeout:
            ...
            if res.get("id") == ready_id:
                ...
                if result.get("subtype") == "promise":
                    ...
                    promise_id = self._websocket_send(
                        "Runtime.awaitPromise",
                        params={"promiseObjectId": remote_promise_id},
                    )
                elif result == awaited_result:
                    if has_exceeded:
                        self._logger.info(
                            "The ready code tooks too much time : %s", tdiff
                        )
                    return True
                else:
                    last_bad_res = res
    ...
    def _wait_code_ok(self, code, timeout):
            ...
            if res.get("id", -1) == code_id:
                ...
                if res.get("result", {}).get("result").get("subtype", "") == "error":
                    raise ChromeBrowserException(
                        "Running code returned an error: %s" % res
                    )
                ...
                self._websocket_send(
                    "Page.screencastFrameAck", params={"sessionId": int(session_id)}
                )
                outfile = os.path.join(
                    self.screencasts_frames_dir, "frame_%05d.b64" % nb_frame
                )
        ...
        self._save_screencast()
        raise ChromeBrowserException(
            "Script timeout exceeded : %s" % (time.time() - start_time)
        )
    ...
    def navigate_to(self, url, wait_stop=False):
        ...
        if wait_stop and frame_id:
            ...
            self._websocket_wait_event(
                "Page.frameStoppedLoading", params={"frameId": frame_id}
            )
    ...
    def clear(self):
        ...
        """
        cl_id = self._websocket_send(
            "Runtime.evaluate",
            params={"expression": clear_service_workers, "awaitPromise": True},
        )
        ...
        self._websocket_wait_id(dc_id)
        cl_id = self._websocket_send(
            "Runtime.evaluate", params={"expression": "localStorage.clear()"}
        )
        self._websocket_wait_id(cl_id)
...
class HttpCase(TransactionCase):
    ...
    def url_open(
        ...
        if url.startswith("/"):
            url = "http://{}:{}{}".format(HOST, odoo.tools.config["http_port"], url)
        if head:
    ...
    def authenticate(self, user, password):
        ...
        if user:  # if authenticated
            ...
            uid = self.registry["res.users"].authenticate(
                db, user, password, {"interactive": False}
            )
    ...
    def browser_js(
        self, url_path, code, ready="", login=None, timeout=60, cookies=None, **kw
    ):
    ...
    @classmethod
    def base_url(cls):
        return "http://{}:{}".format(HOST, odoo.tools.config["http_port"])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop("code", f"odoo.startTour('{tour_name}'{step_delay})")
        ready = kwargs.pop(
            "ready",
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    def __setattr__(self, field, value):
        ...
        assert not self._get_modifier(
            field, "readonly"
        ), f"can't write on readonly field {field}"
    ...
    def _values_to_save(self, all_fields=False):
        ...
        for f in self._view["fields"]:
            if self._get_modifier(f, "required") and not (
                ...
                assert self._values[f] is not False, f"{f} is a required field"
...
def tagged(*tags):
    ...
    def tags_decorator(obj):
        ...
        obj.test_tags = (
            getattr(obj, "test_tags", set()) | include
        ) - exclude  # todo remove getattr in master since we want to limmit tagged to BaseCase and always have +standard tag
        return obj
