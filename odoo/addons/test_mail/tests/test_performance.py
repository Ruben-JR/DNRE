...
class BaseMailPerformance(TransactionCaseWithUserDemo):
    def setUp(self):
        ...
        self.user_employee = (
            self.env["res.users"]
            .with_context(self._quick_create_ctx)
            .create(
                {
...
class TestMailAPIPerformance(BaseMailPerformance):
    ...
    def test_message_post_one_inbox_notification(self):
        ...
        with self.assertQueryCount(__system__=13, emp=17):
            record.message_post(
...
class TestMailComplexPerformance(BaseMailPerformance):
    def setUp(self):
        ...
        self.customer = (
            self.env["res.partner"]
            .with_context(self._quick_create_ctx)
            .create({"name": "Test Customer", "email": "test@example.com"})
        )
        self.container = (
            self.env["mail.test.container"]
    ...
    def test_complex_mail_mail_send(self):
        ...
        self.assertEqual(
            ...
            formataddr(
                (
                    f"{self.env.company.name} {self.container.name}",
    ...
    def test_complex_message_post_template(self):
        ...
        with self.assertQueryCount(__system__=72, emp=73):
...
class TestMailHeavyPerformancePost(BaseMailPerformance):
    ...
    def test_complete_message_post(self):
        ...
        self.attachements = (
            self.env["ir.attachment"].with_user(self.env.user).create(self.vals)
        )
        ...
        self.assertTrue(
            record.message_ids[0].body.startswith('<p>Test body <img src="/web/image/')
        )
        self.assertEqual(
            self.attachements.mapped("res_model"), [record._name for i in range(3)]
        )
        self.assertEqual(
            self.attachements.mapped("res_id"), [record.id for i in range(3)]
        )
