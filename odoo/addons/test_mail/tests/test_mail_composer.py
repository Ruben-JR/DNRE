...
class TestComposerInternals(TestMailComposer):
    ...
    def test_mail_composer_attachments_comment(self):
        ...
        self.assertEqual(
            generated.name, "TestReport for %s.html" % self.test_record.name
        )
    ...
    def test_mail_composer_content(self):
        ...
        for composition_mode in ["comment", "mass_mail"]:
            ...
            if composition_mode == "comment":
                self.assertEqual(
                    composer.subject, "TemplateSubject %s" % self.test_record.name
                )
                self.assertEqual(
                    composer.body, "<p>TemplateBody %s</p>" % self.test_record.name
                )
    ...
    def test_mail_composer_rights_attachments(self):
        ...
        self.assertEqual(
            self.test_record.message_ids[0].subject, "TemplateSubject TestRecord"
        )
        self.assertEqual(
...
class TestComposerResultsComment(TestMailComposer):
        ...
        self.template.write(
            {
                ...
                "attachment_ids": [(0, 0, a) for a in attachment_data],
                "email_to": f"{email_to_1}, {email_to_1}",
    ...
    def test_mail_composer_notifications_delete(self):
        ...
        self.assertEqual(
            ...
            self.env["mail.mail"],
            "Should have deleted mail.mail records",
        )
    ...
    def test_mail_composer_wtpl_complete(self):
        ...
        self.template.write(
            {
                "auto_delete": False,  # keep sent emails to check content
                "attachment_ids": [(0, 0, a) for a in attachment_data],
                "email_to": f"{email_to_1}, {email_to_2}, {email_to_3}",
                "email_cc": email_cc_1,
        ...
        self.assertEqual(
            len(self._mails), 5, "Should have sent 5 emails, one per recipient"
        )
        ...
        self.assertNotified(
            message,
            [{"partner": self.partner_admin, "is_read": False, "type": "inbox"}],
        )
        ...
        self.assertEqual(
            set(message.attachment_ids.mapped("res_id")), set(self.test_record.ids)
        )
        self.assertTrue(
    ...
    def test_mail_composer_wtpl_complete(self):
        ...
        _mails_records = [
            [
                ...
                if f"{record.id}-{record._name}" in mail["message_id"]
            ]
            for record in self.test_records
        ]
        _mails_record2 = [
            mail
            for mail in self._mails
            if "{}-{}".format(self.test_records[1].id, self.test_records._name)
            in mail["message_id"]
        ]
    ...
    def test_mail_composer_wtpl_delete_notif(self):
        ...
        for record in self.test_records:
            ...
            self.assertEqual(
                ...
                "Should have deleted mail.message records",
            )
