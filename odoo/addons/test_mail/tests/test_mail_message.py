...
class TestMessageValues(TestMailCommon):
    ...
    def test_mail_message_values_fromto_no_document(self):
        ...
        reply_to_email = f"{self.alias_catchall}@{self.alias_domain}"
    ...
    def test_mail_message_values_fromto_document_alias(self):
        ...
        reply_to_name = "{} {}".format(
            self.env.user.company_id.name,
            self.alias_record.name,
        )
        reply_to_email = f"{self.alias_record.alias_name}@{self.alias_domain}"
        self.assertEqual(msg.reply_to, formataddr((reply_to_name, reply_to_email)))
        self.assertEqual(
            msg.email_from,
            formataddr((self.user_employee.name, self.user_employee.email)),
        ...
        reply_to_name = f"{self.env.company.name} {self.alias_record.name}"
        reply_to_email = f"{self.alias_record.alias_name}@{self.alias_domain}"
        self.assertEqual(msg.reply_to, formataddr((reply_to_name, reply_to_email)))
        self.assertEqual(
            msg.email_from,
    ...
    def test_mail_message_values_fromto_document_no_alias(self):
        ...
        reply_to_name = f"{self.env.user.company_id.name} {test_record.name}"
        reply_to_email = f"{self.alias_catchall}@{self.alias_domain}"
        self.assertEqual(msg.reply_to, formataddr((reply_to_name, reply_to_email)))
        self.assertEqual(
            msg.email_from,
    ...
    def test_mail_message_values_fromto_document_manual_alias(self):
        ...
        reply_to_name = f"{self.env.user.company_id.name} {test_record.name}"
        reply_to_email = f"{alias.alias_name}@{self.alias_domain}"
        self.assertEqual(msg.reply_to, formataddr((reply_to_name, reply_to_email)))
        self.assertEqual(
            msg.email_from,
            formataddr((self.user_employee.name, self.user_employee.email)),
...
class TestMessageAccess(TestMailCommon):
    ...
    def test_mail_message_access_read_ok_portal(self):
        self.message.write(
            {"subtype_id": self.ref("mail.mt_comment"), "res_id": self.group_public.id}
        )
        self.message.with_user(self.user_portal).read(
            ["body", "message_type", "subtype_id"]
        )
