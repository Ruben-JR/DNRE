...
class TestMailTemplate(TestMailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.test_template.invalidate_cache(
            ["attachment_ids"], ids=cls.test_template.ids
        )
    ...
    def test_template_translation_lang(self):
        ...
        mail_id = test_template.send_mail(
            test_record.id, notif_layout="test_mail.test_layout"
        )
        mail = self.env["mail.mail"].sudo().browse(mail_id)
        self.assertEqual(
            mail.body_html,
            "<body><p>Spanish Body for %s</p> Spanish Layout Spanish description</body>"
