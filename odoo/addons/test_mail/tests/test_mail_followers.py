...
class BaseFollowersTest(TestMailCommon):
    ...
    def test_recipients_fetch_pids_only(self):
        ...
        recipient_data = self.env["mail.followers"]._get_recipient_data(
            self.env["mail.thread"], False, False, pids=users.partner_id.ids
        )
        groups = {pid: set(groups) for pid, _, _, _, groups in recipient_data}

        self.assertEqual(
            groups[self.user_admin.partner_id.id],
            set(self.user_admin.groups_id.ids),
            "User Admin groups are not correctly fetched",
        )
        self.assertEqual(
            groups[self.user_employee.partner_id.id],
            set(self.user_employee.groups_id.ids),
            "User Employee groups are not correctly fetched",
        )
        self.assertEqual(
            groups[self.user_portal.partner_id.id],
            set(self.user_portal.groups_id.ids),
            "User Portal groups are not correctly fetched",
        )
...
class DuplicateNotificationTest(TestMailCommon):
    def test_no_duplicate_notification(self):
        ...
        common_partner = self.env["res.partner"].create(
            {"name": "demo1", "email": "demo1@test.com"}
        )
        ...
        res = self.env["mail.followers"]._get_recipient_data(
            test, "comment", subtype.id, pids=common_partner.ids
        )
