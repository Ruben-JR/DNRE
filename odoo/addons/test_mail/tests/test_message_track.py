...
class TestTrackingInternals(TestMailCommon):
    ...
    def test_track_groups(self):
        ...
        self.assertFalse(
            msg_emp[0].get("tracking_value_ids"),
            "should not have protected tracking values",
        )
        self.assertTrue(
            msg_sudo[0].get("tracking_value_ids"),
            "should have protected tracking values",
        )

        msg_emp = self.record._notify_prepare_template_context(
            self.record.message_ids, {}
        )
        msg_sudo = self.record.sudo()._notify_prepare_template_context(
            self.record.message_ids, {}
        )
        self.assertFalse(
