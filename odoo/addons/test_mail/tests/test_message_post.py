...
class TestMessagePost(TestMailCommon, TestRecipients):
    ...
    def test_notify_mail_add_signature(self):
        ...
        template = self.env.ref(
            "mail.mail_notification_paynow", raise_if_not_found=True
        ).sudo()
    ...
    def test_notify_mail_add_signature_no_author_user_or_no_user(self):
        ...
        template_values = self.test_record._notify_prepare_template_context(
            self.test_message, {}
        )
        self.assertNotEqual(
    ...
    def test_notify_prepare_template_context_company_value(self):
        ...
        template_values = test_record._notify_prepare_template_context(
    ...
    def test_notify_recipients_internals(self):
        ...
        classify_res = self.env[self.test_record._name]._notify_classify_recipients(
    ...
    def test_post_needaction(self):
        ...
        with self.assertSinglePostNotifications(
            [{"partner": self.partner_employee, "type": "inbox"}], {"content": "Body"}
        ):
    ...
    def test_post_inactive_follower(self):
        ...
        with self.assertSinglePostNotifications(
            [{"partner": self.partner_employee, "type": "inbox"}], {"content": "Test"}
        ):
            self.test_record.message_post(
    ...
    def test_post_notifications(self):
        ...
        self.assertFalse(
            self.env["mail.mail"].sudo().search([("mail_message_id", "=", msg.id)]),
    ...
    def test_post_attachments(self):
        ...
        self.assertEqual(
            set(msg.attachment_ids.mapped("res_model")), {self.test_record._name}
        )
        self.assertEqual(
            set(msg.attachment_ids.mapped("res_id")), {self.test_record.id}
        )
        self.assertEqual(
        ...
        self.assertIn(
            ("Attach1", b"migration test", "application/octet-stream"),
            self._mails[0]["attachments"],
        )
        self.assertIn(
            ("Attach2", b"migration test", "application/octet-stream"),
            self._mails[0]["attachments"],
        )
    ...
    def test_post_answer(self):
        ...
        self.assertEqual(
            new_msg.parent_id.id, parent_msg.id, "message_post: flatten error"
        )
    ...
    def test_post_log_with_view(self):
        new_note = self.test_record.with_user(
            self.user_employee
    ...
    def test_post_notify(self):
        ...
        self.assertEqual(
            new_notification.body, "<p>You have received a notification</p>"
        )
        self.assertEqual(new_notification.author_id, self.env.user.partner_id)
        self.assertEqual(
            new_notification.email_from,
            formataddr((self.env.user.name, self.env.user.email)),
        )
        self.assertEqual(
            new_notification.notified_partner_ids,
            self.partner_1 | self.user_employee.partner_id | self.partner_admin,
        )
        self.assertNotIn(new_notification, self.test_record.message_ids)

        admin_mails = [
            x for x in self._mails if self.partner_admin.name in x.get("email_to")[0]
        ]
        self.assertEqual(
        ...
        admin_access_link = (
            admin_mail[admin_mail.index("model=") : admin_mail.index("/>") - 1]
            if "model=" in admin_mail
            else None
        )

        self.assertIsNotNone(
            admin_access_link, "The email sent to admin should contain an access link"
        )
        self.assertIn(
        ...
        self.assertNotIn(
            "/mail/view?model=",
            partner_mail,
            "The email sent to admin should not contain an access link",
        )
    ...
    def test_post_post_w_template(self):
        ...
        self._create_template(
            "mail.test.simple",
            {
                ...
                "partner_to": "%s,%s"
                % (self.partner_2.id, self.user_admin.partner_id.id),
        ...
        self.email_template.invalidate_cache(
            ["attachment_ids"], ids=self.email_template.ids
        )
    ...
    def test_post_post_w_template_mass_mode(self):
        ...
        with self.mock_mail_gateway():
            test_record.with_user(self.user_employee).message_post_with_template(
                self.email_template.id, composition_mode="mass_mail"
            )
