        ...
        for product in self:
            if self._context.get("uom"):
                value = (
                    ...
                    ._compute_price(product.price, product.uom_id)
                )
            else:
                value = product.price
...
class SupplierInfo(models.Model):
    ...
    def _check_product_variant(self):
        for supplier in self:
            if (
                supplier.product_id
                and supplier.product_tmpl_id
                and supplier.product_id.product_tmpl_id != supplier.product_tmpl_id
            ):
                raise ValidationError(
                    _("The product variant must be a variant of the product template.")
                )
