...
class TestProductCommon(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.product_2 = Product.create(
            {"name": "Wood", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id}
        )
        cls.product_3 = Product.create(
            {"name": "Stone", "uom_id": cls.uom_dozen.id, "uom_po_id": cls.uom_dozen.id}
        )

        cls.product_4 = Product.create(
            {"name": "Stick", "uom_id": cls.uom_dozen.id, "uom_po_id": cls.uom_dozen.id}
        )
        ...
        cls.product_6 = Product.create(
            {"name": "Door", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id}
        )
        ...
        cls.prod_attr1_v3 = cls.env["product.attribute.value"].create(
            {"name": "green", "attribute_id": cls.prod_att_1.id, "sequence": 3}
        )
                        ...
                        0,
                        {
                            ...
                            "value_ids": [
                                (
                                    ...
                                    0,
                                    [
                                        cls.prod_attr1_v1.id,
                                        cls.prod_attr1_v2.id,
                                        cls.prod_attr1_v3.id,
                                    ],
                                )
                            ],
                        },
        ...
        cls.product_8 = Product.create(
            {"name": "House", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id}
        )

        cls.product_9 = Product.create(
            {"name": "Paper", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id}
        )

        cls.product_10 = Product.create(
            {"name": "Stone", "uom_id": cls.uom_unit.id, "uom_po_id": cls.uom_unit.id}
        )
