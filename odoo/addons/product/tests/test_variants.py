...
class TestVariants(common.TestProductCommon):
    ...
    def test_variants_creation_mono_double(self):
        ...
        self.assertEqual(
            test_template.product_variant_ids.product_template_attribute_value_ids.product_attribute_value_id,
            self.size_attr_value_s + self.prod_attr1_v2,
        )
    ...
    def test_variants_creation_mono_multi(self):
        ...
        sofa_attr1_v2 = test_template.attribute_line_ids[0].product_template_value_ids[
            0
        ]
    ...
    def test_variants_creation_matrix(self):
                ...
                "attribute_line_ids": [
                    (
                        ...
                        0,
                        {
                            "attribute_id": self.prod_att_1.id,
                            "value_ids": [
                                (4, self.prod_attr1_v1.id),
                                (4, self.prod_attr1_v2.id),
                            ],
                        },
                    ),
                    (
        ...
        sofa_attr1_v1 = test_template.attribute_line_ids[0].product_template_value_ids[
            0
        ]
        sofa_attr1_v2 = test_template.attribute_line_ids[0].product_template_value_ids[
            1
                    ...
                    (
                        ...
                        False,
                        {
                            ...
                            "value_ids": [
                                (
                                    4,
                                    self.size_attr.value_ids[0].id,
                                    self.size_attr_value_s,
                                ),
                                (
                                    4,
                                    self.size_attr.value_ids[1].id,
                                    self.size_attr_value_m,
                                ),
                            ],
                        },
                    )
...
class TestVariantsArchive(common.TestProductCommon):
    ...
    def setUp(self):
        ...
        self.pa_color = self.env["product.attribute"].create(
        ...
        self.pa_size = self.env["product.attribute"].create(
    ...
    def test_uom_update_variant(self):
        ...
        template = self.env["product.template"].create({"name": "kardon"})
    ...
    def _update_color_vars(self, ptal):
        ...
        self.assertEqual(
            self.ptav_color_white.product_attribute_value_id, self.pav_color_white
        )
        self.ptav_color_black = self.ptal_color.product_template_value_ids[1]
        self.assertEqual(
            self.ptav_color_black.product_attribute_value_id, self.pav_color_black
        )
    ...
    def _update_size_vars(self, ptal):
        ...
        if len(self.ptal_size.product_template_value_ids) > 1:
            ...
            self.assertEqual(
                self.ptav_size_m.product_attribute_value_id, self.pav_size_m
            )
                ...
                "attribute_line_ids": [
                    (
                        ...
                        0,
                        {
                            "attribute_id": self.pa_size.id,
                            "value_ids": [
                                (6, 0, (self.pav_size_s + self.pav_size_m).ids)
                            ],
                        },
                    )
                ],
...
class TestVariantsExclusion(common.TestProductCommon):
    def setUp(self):
        ...
        self.size_attr_value_xl = self.env["product.attribute.value"].create(
            {"name": "XL", "attribute_id": self.size_attr.id}
        )

        self.storage_attr = self.env["product.attribute"].create({"name": "Storage"})
        self.storage_attr_value_128 = self.env["product.attribute.value"].create(
            {"name": "128", "attribute_id": self.storage_attr.id}
        )
        self.storage_attr_value_256 = self.env["product.attribute.value"].create(
            {"name": "256", "attribute_id": self.storage_attr.id}
        )

        # add attributes to product
        self.smartphone_size_attribute_lines = self.env[
        ...
        ].create(
            {
                ...
                "attribute_id": self.size_attr.id,
                "value_ids": [
                    (6, 0, [self.size_attr_value_s.id, self.size_attr_value_xl.id])
                ],
            }
        )

        self.smartphone_storage_attribute_lines = self.env[
            "product.template.attribute.line"
        ].create(
            {
                "product_tmpl_id": self.smartphone.id,
                ...
                "value_ids": [
                    (
                        ...
                        0,
                        [
                            self.storage_attr_value_128.id,
                            self.storage_attr_value_256.id,
                        ],
                    )
                ],
            }
