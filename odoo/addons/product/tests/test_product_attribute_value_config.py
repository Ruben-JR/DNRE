...
class TestProductAttributeValueCommon(TransactionCase):
    ...
    def _add_ssd_attribute(cls):
        cls.ssd_attribute = cls.env["product.attribute"].create(
            {"name": "Memory", "sequence": 1}
        )
    ...
    def _add_ram_attribute(cls):
        cls.ram_attribute = cls.env["product.attribute"].create(
            {"name": "RAM", "sequence": 2}
        )
    ...
    def _add_hdd_attribute(cls):
        cls.hdd_attribute = cls.env["product.attribute"].create(
            {"name": "HDD", "sequence": 3}
        )
