...
class TestProductPricelist(TransactionCase):
    ...
    def test_10_calculation_price_of_products_pricelist(self):
        ...
        msg = "Wrong sale price: Customizable Desk. should be {} instead of {}".format(
        ...
        msg = "Wrong sale price: Laptop. should be {} instead of {}".format(
            laptop_E5023.price,
            (laptop_E5023.lst_price + 1),
        )
        self.assertEqual(
            float_compare(
                laptop_E5023.price, laptop_E5023.lst_price + 1, precision_digits=2
            ),
            0,
            msg,
        )
        ...
        msg = "Wrong sale price: IT component. should be {} instead of {}".format(
            apple_headphones.price,
            apple_headphones.lst_price,
        )
        self.assertEqual(
            float_compare(
                apple_headphones.price, apple_headphones.lst_price, precision_digits=2
            ),
            0,
            msg,
        )
        ...
        msg = "Wrong sale price: LCD Monitor. should be {} instead of {}".format(
            ipad_mini.price,
            ipad_mini.lst_price,
        )
        self.assertEqual(
            float_compare(ipad_mini.price, ipad_mini.lst_price, precision_digits=2),
            0,
            msg,
        )
        ...
        msg = (
            ...
            % (ipad_mini.price, ipad_mini.lst_price - ipad_mini.lst_price * (0.30))
        )
        self.assertEqual(
            float_compare(
                ipad_mini.price,
        ...
        partner = self.res_partner_4.with_context(context)
        msg = (
            "Wrong cost price: LCD Monitor. should be 1000 instead of %s"
            % monitor._select_seller(partner_id=partner, quantity=1.0).price
        )
        self.assertEqual(
            float_compare(monitor.price, monitor.lst_price, precision_digits=2), 0, msg
        )
        context.update(
            {
                "quantity": 1,
                "date": datetime.strptime("2020-04-06 10:00:00", "%Y-%m-%d %H:%M:%S"),
            }
        )
        monitor = self.monitor.with_context(context)
        msg = (
            "Wrong cost price: LCD Monitor. should be 500 instead of %s"
            % monitor._select_seller(partner_id=partner, quantity=1.0).price
        )
        self.assertEqual(
            float_compare(monitor.price, monitor.lst_price / 2, precision_digits=2),
            0,
            msg,
        )
    ...
    def test_20_price_different_currency_pricelist(self):
        ...
        product = self.monitor.with_context({"pricelist": pricelist.id, "quantity": 1})
