...
class TestPricelist(TransactionCase):
    ...
    def test_10_discount(self):
        ...
        self.assertEqual(
            usb_adapter_with_pricelist.price, usb_adapter_without_pricelist.price * 0.9
        )
        ...
        self.assertEqual(
            datacard_with_pricelist.price, datacard_without_pricelist.price - 0.5
        )
        ...
        dozen_context = dict(
            context, pricelist=self.sale_pricelist_id.id, uom=self.uom_dozen_id
        )
    ...
    def test_20_pricelist_uom(self):
        ...
        def test_unit_price(qty, uom, expected_unit_price):
            ...
            unit_price = pricelist.with_context({"uom": uom}).get_product_price(
                spam, qty, False
            )
