...
class ImLivechatChannel(models.Model):
    ...
    def _get_livechat_mail_channel_vals(
        self, anonymous_name, operator, user_id=None, country_id=None
    ):
        mail_channel_vals = super()._get_livechat_mail_channel_vals(
        ...
        if visitor_sudo:
            ...
            if not user_id:
                mail_channel_vals["anonymous_name"] = visitor_sudo.display_name + (
                    " (%s)" % visitor_sudo.country_id.name
            ...
            for mail_channel in chat_request_channel:
                mail_channel._close_livechat_session(
                    cancel=True, operator=operator.name
                )
