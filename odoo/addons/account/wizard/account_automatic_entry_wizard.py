...
class AutomaticEntryWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if any(
            line.account_id.user_type_id != move_line_ids[0].account_id.user_type_id
        ...
        for date, grouped_lines in groupby(
            ...
            move_data[date] = {
                ...
                "ref": self._format_strings(
                    _(
                        "Adjusting Entry of {date} ({percent:f}% recognized on {new_date})"
                    ),
    ...
    def _do_action_change_period(self, move_vals):
        ...
        for move in self.move_line_ids.move_id:
            ...
            accrual_move_messages[accrual_move] += [
                self._format_strings(
                    _(
                        "Adjusting Entry for {link}: {percent:f}% of {amount} recognized on {new_date}"
                    ),
                    move,
                    amount,
                )
