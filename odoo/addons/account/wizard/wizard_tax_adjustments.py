...
class TaxAdjustments(models.TransientModel):
    ...
    def _domain_tax_report(self):
        fiscal_country_ids = (
            ...
            .search(
                [("company_id", "=", self.env.company.id), ("foreign_vat", "!=", False)]
            )
            .country_id.ids
        )
        return [
            ("tag_name", "!=", None),
            "|",
            ("report_id.country_id", "=", self.env.company.country_id.id),
            ("report_id.country_id", "in", fiscal_country_ids),
        ]
    ...
    company_currency_id = fields.Many2one(
        "res.currency", readonly=True, default=lambda x: x.env.company.currency_id
    )
    ...
    def create_move(self):
        ...
        sign_multiplier = (self.amount < 0 and -1 or 1) * (
            self.adjustment_type == "credit" and -1 or 1
        )
        filter_lambda = (
            (sign_multiplier < 0)
            and (lambda x: x.tax_negate)
            or (lambda x: not x.tax_negate)
        )
