...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _compute_payment_method_line_fields(self):
        for wizard in self:
            ...
            if (
                wizard.payment_method_line_id.id
                not in wizard.available_payment_method_line_ids.ids
            ):
        ...
        for wizard in self:
            if wizard.source_currency_id == wizard.currency_id:
                ...
                wizard.payment_difference = (
                    wizard.source_amount_currency - wizard.amount
                )
            elif wizard.currency_id == wizard.company_id.currency_id:
                # Payment expressed on the company's currency.
    ...
    def fields_view_get(
        ...
        if view_type == "form":
            ...
            if (
                res.get("view_id") == form_view.id
                and len(tree.xpath("//field[@name='available_partner_bank_ids']")) == 0
            ):
