...
class AccountReconcileModel(models.Model):
            ...
            new_aml_dicts.append(
                {
                    ...
                    "analytic_account_id": tax.analytic
                    and base_line_dict["analytic_account_id"],
                    "analytic_tag_ids": tax.analytic
                    and base_line_dict["analytic_tag_ids"],
    ...
    def _get_write_off_move_lines_dict(self, st_line, residual_balance, partner_id):
        ...
        if st_line:
            ...
            st_line_residual = (
                matched_candidates_values["balance_sign"]
                * matched_candidates_values["residual_balance_curr"]
            )
        else:
        ...
        for line in self.line_ids:
            ...
            if line.tax_ids:
                ...
                ].get_fiscal_position(partner_id)
                ...
                tax_vals_list = self._get_taxes_move_lines_dict(
                    taxes, writeoff_line, statement_line=st_line
                )
    ...
    def _apply_rules(self, st_lines, excluded_ids=None, partner_map=None):
        ...
        for st_line in st_lines:
            ...
            mapped_partner = (
                partner_map
                and partner_map.get(st_line.id)
                and self.env["res.partner"].browse(partner_map[st_line.id])
            ) or st_line.partner_id
        ...
        for rec_model in available_models:
            ...
            filtered_st_lines_with_partner = [
                x
                for x in lines_with_partner_per_model[rec_model]
                if x[0] not in matched_lines
            ]
            ...
            for st_line, partner in filtered_st_lines_with_partner:
                ...
                if candidates:
                    (
                        model_rslt,
                        new_reconciled_aml_ids,
                        new_treated_aml_ids,
                    ) = rec_model._get_rule_result(
                        st_line,
                        candidates,
                        aml_ids_to_exclude,
                        reconciled_amls_ids,
    ...
    def _get_candidates(self, st_lines_with_partner, excluded_ids):
        ...
        treatment_map = {
            "invoice_matching": lambda x: x._get_invoice_matching_query(
                st_lines_with_partner, excluded_ids
            ),
            "writeoff_suggestion": lambda x: x._get_writeoff_suggestion_query(
                st_lines_with_partner, excluded_ids
            ),
        }
    ...
    def _get_invoice_matching_query(self, st_lines_with_partner, excluded_ids):
        ...
        if self.rule_type != "invoice_matching":
            raise UserError(
                _(
                    "Programmation Error: Can't call _get_invoice_matching_query() for different rules than 'invoice_matching'"
                )
            )
        ...
        SELECT
            ...
            + r""" AS communication_flag,
            """
            + self._get_select_payment_reference_flag()
            + r""" AS payment_reference_flag
        ...
        for st_line, partner in st_lines_with_partner:
            ...
            if self.match_same_currency:
                st_line_subquery += (
                    r" AND COALESCE(aml.currency_id, company.currency_id) = %s"
            ...
            st_lines_queries.append(
                rf"st_line.id = {st_line.id} AND ({st_line_subquery})"
            )
    ...
    def _get_select_communication_flag(self):
        ...
        st_ref = " || ' ' || ".join(
            "COALESCE(%s, '')" % st_ref_name for st_ref_name in st_ref_list
        )
        if not st_ref:
            return "FALSE"
    ...
    def _get_writeoff_suggestion_query(self, st_lines_with_partner, excluded_ids=None):
        ...
        if self.rule_type != "writeoff_suggestion":
            raise UserError(
                _(
                    "Programmation Error: Can't call _get_writeoff_suggestion_query() for different rules than 'writeoff_suggestion'"
                )
            )
        ...
        params = {
            "st_line_ids": tuple(
                st_line.id for (st_line, partner) in st_lines_with_partner
            ),
        }
    ...
    def _get_rule_result(
        ...
        if self.rule_type == "invoice_matching":
            return self._get_invoice_matching_rule_result(
                st_line,
                candidates,
                aml_ids_to_exclude,
                reconciled_amls_ids,
            ...
            return (
                self._get_writeoff_suggestion_rule_result(st_line, partner_map),
                set(),
                set(),
            )
        else:
    ...
    def _get_invoice_matching_rule_result(
        self, st_line, candidates, aml_ids_to_exclude, reconciled_amls_ids, partner
    ):
        ...
        candidate_currencies = {
            candidate["aml_currency_id"] for candidate in candidates
        }
            ...
            for candidate in candidates:
                ...
                if (
                    st_line_currency.compare_amounts(
                        candidate_residual, -st_line.amount_residual
                    )
                    == 0
                ):
                    ...
                    st_line_currency.compare_amounts(
                        abs(sum_kept_candidates), abs(st_line.amount_residual)
                    )
                    < 0
                ):
        ...
        kept_candidates_by_priority = self._sort_reconciliation_candidates_by_priority(
            kept_candidates, aml_ids_to_exclude, reconciled_amls_ids
        )
        ...
        matched_candidates_values = self._process_matched_candidates_data(
            st_line, kept_candidates
        )
        ...
        if "rejected" in status:
            ...
            if "allow_auto_reconcile" in status:
                ...
                if lines_vals_list and priorities & {1, 3} and self.auto_reconcile:
                    ...
                    dummy, open_balance_vals = st_line._prepare_reconciliation(
                        lines_vals_list + writeoff_vals_list
                    )
                    ...
                    if not open_balance_vals or open_balance_vals.get("account_id"):
                        ...
                        st_line.reconcile(
                            lines_vals_list + writeoff_vals_list, allow_partial=True
                        )
    ...
    def _process_matched_candidates_data(self, statement_line, candidates=None):
        ...
        (
            ...
            open_balance_vals,
        ) = statement_line._prepare_reconciliation(
            [
                {
    ...
    def _check_rule_propositions(self, matched_candidates_values):
        ...
        if (
            ...
            and -residual_balance_after_rec <= self.payment_tolerance_param
        ):
            return {"allow_write_off", "allow_auto_reconcile"}
    ...
    def _sort_reconciliation_candidates_by_priority(
        self, candidates, already_proposed_aml_ids, already_reconciled_aml_ids
    ):
        ...
        for candidate in filter(
            lambda x: x["aml_id"] not in already_reconciled_aml_ids, candidates
        ):
