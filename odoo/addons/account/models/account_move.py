...
class AccountMove(models.Model):
    ...
    def _get_default_invoice_date(self):
        warnings.warn(
            "Method '_get_default_invoice_date()' is deprecated and has been removed.",
            DeprecationWarning,
        )
        return (
    ...
    suitable_journal_ids = fields.Many2many(
        "account.journal", compute="_compute_suitable_journal_ids"
    )
    ...
    amount_residual = fields.Monetary(
        string="Amount Due", store=True, compute="_compute_amount"
    )
    ...
    display_qr_code = fields.Boolean(
        string="Display QR-code", related="company_id.qr_code"
    )
        ...
        if self.invoice_date:
            ...
            if not self.is_sale_document(include_receipts=True):
                accounting_date = self._get_accounting_date(
                    self.invoice_date, self._affect_tax_report()
                )
    ...
    def _onchange_partner_id(self):
        ...
        if self.is_sale_document(include_receipts=True) and self.partner_id:
            self.invoice_payment_term_id = (
                self.partner_id.property_payment_term_id or self.invoice_payment_term_id
            )
            new_term_account = (
        ...
        self.fiscal_position_id = self.env[
            "account.fiscal.position"
        ].get_fiscal_position(self.partner_id.id, delivery_id=delivery_partner_id)
    ...
    def _onchange_invoice_vendor_bill(self):
        if self.invoice_vendor_bill_id:
            ...
            # Copy payment terms.
            self.invoice_payment_term_id = (
                self.invoice_vendor_bill_id.invoice_payment_term_id
            )
    ...
    def _onchange_name_warning(self):
        ...
        if self.name and self.name != "/" and origin_name and origin_name != "/":
            ...
            if new_format != origin_format or dict(new_format_values, seq=0) != dict(
                origin_format_values, seq=0
            ):
                changed = _(
    ...
    def _compute_type_name(self):
        type_name_mapping = {
            k: v for k, v in self._fields["move_type"]._description_selection(self.env)
        }
    ...
    def _inverse_amount_total(self):
        for move in self:
            ...
            balance = move.currency_id._convert(
                amount_currency, move.company_currency_id, move.company_id, move.date
            )
            ...
            groups_vals = [
                {
                    ...
                    "group_key": f"{subtotal_title}-{group.id}",
                }
    ...
    def _check_journal_type(self):
        for record in self:
            ...
            if (
                ...
                or record.is_purchase_document()
                and journal_type != "purchase"
            ):
                raise ValidationError(
                    _(
                        "The chosen journal has a type that is not compatible with your invoice type. Sales operations should go to 'sale' journals, and purchase operations to 'purchase' ones."
                    )
                )
    ...
    def _move_autocomplete_invoice_lines_values(self):
        ...
        invoice_lines = self.line_ids.filtered(
            lambda x: not x.exclude_from_invoice_tab
        ...
        for cached_vals, line in zip(cached_vals_list, invoice_lines):
            ...
            if not line._cache.get("account_id") and not line._origin:
                line.account_id = (
                    line._get_computed_account() or self.journal_id.default_account_id
                )
    ...
    def _move_autocomplete_invoice_lines_create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            default_move_type = vals.get("move_type") or self._context.get(
                "default_move_type"
            )
            ctx_vals = {}
            ...
            if vals.get("journal_id"):
                ...
                journal_company = (
                    self.env["account.journal"].browse(vals["journal_id"]).company_id
                )
            ...
            is_invoice = vals.get("move_type") in self.get_invoice_types(
    ...
    def _move_autocomplete_invoice_lines_write(self, vals):
        ...
        """
        enable_autocomplete = (
            "invoice_line_ids" in vals and "line_ids" not in vals and True or False
        )
    ...
    def copy(self, default=None):
        ...
        copied_am._message_log(
            body=_(
                ...
                id=self.id,
                title=html_escape(self.display_name),
            )
        )
    ...
    def _unlink_forbid_parts_of_chain(self):
        ...
        if (
            ...
            and not self.filtered(lambda move: move.name != "/")._is_end_of_seq_chain()
        ):
            raise UserError(
                _(
    ...
    def _get_invoice_reference_euro_invoice(self):
        ...
        self.ensure_one()
        base = self.id
        check_digits = calc_check_digits(f"{base}RF")
        reference = "RF{} {}".format(
            check_digits,
            " ".join(
                ["".join(x) for x in zip_longest(*[iter(str(base))] * 4, fillvalue="")]
            ),
        )
        return reference
    ...
    def _get_invoice_reference_euro_partner(self):
        ...
        check_digits = calc_check_digits(f"{partner_ref_nr}RF")
        reference = "RF{} {}".format(
            check_digits,
            " ".join(
                [
    ...
    def _get_move_display_name(self, show_ref=False):
        ...
        return name + (
            show_ref
            and self.ref
            and " ({}{})".format(self.ref[:50], "..." if len(self.ref) > 50 else "")
            or ""
        )
    ...
    def _reverse_move_vals(self, default_values, cancel=True):
        ...
        def compute_tax_repartition_lines_mapping(move_vals):
            ...
            for line_command in move_vals.get("line_ids", []):
                ...
                for tax in (
                    self.env["account.tax"].browse(tax_ids).flatten_taxes_hierarchy()
                ):
        ...
        tax_repartition_lines_mapping = (
            compute_tax_repartition_lines_mapping(move_vals) if is_refund else {}
        )
        ...
        for line_command in move_vals.get("line_ids", []):
            ...
            if line_vals.get("tax_repartition_line_id"):
                ...
                ].browse(line_vals["tax_repartition_line_id"])
                ...
                account_id = (
                    ...
                    ._get_default_tax_account(refund_repartition_line)
                    .id
                )
                if not account_id:
                    if not invoice_repartition_line.account_id:
                        ...
                        base_line = self.line_ids.filtered(
                            lambda line: tax in line.tax_ids.flatten_taxes_hierarchy()
                        )[0]
                        account_id = base_line.account_id.id

                tags = refund_repartition_line.tag_ids
                if line_vals.get("tax_ids"):
                    subsequent_taxes = self.env["account.tax"].browse(
                ...
                taxes = (
                    ...
                    .browse(line_vals["tax_ids"][0][2])
                    .flatten_taxes_hierarchy()
                )
                invoice_repartition_lines = taxes.mapped(
                    "invoice_repartition_line_ids"
                ).filtered(lambda line: line.repartition_type == "base")
                refund_repartition_lines = invoice_repartition_lines.mapped(
                    lambda line: tax_repartition_lines_mapping[line]
                )

                line_vals["tax_tag_ids"] = [
    ...
    def action_view_reverse_entry(self):
        ...
        reverse_entries = self.env["account.move"].search(
    ...
    def generate_qr_code(self):
        ...
        if qr_code_method:
            ...
            for candidate_method, candidate_name in self.env[
                "res.partner.bank"
            ].get_available_qr_methods_in_sequence():
    ...
    def _compute_narration(self):
        ...
        for move in self:
            if not use_invoice_terms or not move.is_sale_document(
                include_receipts=True
            ):
...
class AccountMoveLine(models.Model):
    ...
    move_name = fields.Char(
        string="Number", related="move_id.name", store=True, index=True
    )
    date = fields.Date(
    ...
    ref = fields.Char(
        related="move_id.ref", store=True, copy=False, index=True, readonly=True
    )
    ...
    account_internal_type = fields.Selection(
        related="account_id.user_type_id.type", string="Internal Type", readonly=True
    )
    ...
    account_root_id = fields.Many2one(
        related="account_id.root_id", string="Account Root", store=True, readonly=True
    )
    ...
    price_subtotal = fields.Monetary(
        string="Subtotal", store=True, readonly=True, currency_field="currency_id"
    )
    price_total = fields.Monetary(
        string="Total", store=True, readonly=True, currency_field="currency_id"
    )
    ...
    analytic_line_ids = fields.One2many(
        "account.analytic.line", "move_id", string="Analytic lines"
    )
    ...
    def _set_price_and_tax_after_fpos(self):
        ...
        if (
            self.tax_ids
            and self.move_id.fiscal_position_id
            and self.move_id.fiscal_position_id.tax_ids
        ):
    ...
    def _get_fields_onchange_subtotal(
        self,
        price_subtotal=None,
        move_type=None,
        currency=None,
    ...
    def belongs_to_refund(self):
        ...
        if self.tax_repartition_line_id:
            ...
            return (tax_type == "sale" and self.debit) or (
    ...
    def _compute_amount_residual(self):
        ...
        for line in self:
            if line.id and (
                ...
                or line.account_id.internal_type == "liquidity"
            ):
    ...
    def _compute_tax_tag_invert(self):
        for record in self:
            if not record.tax_repartition_line_id and not record.tax_ids:
                ...
                if rep_line:
                    tax_type = (
                        rep_line.refund_tax_id or rep_line.invoice_tax_id
                    ).type_tax_use
                    is_refund = bool(rep_line.refund_tax_id)
                elif record.tax_ids:
                    tax_type = record.tax_ids[0].type_tax_use
                    is_refund = (tax_type == "sale" and record.debit) or (
                        tax_type == "purchase" and record.credit
                    )
    ...
    def _check_constrains_account_id_journal_id(self):
        for line in self.filtered(
            ...
            if (
                ...
                if journal.type_control_ids:
                    failed_check = account.user_type_id not in (
                        journal.type_control_ids
                        - journal.default_account_id.user_type_id
                    )
            ...
            if failed_check:
                raise UserError(
                    _(
                        "You cannot use this account (%s) in this journal, check the section 'Control-Access' under tab 'Advanced Settings' on the related journal.",
                        account.display_name,
                    )
                )
    ...
    def default_get(self, default_fields):
        ...
        if (
            ...
            for field_name in ("debit", "credit", "account_id", "partner_id")
        ):
            ...
            if "partner_id" in default_fields and not values.get("partner_id"):
                if (
                    len(move.line_ids[-2:]) == 2
                    and move.line_ids[-1].partner_id
            ...
            if "account_id" in default_fields and not values.get("account_id"):
                if (
                    len(move.line_ids[-2:]) == 2
                    and move.line_ids[-1].account_id
                    == move.line_ids[-2].account_id
                    != False
                ):
    ...
    def name_get(self):
        ...
        for line in self:
            ...
            name += (
                ...
                and (" " + (line.name or line.product_id.display_name))
                or ""
            )
    ...
    def invalidate_cache(self, fnames=None, ids=None):
        ...
        if fnames is None or "move_id" in fnames:
            ...
            lines = (
                self.env.cache.get_records(self, field)
                if ids is None
                else self.browse(ids)
            )
    ...
    def _prepare_reconciliation_partials(self):
        ...
        def fix_remaining_cent(currency, abs_residual, partial_amount):
            if (
                abs_residual - currency.rounding
                <= partial_amount
                <= abs_residual + currency.rounding
            ):
                return abs_residual
        ...
        debit_lines = iter(
            self.filtered(lambda line: line.balance > 0.0 or line.amount_currency > 0.0)
        )
        credit_lines = iter(
            self.filtered(lambda line: line.balance < 0.0 or line.amount_currency < 0.0)
        )
        ...
        while True:
            ...
            has_debit_residual_left = (
                not debit_line.company_currency_id.is_zero(debit_amount_residual)
                and debit_amount_residual > 0.0
            )
            has_credit_residual_left = (
                not credit_line.company_currency_id.is_zero(credit_amount_residual)
                and credit_amount_residual < 0.0
            )
            has_debit_residual_curr_left = (
                not debit_line_currency.is_zero(debit_amount_residual_currency)
                and debit_amount_residual_currency > 0.0
            )
            ...
            if debit_line_currency == credit_line_currency:
                ...
                min_amount_residual_currency = min(
                    debit_amount_residual_currency, -credit_amount_residual_currency
                )
                ...
                min_debit_amount_residual_currency = (
                    credit_line.company_currency_id._convert(
                        min_amount_residual,
                ...
                min_credit_amount_residual_currency = (
                    debit_line.company_currency_id._convert(
                        min_amount_residual,
    ...
    def copy_data(self, default=None):
        ...
        for line, values in zip(self, res):
            ...
            if line.move_id.is_invoice() and line.account_id.user_type_id.type in (
                "receivable",
                "payable",
            ):
                values["name"] = ""
    ...
    def _get_analytic_tag_ids(self):
        ...
        return self.analytic_tag_ids.filtered(
            lambda r: not r.active_analytic_distribution
        ).ids
    ...
    def create_analytic_lines(self):
            ...
            for tag in obj_line.analytic_tag_ids.filtered(
                ...
                for distribution in tag.analytic_distribution_ids:
                    analytic_line_vals.append(
                        obj_line._prepare_analytic_distribution_line(distribution)
                    )
        ...
        if lines_to_create_analytic_entries:
            analytic_line_vals += (
                lines_to_create_analytic_entries._prepare_analytic_line()
            )
            ...
            result.append(
                {
                    ...
                    and move_line.product_id.id
                    or False,
                    "product_uom_id": move_line.product_uom_id
                    and move_line.product_uom_id.id
                    or False,
                    "amount": amount,
                    ...
                    or move_line.move_id.company_id.id,
                    "category": category,
                }
            )
        return result
