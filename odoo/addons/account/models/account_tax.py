...
class AccountTaxGroup(models.Model):
    ...
    property_advance_tax_payment_account_id = fields.Many2one(
        "account.account", company_dependent=True, string="Advance Tax payment account"
    )
...
class AccountTax(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        rslt = super().default_get(fields_list + ["company_id"])
    ...
    def onchange_amount(self):
        if (
            ...
            self.description = f"{self.amount:.4g}%"
            ...
            for tax in reversed(taxes):
                ...
                if tax.price_include or self._context.get("force_price_include"):
                    if tax.amount_type == "percent":
                        ...
                        incl_fixed_amount += (
                            abs(quantity) * tax.amount * sum_repartition_factor
                        )
                    else:
