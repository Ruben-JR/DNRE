...
class AccountJournal(models.Model):
    ...
    type_control_ids = fields.Many2many(
        ...
        "journal_account_type_control_rel",
        "journal_id",
        "type_id",
    ...
    default_account_type = fields.Many2one(
        "account.account.type", compute="_compute_default_account_type"
    )
        ...
        for journal in self:
            if journal.type in default_account_id_types:
                journal.default_account_type = self.env.ref(
                    default_account_id_types[journal.type]
                ).id
            else:
    ...
    def _constrains_type_control_ids(self):
        ...
        if self._cr.fetchone():
            raise ValidationError(
                _(
                    "Some journal items already exist in this journal but with accounts from different types than the allowed ones."
                )
            )
    ...
    def _check_type_default_account_id_type(self):
        for journal in self:
            if journal.type in (
                "sale",
                "purchase",
            ) and journal.default_account_id.user_type_id.type in (
                "receivable",
                "payable",
            ):
                raise ValidationError(
    ...
    def _check_payment_method_line_ids_multiplicity(self):
        ...
        self.env["account.payment.method.line"].flush(
    ...
    def _get_alias_values(self, type, alias_name=None):
        ...
        try:
            ...
            _logger.warning(
                "Cannot use '%s' as email alias, fallback to '%s'",
                alias_name,
                safe_alias_name,
            )
        ...
        return {
            "alias_defaults": {
                "move_type": type == "purchase" and "in_invoice" or "out_invoice",
    ...
    def copy(self, default=None):
        ...
        while counter <= len(all_journal_codes) and copy_code in all_journal_codes:
            ...
            copy_code = f"{copy_prefix}{counter_str}"
    ...
    def _update_mail_alias(self, vals=None):
        ...
        if self.type in ("purchase", "sale"):
            alias_values = self._get_alias_values(
                type=self.type, alias_name=self.alias_name
            )
            if self.alias_id:
        ...
        for journal in self:
            ...
            if "bank_account_id" in vals:
                if not vals.get("bank_account_id"):
                    raise UserError(
                        _(
                            "You cannot remove the bank account from the journal once set."
                        )
                    )
                else:
    ...
    def create_invoice_from_attachment(self, attachment_ids=[]):
        ...
        for attachment in attachments:
            ...
            decoders = self.env[
                "account.move"
            ]._get_create_invoice_from_attachment_decoders()
            invoice = False
            ...
            for seq_field in sequence_fields:
                if not journal[seq_field]:
                    vals = {
                        ...
                        "code": f"SECUR{journal.id}-{seq_field}",
    ...
    def _is_payment_method_available(self, payment_method_code):
        ...
        journal_types = next(
            right
            for left, operator, right in domain
            if left == "type" and operator in ("in", "=")
        )
