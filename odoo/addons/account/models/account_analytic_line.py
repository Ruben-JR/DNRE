...
class AccountAnalyticAccount(models.Model):
    ...
    vendor_bill_count = fields.Integer(
        "Vendor Bill Count", compute="_compute_vendor_bill_count"
    )
    ...
    def _check_company_consistency(self):
        ...
        if self._cr.fetchone():
            raise UserError(
                _(
                    "You can't set a different company on your analytic account since there are some journal items linked to it."
                )
            )
    ...
    def _compute_invoice_count(self):
        ...
        moves_count_mapping = {
            g["analytic_account_id"][0]: g["move_id"] for g in groups
        }
        for account in self:
    ...
    def action_view_invoice(self):
        ...
        result = {
            ...
            "domain": [
                ("id", "in", self.line_ids.move_id.move_id.ids),
...
class AccountAnalyticTag(models.Model):
    ...
    def _check_company_consistency(self):
        ...
        if self._cr.fetchone():
            raise UserError(
                _(
                    "You can't set a different company on your analytic tags since there are some journal items linked to it."
                )
            )
