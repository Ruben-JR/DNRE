...
class IrModuleModule(models.Model):
    ...
    def write(self, values):
        ...
        if (
            new_state in ("to upgrade", "to install")
            and "account_reports" in mod_names
            and "account" not in mod_names
        ):
            ...
            if parse_version(invoicing_mod.latest_version) < parse_version(
                f"{series}.1.2"
            ) and invoicing_mod.state not in ("to install", "to upgrade"):
                raise UserError(
                    _("Please update the Invoicing module before continuing.")
                )
