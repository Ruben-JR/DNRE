...
class AccountBankStmtCashWizard(models.Model):
    ...
    cashbox_lines_ids = fields.One2many(
        "account.cashbox.line", "cashbox_id", string="Cashbox Lines"
    )
    ...
    @api.depends(
        "cashbox_lines_ids", "cashbox_lines_ids.coin_value", "cashbox_lines_ids.number"
    )
    ...
    def default_get(self, fields):
        ...
        if (
            "start_bank_stmt_ids" in fields
            and not vals.get("start_bank_stmt_ids")
            and statement_id
            and balance == "start"
        ):
            vals["start_bank_stmt_ids"] = [(6, 0, [statement_id])]
        if (
            "end_bank_stmt_ids" in fields
            and not vals.get("end_bank_stmt_ids")
            and statement_id
            and balance == "close"
        ):
...
class AccountBankStatement(models.Model):
    ...
    def _compute_starting_balance(self):
        for statement in self:
            if (
                statement.previous_statement_id.balance_end_real
    ...
    def _compute_ending_balance(self):
        latest_statement = self.env["account.bank.statement"].search(
            [("journal_id", "=", self[0].journal_id.id)], limit=1
        )
        for statement in self:
            ...
            if statement.journal_type == "bank":
                ...
                if (
                    latest_statement.id
                    and statement.id == latest_statement.id
    ...
    def _end_balance(self):
        for statement in self:
            ...
            statement.balance_end = (
                statement.balance_start + statement.total_entry_encoding
            )
    ...
    def _default_journal(self):
        ...
        if journal_type:
            return self.env["account.journal"].search(
                [("type", "=", journal_type), ("company_id", "=", company_id)], limit=1
            )
    ...
    cashbox_start_id = fields.Many2one(
        "account.bank.statement.cashbox", string="Starting Cashbox"
    )
    cashbox_end_id = fields.Many2one(
        "account.bank.statement.cashbox", string="Ending Cashbox"
    )
        ...
        if values.get("date") or values.get("journal"):
            ...
            if to_recompute:
                self.env.add_to_compute(
                    self._fields["previous_statement_id"], to_recompute
                )
            ...
            if next_statements_to_recompute:
                self.env.add_to_compute(
                    self._fields["previous_statement_id"], next_statements_to_recompute
                )
    ...
    def _compute_all_lines_reconciled(self):
        for statement in self:
            statement.all_lines_reconciled = all(
                st_line.is_reconciled for st_line in statement.line_ids
            )
    ...
    def _check_journal(self):
        for statement in self:
            if any(
                st_line.journal_id != statement.journal_id
                for st_line in statement.line_ids
            ):
                raise ValidationError(
                    _(
                        "The journal of a bank statement line must always be the same as the bank statement one."
                    )
                )
    ...
    def button_validate(self):
        if any(
            ...
            raise UserError(
                _(
                    "All the account entries lines must be processed in order to validate the statement."
                )
            )
        ...
        for statement in self:
            ...
            if statement.journal_id.type == "bank":
                ...
                )._render(statement.id)
                self.env["ir.attachment"].create(
                    {
                        "name": statement.name
                        and _("Bank Statement %s.pdf", statement.name)
                        or _("Bank Statement.pdf"),
                        "type": "binary",
                        "raw": content,
    ...
    def _get_last_sequence_domain(self, relaxed=False):
        ...
        if not relaxed:
            ...
            if sequence_number_reset == "year":
                where_string += (
                    " AND date_trunc('year', date) = date_trunc('year', %(date)s) "
                )
                param["date"] = self.date
            elif sequence_number_reset == "month":
                where_string += (
                    " AND date_trunc('month', date) = date_trunc('month', %(date)s) "
                )
                param["date"] = self.date
...
class AccountBankStatementLine(models.Model):
    ...
    state = fields.Selection(
        related="statement_id.state", string="Status", readonly=True
    )
    country_code = fields.Char(related="company_id.account_fiscal_country_id.code")
    ...
    def _prepare_liquidity_move_line_vals(self):
        ...
        if self.foreign_currency_id and journal_currency:
            ...
            if self.foreign_currency_id == company_currency:
                ...
                amount_currency = self.amount
                balance = journal_currency._convert(
                    amount_currency, company_currency, journal.company_id, self.date
                )
    ...
    def _prepare_counterpart_move_line_vals(self, counterpart_vals, move_line=None):
        ...
        statement_line_rate = (
            (self.amount_currency / self.amount) if self.amount else 0.0
        )

        balance_to_reconcile = counterpart_vals.pop("balance", None)
        amount_residual = (
            -counterpart_vals.pop(
                "amount_residual", move_line.amount_residual if move_line else 0.0
            )
            if balance_to_reconcile is None
            else balance_to_reconcile
        )
        ...
        if currency_id == journal_currency.id and journal_currency != company_currency:
            if foreign_currency != company_currency:
                amounts[company_currency.id] = journal_currency._convert(
                    amounts[currency_id],
                    company_currency,
                    journal.company_id,
                    self.date,
            ...
            if statement_line_rate:
                amounts[foreign_currency.id] = (
                    amounts[currency_id] * statement_line_rate
                )
        elif currency_id == foreign_currency.id and self.foreign_currency_id:
            if statement_line_rate:
                amounts[journal_currency.id] = (
                    amounts[foreign_currency.id] / statement_line_rate
                )
                if foreign_currency != company_currency:
                    amounts[company_currency.id] = journal_currency._convert(
                        amounts[journal_currency.id],
            ...
            amounts[journal_currency.id] = company_currency._convert(
                amounts[company_currency.id],
                journal_currency,
                journal.company_id,
                self.date,
            )
            if statement_line_rate:
                amounts[foreign_currency.id] = (
                    amounts[journal_currency.id] * statement_line_rate
                )
        ...
        return {
            **counterpart_vals,
            ...
            "move_id": self.move_id.id,
            "partner_id": self.partner_id.id
            or counterpart_vals.get(
                "partner_id", move_line.partner_id.id if move_line else False
            ),
            "currency_id": currency_id,
            "account_id": counterpart_vals.get(
                "account_id", move_line.account_id.id if move_line else False
            ),
    ...
    def _prepare_move_line_default_vals(self, counterpart_account_id=None):
        ...
        counterpart_vals = {
            ...
            "amount_residual": liquidity_line_vals["debit"]
            - liquidity_line_vals["credit"],
        }

        if (
            self.foreign_currency_id
            and self.foreign_currency_id != self.company_currency_id
        ):
        ...
        counterpart_line_vals = self._prepare_counterpart_move_line_vals(
            counterpart_vals
        )
    ...
    def _check_amounts_currencies(self):
        ...
        for st_line in self:
            if st_line.journal_id != st_line.statement_id.journal_id:
                raise ValidationError(
                    _(
                        "The journal of a statement line must always be the same as the bank statement one."
                    )
                )
            ...
            if not st_line.foreign_currency_id and st_line.amount_currency:
                raise ValidationError(
                    _(
                        "You can't provide an amount in foreign currency without specifying a foreign currency."
                    )
                )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if statement.state != "open" and self._context.get(
                "check_move_validity", True
            ):
                raise UserError(
                    _("You can only create statement line in open bank statements.")
                )
        ...
        for st_line in self.with_context(skip_account_move_synchronization=True):
            ...
            if "state" in changed_fields:
                if (st_line.state == "open" and move.state != "draft") or (
                    st_line.state in ("posted", "confirm") and move.state != "posted"
                ):
                    raise UserError(
                        _(
                        ...
                        % (
                            ...
                            st_line.statement_id.display_name,
                        )
                    )
    ...
    def _prepare_reconciliation(self, lines_vals_list, allow_partial=False):
        ...
        company_currency = journal.company_id.currency_id
        foreign_currency = (
            self.foreign_currency_id or journal.currency_id or company_currency
        )
        ...
        for vals in lines_vals_list:
            ...
            if "id" in vals:
                ...
                if any(
                    x in vals
                    for x in ("balance", "amount_residual", "amount_residual_currency")
                ):
        ...
        for line, counterpart_vals in zip(existing_lines, to_process_vals):
            line_vals = self._prepare_counterpart_move_line_vals(
                counterpart_vals, move_line=line
            )
            ...
            if i == len(existing_lines):
                ...
                if (
                    partial_rec_needed
                    and sign * (total_amount_currency + amount_currency) < 0.0
                ):
                    ...
                    line_vals = self._prepare_counterpart_move_line_vals(
                        {
                            **counterpart_vals,
                            ...
                            "amount_residual_currency": -math.copysign(
        ...
        if foreign_currency.is_zero(
            total_amount_currency
        ) and not company_currency.is_zero(total_balance):
        ...
        if self.company_currency_id.round(total_balance):
            counterpart_vals = {
                "name": "{}: {}".format(self.payment_ref, _("Open Balance")),
            ...
            if partner:
                if self.amount > 0:
                    open_balance_account = partner.with_company(
                        self.company_id
                ...
                if self.amount > 0:
                    open_balance_account = self.company_id.partner_id.with_company(
                        self.company_id
                    ).property_account_receivable_id
                else:
                    open_balance_account = self.company_id.partner_id.with_company(
