...
class AccountGroupTemplate(models.Model):
    ...
    parent_id = fields.Many2one(
        "account.group.template", index=True, ondelete="cascade"
    )
...
class AccountChartTemplate(models.Model):
    ...
    def _load(self, sale_tax_rate, purchase_tax_rate, company):
        ...
        if existing_accounts:
            ...
            prop_values = [
                f"account.account,{account_id}" for account_id in existing_accounts.ids
            ]
        ...
        account_type_current_assets = self.env.ref(
            "account.data_account_type_current_assets"
        )
    ...
    def existing_accounting(self, company_id):
        ...
        if (
            ...
            .sudo()
            .search(
                [("company_id", "=", company_id.id), ("state", "!=", "draft")], limit=1
            )
        ):
            return True
        return False

    def _create_tax_templates_from_rates(
        ...
        if not self.complete_tax_set:
            ref_taxs = obj_tax_temp.search(
                [
                    ("type_tax_use", "=", "sale"),
                    ("chart_template_id", "in", all_parents),
                ],
            ...
            ref_taxs.write(
                {
                    "amount": sale_tax_rate,
                    "name": _("Tax %.2f%%") % sale_tax_rate,
                    "description": "%.2f%%" % sale_tax_rate,
                }
            )
            ref_taxs = obj_tax_temp.search(
                [
            ...
            ref_taxs.write(
                {
                    ...
                    "name": _("Tax %.2f%%") % purchase_tax_rate,
                    "description": "%.2f%%" % purchase_tax_rate,
                }
            )
        return True
...
class AccountTaxRepartitionLineTemplate(models.Model):
    ...
    plus_report_line_ids = fields.Many2many(
    ...
    minus_report_line_ids = fields.Many2many(
    ...
    def create(self, vals):
        if vals.get("plus_report_line_ids"):
            vals["plus_report_line_ids"] = self._convert_tag_syntax_to_orm(
                vals["plus_report_line_ids"]
            )

        if vals.get("minus_report_line_ids"):
            vals["minus_report_line_ids"] = self._convert_tag_syntax_to_orm(
                vals["minus_report_line_ids"]
            )
        ...
        if vals.get("use_in_tax_closing") is None:
            if not vals.get("account_id"):
                ...
                internal_group = (
                    ...
                    .user_type_id.internal_group
                )
                vals["use_in_tax_closing"] = not (
                    internal_group == "income" or internal_group == "expense"
                )
    ...
    def validate_tags(self):
        all_tax_rep_lines = self.mapped("plus_report_line_ids") + self.mapped(
        ...
        if lines_without_tag:
            raise ValidationError(
                _(
                    "The following tax report lines are used in some tax distribution template though they don't generate any tag: %s . This probably means you forgot to set a tag_name on these lines.",
                    str(lines_without_tag.mapped("name")),
                )
            )
    ...
    def _get_tags_to_add(self):
        ...
        tags_to_add += self.plus_report_line_ids.mapped("tag_ids").filtered(
            lambda x: not x.tax_negate
        )
        tags_to_add += self.minus_report_line_ids.mapped("tag_ids").filtered(
