...
class AccountBankStatementLine(models.Model):
    ...
    def _populate_factories(self):
        ...
        def get_date(random, values, **kwargs):
            ...
            """
            statement_date = (
                self.env["account.bank.statement"].browse(values["statement_id"]).date
            )
        ...
        self = self.with_prefetch(
            self.env.registry.populated_models["account.bank.statement"]
        )
        return [
            (
                "statement_id",
            ...
            (
                "payment_ref",
                populate.constant("statement_{values[statement_id]}_{counter}"),
            ),
    ...
    def _populate(self, size):
        ...
        for statement in statements:
            ...
            previous[statement.journal_id] = statement.balance_end_real = (
