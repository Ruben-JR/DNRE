...
class TaxReportTest(AccountTestInvoicingCommon):
    ...
    def _get_tax_tags(self, tag_name=None):
        domain = [
            ("country_id", "=", self.test_country_1.id),
            ("applicability", "=", "taxes"),
        ]
        if tag_name:
    ...
    def test_write_single_line_tagname(self):
        ...
        self.assertEqual(
            len(self._get_tax_tags(original_tag_name)),
            0,
            "The original tag name of the line should not correspond to any tag anymore.",
        )
        self.assertEqual(
            original_tags,
            self.tax_report_line_1_55.tag_ids,
            "The tax report line should still be linked to the same tags.",
        )
        self.assertEqual(
    ...
    def test_write_single_line_remove_tagname(self):
                ...
                "invoice_repartition_line_ids": [
                    ...
                    (
                        ...
                        0,
                        {
                            ...
                            "tag_ids": [
                                (6, 0, self.tax_report_line_1_55.tag_ids[0].ids)
                            ],
                        },
                    ),
                ],
        ...
        self.assertTrue(
            any(
                line.tax_tag_ids == self.tax_report_line_1_55.tag_ids[0]
        ...
        self.assertFalse(
            self.tax_report_line_1_55.tag_name,
            "The tag name for line 55 should now be None",
        )
        self.assertEqual(
            len(self._get_tax_tags(tag_name_before)),
            0,
            "None of the original tags for this line should be left after setting tag_name to None if no other line was using this tag_name.",
        )
        self.assertEqual(
            len(self._get_tax_tags()),
            tag_nber_before - 2,
            "No new tag should have been created, and the two that were assigned to the report line should have been removed.",
        )
        self.assertFalse(
            test_tax.mapped("invoice_repartition_line_ids.tag_ids"),
            "There should be no tag left on test tax's repartition lines after the removal of tag 55.",
        )
        self.assertFalse(
            test_invoice.mapped("line_ids.tax_tag_ids"),
            "The link between test invoice and tag 55 should have been broken. There should be no tag left on the invoice's lines.",
        )
    ...
    def test_write_multi_no_change(self):
        ...
        self.assertEqual(
            tags_before,
            tags_after,
            "Re-assigning the same tag_name should keep the same tags.",
        )
    ...
    def test_edit_line_shared_tags(self):
        ...
        self.assertEqual(
            self.tax_report_line_2_1.tag_name,
            self.tax_report_line_1_1.tag_name,
            "Modifying the tag name of a tax report line sharing it with another one should also modify the other's.",
        )
    ...
    def test_edit_multi_line_tagname_all_different_new(self):
        ...
        self.assertNotEqual(
            new_tags.ids, previous_tag_ids, "All the tags should have changed"
        )
        self.assertEqual(
            len(new_tags),
            2,
            "Only two distinct tags should be assigned to all the lines after writing tag_name on them all",
        )
        surviving_tags = self.env["account.account.tag"].search(
            [("id", "in", previous_tag_ids)]
        )
        self.assertEqual(
            len(surviving_tags), 0, "All former tags should have been deleted"
        )
        self.assertEqual(
            self.tax_report_line_1_1.tag_ids,
            self.tax_report_line_2_1.tag_ids,
            "The report lines initially sharing their tag_name with the written-on lines should also have been impacted",
        )
    ...
    def test_remove_line_dependency(self):
        ...
        self.assertEqual(
            len(self.tax_report_line_1_1.tag_ids),
            0,
            "Setting the tag_name to None should have removed the tags.",
        )
        self.assertEqual(
            self.tax_report_line_2_1.tag_ids,
            tags_before,
            "Setting tag_name to None on a line linked to another one via tag_name should break this link.",
        )
    ...
    def test_tax_report_change_country(self):
        ...
        for original, copy in zip(
            self.tax_report_1.get_lines_in_hierarchy(),
            copied_report_1.get_lines_in_hierarchy(),
        ):
            self.assertEqual(
                original.tag_ids,
                copy.tag_ids,
                "Copying the lines of a tax report should keep the same tags on lines",
            )
        ...
        for original, copy in zip(
            ...
            if original.tag_ids or copy.tag_ids:
                self.assertNotEqual(
                    original.tag_ids,
                    copy.tag_ids,
                    "Changing the country of a copied report should create brand new tags for all of its lines",
                )

        for original, copy in zip(
            self.tax_report_1.get_lines_in_hierarchy(),
            copied_report_2.get_lines_in_hierarchy(),
        ):
            self.assertEqual(
                original.tag_ids,
                copy.tag_ids,
                "Changing the country of a copied report should not impact the other copies or the original report",
            )
        ...
        for line in self.tax_report_2.get_lines_in_hierarchy():
            if line == self.tax_report_line_2_42:
                ...
                self.assertEqual(
                    line.tag_ids.ids,
                    original_report_2_tags[line.id],
                    "The tax report lines not sharing their tags with any other report should keep the same tags when the country of their report is changed",
                )
            elif line.tag_ids or original_report_2_tags[line.id]:
                self.assertNotEqual(
                    line.tag_ids.ids,
                    original_report_2_tags[line.id],
                    "The tax report lines sharing their tags with other report should receive new tags when the country of their report is changed",
                )
    ...
    def test_unlink_report_line_tags(self):
        ...
        check_tags_unlink(
            ...
            self.tax_report_line_2_42,
            True,
            "Unlinking one line not sharing its tags should also unlink them",
        )
        check_tags_unlink(
            "01",
            self.tax_report_line_1_1,
            False,
            "Unlinking one line sharing its tags with others should keep the tags",
        )
        check_tags_unlink(
            "100",
            self.tax_report_line_1_6 + self.tax_report_line_2_6,
            True,
            "Unlinkink all the lines sharing the same tags should also unlink them",
        )
