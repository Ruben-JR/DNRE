...
class TestReconciliationMatchingRules(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.bank_st, cls.bank_st_2, cls.cash_st = cls.env[
    ...
    def test_matching_fields(self):
        ...
        self._check_statement_matching(
            ...
            {
                self.bank_line_1.id: {
                    "aml_ids": [self.invoice_line_1.id],
                    "model": self.rule_1,
                ...
                self.cash_line_1.id: {
                    "aml_ids": [self.invoice_line_4.id],
                    "model": self.rule_1,
                    "partner": self.cash_line_1.partner_id,
                },
            },
        )
        ...
        self._check_statement_matching(
            self.rule_1,
            {
                self.bank_line_3.id: {
                    ...
                    "model": self.rule_1,
                    "partner": self.bank_line_3.partner_id,
                },
                self.bank_line_4.id: {
                    "aml_ids": [self.invoice_line_7.id],
                    "model": self.rule_1,
                    "partner": self.bank_line_4.partner_id,
                },
                self.bank_line_5.id: {
                    "aml_ids": [self.invoice_line_6.id],
                    "model": self.rule_1,
                    "partner": self.bank_line_5.partner_id,
                },
            },
    ...
    def test_matching_fields_match_journal_ids(self):
        ...
        self.rule_1.match_journal_ids |= (
            self.bank_st.journal_id + self.cash_st.journal_id
        )
    ...
    def test_zero_payment_tolerance_auto_reconcile(self):
        ...
        for inv_type, bsl_sign in (("out_invoice", 1), ("in_invoice", -1)):
            ...
            self.assertRecordValues(
                st_line.line_ids.sorted(lambda x: abs(x.balance)),
                [
                    # pylint: disable=bad-whitespace
                    {
                        "balance": bsl_sign * -10.0,
                        "account_id": invl.account_id.id,
                        "reconciled": False,
                    },
                    {
                        "balance": bsl_sign * -1000.0,
                        "account_id": invl.account_id.id,
                        "reconciled": True,
                    },
                    {
                        "balance": bsl_sign * 1010.0,
        ...
        for inv_type, bsl_sign in (("out_invoice", 1), ("in_invoice", -1)):
            ...
            self.assertRecordValues(
                ...
                [
                    ...
                    {
                        "balance": bsl_sign * -1980.0,
                        "account_id": self.company_data["default_account_revenue"].id,
                        "reconciled": False,
                    },
                    {
                        "balance": bsl_sign * 1990.0,
                        "account_id": invl.account_id.id,
                        "reconciled": False,
                    },
                ],
    ...
    def test_percentage_st_line_auto_reconcile(self):
        ...
        for bsl_sign in (1, -1):
            ...
            self.assertRecordValues(
                st_line.line_ids.sorted(lambda x: x.account_id),
                [
                    # pylint: disable=bad-whitespace
                    {
                        "balance": bsl_sign * 1000.0,
    ...
    def test_larger_invoice_auto_reconcile(self):
        ...
        self.assertEqual(
            self.invoice_line_1.amount_residual,
            60.0,
            "The invoice should have been partially reconciled",
        )
        ...
        self.assertRecordValues(
            ...
            [
                ...
                },
                {
                    ...
                    "credit": 0.0,
                    "tax_ids": [],
                    "tax_line_id": self.tax21.id,
                },
                {
                    "partner_id": self.partner_1.id,
                    "debit": 60.5,
                    "credit": 0.0,
                    "tax_ids": [self.tax12.id],
                    "tax_line_id": False,
                },
                {
                    "partner_id": self.partner_1.id,
    ...
    def test_inv_matching_rule_auto_rec_no_partner_with_writeoff(self):
        ...
        self.assertRecordValues(
            ...
            [
                ...
                },
                {
                    ...
                    "debit": 0.0,
                    "credit": 100.0,
                    "account_id": self.invoice_line_1.account_id.id,
                    "reconciled": True,
                },
            ],
        )
        ...
        self.assertEqual(
            self.invoice_line_1.amount_residual,
            0.0,
            "The invoice should have been fully reconciled",
        )

    def test_partner_mapping_rule(self):
        self.bank_line_1.write(
            {"partner_id": None, "payment_ref": "toto42", "narration": None}
        )
        ...
        for rule in self.rule_1 + self.rule_2:
            ...
            matching_amls = (
                rule.rule_type == "invoice_matching" and self.invoice_line_1.ids or []
            )
            result_status = (
                rule.rule_type == "writeoff_suggestion"
            ...
            self.bank_line_1.write(
                {"narration": None}
            )  # Reset from possible previous iteration
    ...
    def test_match_multi_currencies(self):
        ...
        self._check_statement_matching(
            ...
            {
                statement_line.id: {
                    "aml_ids": (move_line_1 + move_line_2).ids,
                    "model": matching_rule,
    ...
    def test_inv_matching_with_write_off(self):
        ...
        self._check_statement_matching(
            self.rule_1,
            {
                statement_line.id: {
                    "aml_ids": self.invoice_line_1.ids,
                    "model": self.rule_1,
                    "partner": self.invoice_line_1.partner_id,
                    "status": "write_off",
                },
            },
            self.bank_st,
        ...
        self.assertEqual(
            len(matching_result[statement_line.id].get("write_off_vals", [])),
            1,
            "Exactly one write-off line should be proposed.",
        )
    ...
    def test_matching_with_write_off_foreign_currency(self):
        ...
        self.assertRecordValues(
            st_line.line_ids.sorted("amount_currency"),
            [
                {
    ...
    def test_payment_similar_communications(self):
        ...
        self._check_statement_matching(
            ...
            {
                self.bank_line_1.id: {
                    "aml_ids": (pmt_line_1 + pmt_line_2).ids,
                    "model": self.rule_1,
    ...
    def test_no_amount_check_keep_first(self):
        ...
        self._check_statement_matching(
            self.rule_1,
            {
                self.bank_line_1.id: {"aml_ids": []},
                self.bank_line_2.id: {
                    "aml_ids": [self.invoice_line_1.id, self.invoice_line_2.id],
                    "model": self.rule_1,
                    "partner": self.bank_line_2.partner_id,
                },
            },
    ...
    def test_no_amount_check_exact_match(self):
        ...
        self._check_statement_matching(
            self.rule_1,
            {
                self.bank_line_1.id: {"aml_ids": []},
                self.bank_line_2.id: {
                    "aml_ids": [self.invoice_line_3.id],
                    "model": self.rule_1,
                    "partner": self.bank_line_2.partner_id,
                },
            },
