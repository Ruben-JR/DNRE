...
class TestReconciliationExec(TestAccountReconciliationCommon):
    ...
    def test_statement_euro_invoice_usd_transaction_euro_full(self):
        ...
        self.assertEqual(
            move.payment_state, "paid", "The invoice should be paid by now"
        )
        self.assertTrue(
            receivable_line.reconciled, "The invoice should be totally reconciled"
        )
        self.assertTrue(
            receivable_line.full_reconcile_id,
            "The invoice should have a full reconcile number",
        )
        self.assertEqual(
            receivable_line.amount_residual,
            0,
            "The invoice should be totally reconciled",
        )
        self.assertEqual(
    ...
    def test_balanced_exchanges_gain_loss(self):
        ...
        for line in reconcile.line_id:
            res_account = result.setdefault(
                line.account_id, {"debit": 0.0, "credit": 0.0, "count": 0}
            )
        ...
        self.assertTrue(
            exchange_loss_line, "There should be one move line of 0.01 EUR in credit"
        )
    ...
    def test_manual_reconcile_wizard_opw678153(self):
        ...
        for aml in aml_recs:
            self.assertTrue(
                aml.reconciled, "The journal item should be totally reconciled"
            )
            self.assertEqual(
                aml.amount_residual, 0, "The journal item should be totally reconciled"
            )
            self.assertEqual(
                aml.amount_residual_currency,
                0,
                "The journal item should be totally reconciled",
            )
    ...
    def test_partial_reconcile_currencies_01(self):
        ...
        reconciled_amls = (
            ...
            + debit_line_b
            + debit_line_c
            + (invoice_a + invoice_b).mapped("line_ids")
        ...
        full_rec_payable = full_rec_move.line_ids.filtered(
    ...
    def test_unreconcile_exchange(self):
        ...
        self.assertEqual(
            len(payment.line_ids.mapped("full_reconcile_id").exchange_move_id), 1
        )
    ...
    def test_revert_payment_and_reconcile(self):
        ...
        bank_line = payment.line_ids.filtered(
            lambda l: l.account_id.id
            == self.bank_journal_usd.company_id.account_journal_payment_debit_account_id.id
        )
        ...
        self.assertEqual(
            reversed_bank_line.move_id.id, reversed_customer_line.move_id.id
        )

        self.assertEqual(
            reversed_bank_line.full_reconcile_id.id, bank_line.full_reconcile_id.id
        )
        self.assertEqual(
            reversed_customer_line.full_reconcile_id.id,
            customer_line.full_reconcile_id.id,
        )
    ...
    def test_revert_payment_and_reconcile_exchange(self):
        ...
        def _determine_debit_credit_line(move):
            line_ids_reconciliable = move.line_ids.filtered(
                ...
                or l.account_id.internal_type == "liquidity"
            )
            return line_ids_reconciliable.filtered(
                lambda l: l.debit
            ), line_ids_reconciliable.filtered(lambda l: l.credit)
        ...
        def _move_revert_test_pair(move, revert):
            ...
            if move_lines[0] and revert_lines[1]:
                ...
                self.assertEqual(
                    move_lines[0].full_reconcile_id.id,
                    revert_lines[1].full_reconcile_id.id,
                )

            if move_lines[1] and revert_lines[0]:
                self.assertTrue(move_lines[1].full_reconcile_id.exists())
                self.assertEqual(
                    move_lines[1].full_reconcile_id.id,
                    revert_lines[0].full_reconcile_id.id,
                )
        ...
        inv.js_assign_outstanding_line(credit_aml.id)
        self.assertTrue(
            inv.payment_state in ("in_payment", "paid"), "Invoice should be paid"
        )
        ...
        reverted_payment_move = payment_move._reverse_moves(
            [{"date": time.strftime("%Y") + "-08-01"}], cancel=True
        )
    ...
    def test_reconciliation_with_currency(self):
        ...
        to_reconcile = (
            (general_move1 + general_move2 + general_move3)
            .mapped("line_ids")
    ...
    def test_inv_refund_foreign_payment_writeoff_domestic2(self):
        ...
        inv2 = self.create_invoice(
            move_type="out_refund", invoice_amount=400, currency_id=self.currency_usd_id
        )
        ...
        inv1_receivable = inv1.line_ids.filtered(
            lambda l: l.account_id.internal_type == "receivable"
        )
        inv2_receivable = inv2.line_ids.filtered(
            lambda l: l.account_id.internal_type == "receivable"
        )
        ...
        move_balance = self.env["account.move"].create(
            {
                ...
                "line_ids": [
                    (
                        ...
                        {
                            "credit": 160.16,
                            "account_id": inv1_receivable.account_id.id,
        ...
        move_balance_receiv = move_balance.line_ids.filtered(
            lambda l: l.account_id.internal_type == "receivable"
        )

        (
            inv1_receivable + inv2_receivable + pay_receivable + move_balance_receiv
        ).reconcile()

        self.assertTrue(inv1_receivable.full_reconcile_id.exists())
        self.assertEqual(
            inv1_receivable.full_reconcile_id, inv2_receivable.full_reconcile_id
        )
        self.assertEqual(
            inv1_receivable.full_reconcile_id, pay_receivable.full_reconcile_id
        )
        self.assertEqual(
            inv1_receivable.full_reconcile_id, move_balance_receiv.full_reconcile_id
        )

        self.assertTrue(
            inv1.payment_state in ("in_payment", "paid"), "Invoice should be paid"
        )
    ...
    def test_inv_refund_foreign_payment_writeoff_domestic3(self):
        ...
        inv2 = self.create_invoice(
            move_type="out_refund", invoice_amount=225, currency_id=self.currency_usd_id
        )
    ...
    def test_inv_refund_foreign_payment_writeoff_domestic5(self):
        ...
        self.assertEqual(
            inv1_receivable.full_reconcile_id, payment_receivable.full_reconcile_id
        )
    ...
    def test_inv_refund_foreign_payment_writeoff_domestic8(self):
        ...
        self.assertEqual(
            move_balance_receiv.full_reconcile_id, inv1_receivable.full_reconcile_id
        )
