...
class TestInvoiceTaxes(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.tax_tag_pos = cls.tax_report_line.tag_ids.filtered(
        ...
        cls.base_tag_pos = cls.base_tax_report_line.tag_ids.filtered(
            lambda x: not x.tax_negate
        )
        cls.base_tag_neg = cls.base_tax_report_line.tag_ids.filtered(
    ...
    def test_affecting_base_amount(self):
        ...
        invoice.action_post()
        self.assertRecordValues(
            invoice.line_ids.filtered("tax_line_id").sorted(lambda x: x.price_unit),
            [
                {
                    "name": self.percent_tax_1_incl.name,
                    "tax_base_amount": 100,
    ...
    def test_tax_calculation_multi_currency(self):
        ...
        self.assertRecordValues(
            invoice.line_ids.filtered(lambda l: not l.name), [{"balance": 686.54}]
        )
