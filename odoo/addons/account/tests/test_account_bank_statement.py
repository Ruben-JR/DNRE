...
class TestAccountBankStatementLine(TestAccountBankStatementCommon):
    ...
    def _test_statement_line_reconciliation(
        ...
        statement.button_post()
        counterpart_lines = test_invoices.mapped("line_ids").filtered(
    ...
    def test_reconciliation_statement_line_state(self):
        ...
        receivable_acc_2 = self.copy_account(
            self.company_data["default_account_receivable"]
        )
        payment_account = (
            self.bank_journal_1.company_id.account_journal_payment_debit_account_id
        )
        ...
        test_line_1 = test_move.line_ids.filtered(
            lambda line: line.account_id == receivable_acc_1
        )
        test_line_2 = test_move.line_ids.filtered(
            lambda line: line.account_id == receivable_acc_2
        )
        test_line_3 = test_move.line_ids.filtered(
            lambda line: line.account_id == payment_account
        )
    ...
    def test_bank_statement_line_analytic(self):
        ...
        self.assertRecordValues(
            statement_line.line_ids.sorted("balance"),
            [
    ...
    def test_reconciliation_line_with_no_partner(self):
        ...
        outstanding_account = self.env["account.account"].search(
            [("code", "=", "101402"), ("company_id", "=", self.env.company.id)]
        )
