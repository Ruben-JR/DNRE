...
class TestAccountJournal(AccountTestInvoicingCommon):
    ...
    def test_account_journal_alias_name(self):
        ...
        self.assertEqual(
            journal._get_alias_values(journal.type)["alias_name"],
            "Miscellaneous Operations-company_1_data",
        )
        self.assertEqual(
            journal._get_alias_values(journal.type, "ぁ")["alias_name"],
            "MISC-company_1_data",
        )
        journal.name = "ぁ"
        self.assertEqual(
            journal._get_alias_values(journal.type)["alias_name"], "MISC-company_1_data"
        )
        journal.code = "ぁ"
        self.assertEqual(
            journal._get_alias_values(journal.type)["alias_name"],
            "general-company_1_data",
        )
        ...
        self.assertEqual(
            journal_2._get_alias_values(journal_2.type)["alias_name"],
            "Customer Invoices-" + company_2_id,
        )
        journal_2.name = "ぁ"
        self.assertEqual(
            journal_2._get_alias_values(journal_2.type)["alias_name"],
