...
class GoogleAuth(http.Controller):
    ...
    def oauth2callback(self, **kw):
        ...
        if kw.get("code"):
            ...
            request.env.user.google_cal_account_id._set_auth_tokens(
                access_token, refresh_token, ttl
            )
            return request.redirect(url_return)
        elif kw.get("error"):
            return request.redirect("{}{}{}".format(url_return, "?error=", kw["error"]))
        else:
            return request.redirect("{}{}".format(url_return, "?error=Unknown_error"))
