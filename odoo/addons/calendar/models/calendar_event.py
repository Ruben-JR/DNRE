...
class Meeting(models.Model):
    ...
    def _find_attendee(self):
        ...
        if (
            self.partner_id in event_checked_attendees
            and self.partner_id in self.attendee_ids.partner_id
        ):
            return self.attendee_ids.filtered(
                lambda attendee: attendee.partner_id == self.partner_id
            )[:1]

        attendee = self.attendee_ids.filtered(
            lambda att: att.partner_id in event_checked_attendees
            and att.state != "needsAction"
        )
        return attendee[:1]
