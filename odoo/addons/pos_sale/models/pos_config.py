...
class PosConfig(models.Model):
    ...
    def _get_default_pos_team(self):
        default_sale_team = self.env.ref(
            "sales_team.pos_sales_team", raise_if_not_found=False
        )
        if default_sale_team and (
            not default_sale_team.company_id
            or default_sale_team.company_id == self.company_id
        ):
            self.crm_team_id = default_sale_team
        else:
            self.crm_team_id = self.env["crm.team"].search(
                [
                    "|",
