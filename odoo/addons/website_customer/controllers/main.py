...
class WebsiteCustomer(http.Controller):
    ...
    def sitemap_industry(env, rule, qs):
        ...
        countries = (
            env["res.partner"]
            .sudo()
            .read_group(dom, ["id", "country_id"], groupby="country_id")
        )
    ...
    def customers(self, country=None, industry=None, page=0, **post):
        ...
        if industry:
            ...
            if industry.id not in (
                x["industry_id"][0] for x in industries if x["industry_id"]
            ):
                if industry.exists():
                    industries.append(
                        {
        ...
        if country:
            ...
            if country.id not in (
                x["country_id"][0] for x in countries if x["country_id"]
            ):
                if country.exists():
                    countries.append(
                        {
        ...
        countries.insert(
            0,
            {"country_id_count": country_count, "country_id": (0, _("All Countries"))},
        )
