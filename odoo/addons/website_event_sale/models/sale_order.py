...
class SaleOrder(models.Model):
    ...
    def _website_product_id_change(self, order_id, product_id, qty=0, **kwargs):
        ...
        values = super()._website_product_id_change(
    ...
    def _cart_update(
        ...
        if line_id:
            ...
            if self.env.context.get("event_ticket_id"):
                ticket_domain = expression.AND(
                    [ticket_domain, [("id", "=", self.env.context["event_ticket_id"])]]
                )
        ...
        # case: buying tickets, too much attendees
            ...
            new_qty, set_qty, add_qty = (
                ticket.seats_available,
                ticket.seats_available,
                0,
            )
        values.update(
            super()._cart_update(product_id, line_id, add_qty, set_qty, **kwargs)
        )
