...
class MassMailing(models.Model):
    ...
    is_body_empty = fields.Boolean(
        compute="_compute_is_body_empty",
    ...
    def _compute_mailing_model_real(self):
        for mailing in self:
            mailing.mailing_model_real = (
                (mailing.mailing_model_id.model != "mailing.list")
                and mailing.mailing_model_id.model
                or "mailing.contact"
            )
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            if values.get("subject") and not values.get("name"):
                values["name"] = "{} {}".format(values["subject"], now)
    ...
    def write(self, values):
        ...
        if values.get("ab_testing_enabled") and not values.get("campaign_id"):
            ...
            values["campaign_id"] = (
                ...
                .create(self[0]._get_default_ab_testing_campaign_values(values))
                .id
            )
    ...
    def _action_view_documents_filtered(self, view_filter):
        if view_filter in ("reply", "bounce"):
            ...
            found_traces = self.mailing_trace_ids.filtered(
                lambda trace: trace.links_click_datetime
            )
    ...
    def _convert_inline_images_to_urls(self, body_html):
        ...
        def _image_to_url(b64image: bytes):
            ...
            attachment = self.env["ir.attachment"].create(
                {
                    "datas": b64image,
                    "name": f"cropped_image_mailing_{self.id}",
                    "type": "binary",
                }
            )
            ...
            return "/web/image/{}?access_token={}".format(
                attachment.id,
