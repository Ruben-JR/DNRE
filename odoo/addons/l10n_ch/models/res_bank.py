...
class ResPartnerBank(models.Model):
        ...
        for bank in self:
            if bank.partner_id:
                bank.l10n_ch_show_subscription = (
                    bank.partner_id.ref_company_ids.country_id.code == "CH"
                )
            elif bank.company_id:
                bank.l10n_ch_show_subscription = (
                    bank.company_id.account_fiscal_country_id.code == "CH"
                )
            else:
                bank.l10n_ch_show_subscription = (
                    self.env.company.account_fiscal_country_id.code == "CH"
                )
    ...
    def _eligible_for_qr_code(self, qr_method, debtor_partner, currency):
        if qr_method == "ch_qr":
            ...
            return (
                self.acc_type == "iban"
                and self.partner_id.country_id.code == "CH"
                and (not debtor_partner or debtor_partner.country_id.code == "CH")
