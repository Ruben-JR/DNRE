    ...
    def generate_email(self, res_ids, fields):
        ...
        if self.model == "account.move":
            for record in self.env[self.model].browse(res_ids):
                inv_print_name = self._render_field(
                    "report_name", record.ids, compute_lang=True
                )[record.id]
                new_attachments = []
