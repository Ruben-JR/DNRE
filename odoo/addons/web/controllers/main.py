...
def abort_and_redirect(url):
    ...
    response = http.root.get_response(
        request.httprequest, response, explicit_session=False
    )
...
class HomeStaticTemplateHelpers:
    ...
    def _get_parent_template(self, addon, template):
        ...
        parent_addon, parent_name = (
            tuple(split_name_attempt)
            if len(split_name_attempt) == 2
            else (addon, original_template_name)
        )
        if parent_addon not in self.template_dict:
            if original_template_name in self.template_dict[addon]:
                ...
                raise ValueError(
                    _(
                        "Module %s not loaded or inexistent, or templates of addon being loaded (%s) are misordered"
                    )
                    % (parent_addon, addon)
                )
        ...
        if parent_name not in self.template_dict[parent_addon]:
            raise ValueError(
                _("No template found to inherit from. Module %s and template name %s")
                % (parent_addon, parent_name)
            )
    ...
    def _compute_xml_tree(self, addon, file_name, source):
        ...
        """
        try:
            all_templates_tree = etree.parse(
                io.BytesIO(source), parser=etree.XMLParser(remove_comments=True)
            ).getroot()
        except etree.ParseError as e:
            _logger.error(f"Could not parse file {file_name}: {e.msg}")
            raise e
        ...
        for template_tree in list(all_templates_tree):
            ...
            if self.STATIC_INHERIT_DIRECTIVE in template_tree.attrib:
                inherit_mode = template_tree.attrib.get(
                    self.STATIC_INHERIT_MODE_DIRECTIVE, self.DEFAULT_MODE
                )
                if inherit_mode not in [self.PRIMARY_MODE, self.EXTENSION_MODE]:
                    raise ValueError(
                        _("Invalid inherit mode. Module %s and template name %s")
                        % (addon, template_name)
                    )

                parent_addon, parent_name = self._get_parent_template(
                    addon, template_tree
                )
                ...
                parent_tree = copy.deepcopy(
                    self.template_dict[parent_addon][parent_name]
                )
                ...
                if False:  # self.debug and inherit_mode == self.EXTENSION_MODE:
                    for xpath in xpaths:
                        xpath.insert(
                            0,
                            etree.Comment(
                                f" Modified by {template_name} from {addon} "
                            ),
                        )
                ...
                if template_name in self.template_dict[addon]:
                    raise ValueError(
                        _("Template %s already exists in module %s")
                        % (template_name, addon)
                    )
    ...
    def _concat_xml(self, file_dict):
        ...
        return (
            etree.tostring(root, encoding="utf-8") if root is not None else b"",
            checksum.hexdigest()[:64],
        )
    ...
    def _get_asset_paths(self, bundle):
        ...
        """
        return request.env["ir.asset"]._get_asset_paths(
            addons=self.addons, bundle=bundle, xml=True
        )
    ...
    @classmethod
    def get_qweb_templates_checksum(
        cls, addons=None, db=None, debug=False, bundle=None
    ):
        return cls(addons, db, checksum_only=True, debug=debug)._get_qweb_templates(
            bundle
        )[1]
    ...
    def _write_group_header(self, row, column, label, group, group_depth=0):
        ...
        for field in self.fields[
            ...
            self.write(
                row,
                column,
                str(aggregated_value if aggregated_value is not None else ""),
...
class WebClient(http.Controller):
    ...
    @http.route(
        "/web/webclient/qweb/<string:unique>", type="http", auth="none", cors="*"
    )
    def qweb(self, unique, mods=None, db=None, bundle=None):
        ...
        content = HomeStaticTemplateHelpers.get_qweb_templates(
            mods, db, debug=request.session.debug, bundle=bundle
        )
...
class Proxy(http.Controller):
    @http.route(
        "/web/proxy/post/<path:path>", type="http", auth="user", methods=["GET"]
    )
    def post(self, path):
...
class Session(http.Controller):
    ...
    def change_password(self, fields):
        ...
        if not (
            old_password.strip() and new_password.strip() and confirm_password.strip()
        ):
    ...
    def modules(self):
        ...
        return list(
            request.env.registry._init_modules
            | set([module.current_test] if module.current_test else [])
        )
...
class Binary(http.Controller):
            ...
            .search_read(
                [
                    (
                        "url",
                        "=like",
                        f"/web/assets/%/{extra}/{filename}"
                        if extra
                        else f"/web/assets/%/{filename}",
                    )
                ],
                fields=["id"],
                limit=1,
    ...
    def content_image_backward_compatibility(self, model, id, field, resize=None, **kw):
        ...
        return request.env["ir.http"]._content_image(
            model=model, res_id=id, field=field, width=width, height=height
        )
