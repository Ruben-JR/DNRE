...
class WebsiteSlides(WebsiteProfile):
    ...
    def _set_viewed_slide(self, slide, quiz_attempts_inc=False):
        if (
            ...
            or not slide.channel_id.is_member
        ):
                    ...
                    "answer_ids": [
                        {
                            ...
                            if slide_completed or request.website.is_publisher()
                            else None,
    ...
    def _get_channel_progress(self, channel, include_quiz=False):
        ...
        channel_progress = {sid: dict() for sid in slides.ids}
    ...
    @http.route(
        "/slides/channel/add", type="http", auth="user", methods=["POST"], website=True
    )
        ...
        values.update(
            {
                ...
                if kwargs.get("search_slide_type")
                else None,
    ...
    def slide_get_image(
        ...
        image_base64 = tools.image_process(
            image_base64, size=(int(width), int(height)), crop=crop
        )
    ...
    def prepare_preview(self, **data):
        ...
        if existing_slide:
            preview["error"] = _(
                "This video already exists in this channel on the following slide: %s",
                existing_slide.name,
            )
            return preview
