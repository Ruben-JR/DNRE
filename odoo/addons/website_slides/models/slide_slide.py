...
class Slide(models.Model):
    ...
    def _compute_slides_statistics(self):
        ...
        default_vals = {key: 0 for key in keys + ["total_slides"]}
    ...
    def _compute_slides_statistics_type(self, read_group_res):
        ...
        result = {cid: {key: 0 for key in keys + ["total_slides"]} for cid in self.ids}
    ...
    def _compute_embed_code(self):
        ...
        for record in self:
            ...
            if record.datas and (
                ...
                or record.slide_type in ["document", "presentation"]
            ):
        ...
        if self.datas:
            ...
            if data.startswith(b"%PDF-"):
                pdf = PyPDF2.PdfFileReader(
                    io.BytesIO(data), overwriteWarnings=False, strict=False
                )
                try:
                    pdf.getNumPages()
    ...
    def _compute_website_url(self):
        ...
        for slide in self:
            if (
                ...
                slide.website_url = "{}/slides/slide/{}".format(base_url, slug(slide))
    ...
    def _unlink_except_already_taken(self):
        if self.question_ids and self.channel_id.channel_partner_ids:
            raise UserError(
                _(
                    "People already took this quiz. To keep course progression it should not be deleted."
                )
            )
    ...
    def _action_set_quiz_done(self):
        ...
        for slide in self:
            ...
            if (
                ...
                or user_membership_sudo.completed
    ...
    def _parse_google_document(self, document_id, only_preview_fields):
        ...
        fetch_res = self._fetch_data(
            "https://www.googleapis.com/drive/v2/files/%s" % document_id, params, "json"
        )
        if fetch_res.get("error"):
            return {"error": self._extract_google_error_message(fetch_res.get("error"))}
        ...
        values = {
            ...
            "image_1920": self._fetch_data(
                google_values["thumbnailLink"].replace("=s220", ""), {}, "image"
            )["values"],
