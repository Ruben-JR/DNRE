...
class ValuationReconciliationTestCommon(AccountTestInvoicingCommon):
    ...
    def check_reconciliation(
        ...
        if invoice.is_purchase_document() and any(
            l.is_anglo_saxon_line for l in invoice_line
        ):
            self.assertEqual(
                len(invoice_line),
                2,
            ...
            self.assertTrue(
                valuation_line.reconciled or invoice_line.reconciled,
                "The valuation and invoice line should have been reconciled together.",
            )
        ...
        if invoice.move_type not in ("out_refund", "in_refund"):
            self.assertEqual(
                len(valuation_line),
                1,
                "Only one line should have been written for stock valuation in stock input account",
            )
    ...
    def _change_pickings_date(self, pickings, date):
        ...
        pickings.mapped("move_lines.account_move_ids").write(
            {"name": "/", "state": "draft"}
        )
