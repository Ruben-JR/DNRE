    ...
    def _compute_average_price(self, qty_invoiced, qty_to_invoice, stock_moves):
        ...
        for move in stock_moves:
            if move.origin_returned_move_id:
                returned_quantities[move.origin_returned_move_id.id] += abs(
                    sum(move.sudo().stock_valuation_layer_ids.mapped("quantity"))
                )
        candidates = (
            stock_moves.sudo()
            .filtered(
        ...
        for candidate in candidates:
            ...
            if float_is_zero(
                candidate_quantity, precision_rounding=candidate.uom_id.rounding
            ):
                continue  # correction entries
            if not float_is_zero(
                qty_invoiced, precision_rounding=candidate.uom_id.rounding
            ):
            ...
            if float_is_zero(
                qty_to_take_on_candidates, precision_rounding=candidate.uom_id.rounding
            ):
                break
