...
class WebsiteBlog(http.Controller):
    ...
    def _prepare_blog_values(
        ...
        if active_tag_ids:
            ...
            if fixed_tag_slug != tags:
                new_url = request.httprequest.full_path.replace(
                    "/tag/%s" % tags, "/tag/%s" % fixed_tag_slug, 1
                )
                if (
                    new_url != request.httprequest.full_path
                ):  # check that really replaced and avoid loop
    ...
    def old_blog_post(
        ...
        return request.redirect(
            _build_url_w_params(
                "/blog/{}/{}".format(slug(blog), slug(blog_post)), request.params
            ),
            code=301,
        )
    ...
    def blog_post(
        ...
        if not blog_post.blog_id.id == blog.id:
            return request.redirect(
                "/blog/{}/{}".format(slug(blog_post.blog_id), slug(blog_post)), code=301
            )
    ...
    @http.route(
        "/blog/post_duplicate", type="http", auth="user", website=True, methods=["POST"]
    )
