...
class ReportBomStructure(models.AbstractModel):
    ...
    def _get_price(self, bom, factor, product):
        ...
        if bom and bom.type == "subcontract":
            ...
            if seller:
                price += (
                    seller.product_uom._compute_price(seller.price, product.uom_id)
                    * bom_quantity
                )
        return price
    ...
    def _get_sub_lines(
        ...
        res = super()._get_sub_lines(
            bom, product_id, line_qty, line_id, level, child_bom_ids, unfolded
        )
        if bom and bom.type == "subcontract":
            ...
            if seller:
                values_sub = self._get_subcontracting_line(
                    bom, seller, level, bom_quantity
                )
                values_sub["type"] = "bom"
