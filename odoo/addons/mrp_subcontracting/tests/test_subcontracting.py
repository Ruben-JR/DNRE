...
class TestSubcontractingFlows(TestMrpSubcontractingCommon):
    ...
    def test_flow_2(self):
        ...
        resupply_rule = resupply_sub_on_order_route.rule_ids.filtered(
            lambda l: l.location_id == self.comp1.property_stock_production
            and l.location_src_id == self.env.company.subcontracting_location_id
        )
        ...
        resupply_warehouse_rule = self.warehouse.route_ids.rule_ids.filtered(
            lambda l: l.location_id == self.env.company.subcontracting_location_id
            and l.location_src_id == self.warehouse.lot_stock_id
        )
    ...
    def test_flow_8(self):
        ...
        mo_done = backorder.move_lines.move_orig_ids.production_id.filtered(
            lambda p: p.state == "done"
        )
        backorder_mo = backorder.move_lines.move_orig_ids.production_id.filtered(
            lambda p: p.state != "done"
        )
        ...
        self.assertTrue(
            picking_receipt.move_lines.move_orig_ids.production_id.state == "done"
        )
...
class TestSubcontractingTracking(TransactionCase):
    ...
    def test_flow_tracked_only_finished(self):
        ...
        move_details = Form(
            ...
            view="stock.view_stock_move_nosuggest_operations",
        )
