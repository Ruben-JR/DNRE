...
class AccountEdiFormat(models.Model):
    ...
    def _bis3_get_extra_partner_domains(self, tree):
        if self.code == "ehf_3":
            ns = self._get_bis3_namespaces()
            bronnoysund = tree.xpath(
                './cac:AccountingSupplierParty/cac:Party/cbc:EndpointID[@schemeID="0192"]/text()',
                namespaces=ns,
            )
            if bronnoysund:
    ...
    def _export_ehf_3(self, invoice):
        self.ensure_one()
        # Create file content.
        xml_content = self.env.ref("l10n_no_edi.export_ehf_3_invoice")._render(
            self._get_ehf_3_values(invoice)
        )
        vat = invoice.company_id.partner_id.commercial_partner_id.vat
        xml_name = "ehf-{}{}{}.xml".format(
            vat or "",
            "-" if vat else "",
    ...
    def _check_move_configuration(self, invoice):
        ...
        if supplier.country_code == "NO" and not supplier.l10n_no_bronnoysund_number:
            errors.append(
                _(
                    "The supplier %r must have a Bronnoysund company registry.",
        ...
        if supplier.country_code != "NO" and supplier.country_code not in COUNTRY_EAS:
            errors.append(
                _(
                    "The supplier %r is from a country that is not supported for EHF (Bis3)",
                    supplier.display_name,
                )
            )
        ...
        if customer.country_code == "NO" and not customer.l10n_no_bronnoysund_number:
            errors.append(
                _(
                    "The customer %r must have a Bronnoysund company registry.",
        ...
        if customer.country_code != "NO" and customer.country_code not in COUNTRY_EAS:
            errors.append(
                _(
                    "The customer %r is from a country that is not supported for EHF (Bis3)",
                    customer.display_name,
                )
            )
