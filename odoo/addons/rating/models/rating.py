...
class Rating(models.Model):
    ...
    rated_partner_id = fields.Many2one(
        "res.partner", string="Rated Operator", help="Owner of the rated resource"
    )
    rated_partner_name = fields.Char(related="rated_partner_id.name")
    partner_id = fields.Many2one(
        "res.partner", string="Customer", help="Author of the rating"
    )
    rating = fields.Float(
        string="Rating Value",
        group_operator="avg",
    ...
    consumed = fields.Boolean(
        string="Filled Rating", help="Enabled if the rating has been filled."
    )
    ...
    def _compute_resource_ref(self):
        for rating in self:
            if rating.res_model and rating.res_model in self.env:
                rating.resource_ref = "{},{}".format(
                    rating.res_model, rating.res_id or 0
                )
            else:
