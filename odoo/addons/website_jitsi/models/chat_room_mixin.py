...
class ChatRoomMixin(models.AbstractModel):
    ...
    room_max_capacity = fields.Selection(
        string="Max capacity",
        related="chat_room_id.max_capacity",
        readonly=False,
        required=True,
    )
    ...
    room_last_activity = fields.Datetime(
        "Last activity", related="chat_room_id.last_activity"
    )
    ...
    def create(self, values_list):
        for values in values_list:
            if any(
                values.get(fmatch[0]) for fmatch in self.ROOM_CONFIG_FIELDS
    ...
    def copy_data(self, default=None):
        ...
        if self.chat_room_id:
            ...
            if "room_name" not in default:
                chat_room_default["name"] = self._jitsi_sanitize_name(
                    self.chat_room_id.name
                )
            default["chat_room_id"] = self.chat_room_id.copy(
                default=chat_room_default
            ).id
