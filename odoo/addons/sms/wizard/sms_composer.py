...
class SendSMS(models.TransientModel):
    ...
    body = fields.Text(
        "Message", compute="_compute_body", readonly=False, store=True, required=True
    )
    ...
    def _compute_composition_mode(self):
        for composer in self:
            if (
                ...
                if composer.res_ids_count > 1 or (
                    composer.use_active_domain and composer.active_domain_count > 1
                ):
    ...
    def _compute_recipients_count(self):
        for composer in self:
            ...
            if composer.res_model:
                composer.active_domain_count = self.env[
                    composer.res_model
                ].search_count(literal_eval(composer.active_domain or "[]"))
            else:
            ...
            if records and issubclass(type(records), self.pool["mail.thread"]):
                ...
                composer.recipient_invalid_count = (
                    ...
                    if (
                        ...
                        or (
                            composer.composition_mode == "mass"
                            and composer.use_active_domain
                        )
                    )
                    else 1
                )
    ...
    def create(self, values):
        ...
        if not values.get("body") or not values.get("composition_mode"):
            ...
            values["composition_mode"] = (
                values.get("composition_mode") or cache_composer.composition_mode
            )
