...
class StockPickingBatch(models.Model):
    ...
    def _compute_scheduled_date(self):
        self.scheduled_date = min(
            self.picking_ids.filtered("scheduled_date").mapped("scheduled_date"),
            default=False,
        )
    ...
    def create(self, vals):
        if vals.get("name", "/") == "/":
            if vals.get("is_wave"):
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("picking.wave") or "/"
                )
            else:
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("picking.batch") or "/"
                )
