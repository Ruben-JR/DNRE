...
class TestBatchPicking(TransactionCase):
    def setUp(self):
        ...
        self.picking_type_out = self.env["ir.model.data"]._xmlid_to_res_id(
        ...
        self.batch = self.env["stock.picking.batch"].create(
            {
                ...
                "company_id": self.env.company.id,
                "picking_ids": [
                    (4, self.picking_client_1.id),
                    (4, self.picking_client_2.id),
                ],
            }
        )
    ...
    def test_batch_with_backorder_wizard(self):
        ...
        self.assertEqual(
            self.picking_client_1.move_lines.product_uom_qty,
            5,
