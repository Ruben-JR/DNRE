...
class User(models.Model):
    ...
    google_calendar_rtoken = fields.Char(
        related="google_cal_account_id.calendar_rtoken", groups="base.group_system"
    )
    ...
    google_synchronization_stopped = fields.Boolean(
        related="google_cal_account_id.synchronization_stopped", readonly=False
    )

    _sql_constraints = [
        (
            "google_token_uniq",
            "unique (google_cal_account_id)",
    ...
    def _get_google_calendar_token(self):
        self.ensure_one()
        if (
            self.google_cal_account_id.calendar_rtoken
            and not self.google_cal_account_id._is_google_calendar_valid()
        ):
        ...
        with google_calendar_token(self) as token:
            try:
                (
                    ...
                    self.google_cal_account_id.calendar_sync_token, token=token
                )
