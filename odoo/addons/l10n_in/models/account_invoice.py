...
class AccountMove(models.Model):
    ...
    l10n_in_shipping_port_code_id = fields.Many2one(
        "l10n_in.port.code", "Port code", states={"draft": [("readonly", False)]}
    )
    ...
    def _l10n_in_get_indian_state(self, partner):
        ...
        if (
            ...
            raise ValidationError(
                _(
                    "State is missing from address in '%s'. First set state after post this invoice again.",
                    partner.name,
                )
            )
    ...
    def _get_tax_grouping_key_from_tax_line(self, tax_line):
        ...
        if (
            tax_line.move_id.journal_id.company_id.account_fiscal_country_id.code
            == "IN"
        ):
    ...
    def _get_tax_grouping_key_from_base_line(self, base_line, tax_vals):
        ...
        if (
            base_line.move_id.journal_id.company_id.account_fiscal_country_id.code
            == "IN"
        ):
    ...
    def _post(self, soft=True):
        ...
        for move in posted.filtered(lambda m: m.country_code == "IN"):
            ...
            move.l10n_in_gstin = (
                move._l10n_in_get_shipping_partner_gstin(shipping_partner)
            ...
            if move.journal_id.type == "sale":
                ...
                if not move.l10n_in_state_id:
                    move.l10n_in_state_id = self._l10n_in_get_indian_state(
                        move.partner_id
                    )
