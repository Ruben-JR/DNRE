...
class WebsiteBackend(http.Controller):
    ...
    def fetch_dashboard_data(self, website_id, date_from, date_to):
        ...
        if has_group_designer:
            if (
                current_website.google_management_client_id
                and current_website.google_analytics_key
            ):
        ...
        if not request.env.user.has_group("base.group_system"):
            return {
                "error": {
                    ...
                    "message": _(
                        "You do not have sufficient rights to perform that action."
                    ),
                }
            }
        if not ga_analytics_key or not ga_client_id.endswith(
            ...
            return {
                "error": {
                    ...
                    "message": _(
                        "The Google Analytics Client ID or Key you entered seems incorrect."
                    ),
                }
            }
        Website = request.env["website"]
