...
class Website(Home):
        ...
        if not isredir and website.domain:
            ...
            if domain_from != domain_to:
                ...
                url_to = werkzeug.urls.url_join(
                    ...
                    f"/website/force/{website.id}?isredir=1&path={path}",
                )
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        if request.env.lang != website_id.default_lang_id.code:
            return request.redirect(
                "/%s%s"
                % (website_id.default_lang_id.url_code, request.httprequest.path)
            )
    ...
    def get_suggested_link(self, needle, limit=10):
        ...
        for name, url, mod in current_website.get_suggested_controllers():
            if needle.lower() in name.lower() or needle.lower() in url.lower():
                ...
                icon = (
                    mod
                    and "<img src='%s' width='24px' class='mr-2 rounded' /> "
            ...
            for mapped_name, field_meta in mapping.items():
                ...
                if field_type == "dict":
                    ...
                    for key, item in value.items():
                        mapped[mapped_name][key] = get_mapping_value(
                            item_type, item, field_meta
                        )
                else:
                    mapped[mapped_name] = get_mapping_value(
                        field_type, value, field_meta
                    )
    ...
    @http.route(
        ["/website/pages", "/website/pages/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def pages_management(self, page=1, sortby="url", search="", **kw):
        ...
        sort_order = (
            searchbar_sortings.get(sortby, "url")["order"] + ", website_id desc, id"
        )
    ...
    def pagenew(
        self, path="", noredirect=False, add_menu=False, template=False, **kwargs
    ):
    ...
    @http.route(
        "/website/toggle_switchable_view", type="json", auth="user", website=True
    )
    ...
    # compatibility.
    @http.route(
        ["/website/theme_customize_get"], type="json", auth="user", website=True
    )
    ...
    def theme_customize_data(
        ...
        """
        disabled_data = self._get_customize_data(disable, is_view_data).filtered(
            "active"
        )
        if reset_view_arch:
            disabled_data.reset_arch(mode="hard")
        disabled_data.write({"active": False})
        self._get_customize_data(enable, is_view_data).filtered(
    ...
    def theme_customize_bundle_reload(self):
        ...
        """
        return {
            "web.assets_common": request.env["ir.qweb"]._get_asset_link_urls(
                "web.assets_common"
            ),
            "web.assets_frontend": request.env["ir.qweb"]._get_asset_link_urls(
                "web.assets_frontend"
            ),
            "website.assets_editor": request.env["ir.qweb"]._get_asset_link_urls(
