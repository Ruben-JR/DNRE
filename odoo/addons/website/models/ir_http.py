...
class Http(models.AbstractModel):
    ...
    def _match(cls, path_info, key=None):
        key = key or (request and request.website_routing)
        return super()._match(path_info, key=key)
    ...
    def _generate_routing_rules(cls, modules, converters):
        ...
        for url, endpoint, routing in super()._generate_routing_rules(
            modules, converters
        ):
            ...
            if url in rewrites:
                ...
                if rewrite.redirect_type == "308":
                    logger.debug(f"Add rule {url_to} for {website_id}")
                    ...
                    logger.debug(f"Return 404 for {url} for website {website_id}")
                    continue
            else:
    ...
    def _register_website_track(cls, response):
        ...
        if view and view.track:
            request.env["website.visitor"]._handle_webpage_dispatch(
                response, website_page
            )
            ...
            if isinstance(record, models.BaseModel) and hasattr(
                ...
                try:
                    if not record.can_access_from_current_website():
                        return request.env["ir.http"]._handle_exception(
    ...
    def _add_dispatch_parameters(cls, func):
        ...
        if (
            force_website_id
            and request.session.get("force_website_id") != force_website_id
    ...
    def _get_default_lang(cls):
        if getattr(request, "website", False):
            return request.env["res.lang"].browse(
                request.website._get_cached("default_lang_id")
            )
    ...
    def _serve_page(cls):
        ...
        page = (
            ...
            .sudo()
            .search(published_domain, order="website_id asc", limit=1)
        )
                ...
                try:
                    ...
                    if r["time"] + page.cache_time > time.time():
                        response = odoo.http.Response(
                            r["content"], mimetype=r["contenttype"]
                        )
            ...
            if need_to_cache and response.status_code == 200:
                ...
                page._set_cache_response(
                    cache_key,
                    {
                        ...
                        "time": time.time(),
                        "template": getattr(response, "qcontext", {}).get(
                            "response_template"
                        ),
                    },
                )
    ...
    def _get_exception_code_values(cls, exception):
        ...
        if (
            ...
            and request.website.is_publisher()
        ):
            code = "page_404"
    ...
    def binary_content(
        ...
        if obj and "website_published" in obj._fields:
            if (
                self.env[obj._name]
                .sudo()
                .search([("id", "=", obj.id), ("website_published", "=", True)])
            ):
                self = self.sudo()
        ...
        session_info.update(
            {
                ...
                "geoip_country_code": request.session.get("geoip", {}).get(
                    "country_code"
                ),
            }
        )
