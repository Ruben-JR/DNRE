...
class Page(models.Model):
    ...
    is_homepage = fields.Boolean(
        compute="_compute_homepage", inverse="_set_homepage", string="Homepage"
    )
    is_visible = fields.Boolean(compute="_compute_visible", string="Is Visible")

    cache_time = fields.Integer(
        default=3600, help="Time to cache the page. (0 = no cache)"
    )
    cache_key_expr = fields.Char(
        help='Expression (tuple) to evaluate the cached key. \nE.g.: "(request.params.get("currency"), )"'
    )
    ...
    def _compute_homepage(self):
        for page in self:
            page.is_homepage = (
                page == self.env["website"].get_current_website().homepage_id
            )
    ...
    def get_page_properties(self):
        ...
        if not res["groups_id"]:
            ...
            res["group_id"] = (
                self.env["res.groups"].browse(res["groups_id"]).name_get()[0]
            )
        del res["groups_id"]

        res["visibility_password"] = (
            res["visibility"] == "password" and self.visibility_password_display or ""
        )
        return res
    ...
    def save_page_info(self, website_id, data):
        ...
        if (
            page.visibility == "restricted_group"
            and data["visibility"] != "restricted_group"
        ):
    ...
    def _get_cache_response(self, cache_key):
        ...
        return self.pool._Registry__cache[
            ("website.page", _cached_response, self.id, cache_key)
        ]

    def _set_cache_response(self, cache_key, response):
        """Put in cache the given response."""
        self.pool._Registry__cache[
            ("website.page", _cached_response, self.id, cache_key)
        ] = response
    ...
    def _search_fetch(self, search_detail, search, limit, order):
        ...
        def filter_page(search, page, all_pages):
            ...
            text = "{} {} {}".format(page.name, page.url, text_from_html(page.arch))
            pattern = "|".join(
