...
class WebsiteVisitor(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Contact", help="Partner of the last logged in user."
    )
    ...
    def _handle_webpage_dispatch(self, response, website_page):
        ...
        if (
            ...
            response.set_cookie(
                "visitor_uuid", visitor_sudo.access_token, expires=expiration_date
            )
    ...
    def _cron_archive_visitors(self):
        delay_days = int(
            ...
            .sudo()
            .get_param("website.visitor.live.days", 30)
        )
        deadline = datetime.now() - timedelta(days=delay_days)
        visitors_to_archive = (
            self.env["website.visitor"]
            .sudo()
            .search([("last_connection_datetime", "<", deadline)])
        )
