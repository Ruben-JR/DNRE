...
class Website(models.Model):
    ...
    def _get_menu_page_ids(self):
        return (
            self.env["website.menu"].search([("website_id", "=", self.id)]).page_id.ids
        )
    ...
    def _handle_favicon(self, vals):
        if "favicon" in vals:
            vals["favicon"] = tools.image_process(
                vals["favicon"], size=(256, 256), crop="center", output_format="ICO"
            )
    ...
    def get_theme_snippet_lists(self, theme_name):
        default_snippet_lists = http.addons_manifest["theme_default"].get(
            "snippet_lists", {}
        )
    ...
    def configurator_init(self):
        ...
        configurator_features = (
            self.env["website.configurator.feature"]
            .with_context(lang=self.get_current_website().default_lang_id.code)
            .search([])
        )
        r["features"] = [
            {
        ...
        try:
            result = self._website_api_rpc(
                ...
                {"lang": self.get_current_website().default_lang_id.code},
            )
    ...
    def configurator_recommended_themes(self, industry_id, palette):
        domain = [
            ("name", "=like", "theme%"),
            ("name", "not in", ["theme_default", "theme_common"]),
            ("state", "!=", "uninstallable"),
        ]
    ...
    def page_search_key_dependencies(self, page_id=False):
        ...
        for view in views:
            ...
            dependencies[view_key].append(
                {
                    "text": _("Template <b>%s (id:%s)</b> is calling this file")
                    % (view.key or view.name, view.id),
                    "item": _("%s (id:%s)") % (view.key or view.name, view.id),
                    "link": "/web#id=%s&view_type=form&model=ir.ui.view" % view.id,
                }
    ...
    def get_current_website(self, fallback=True):
        ...
        country = (
            ...
            if request and request.session.geoip
            else False
        )
        country_id = False
        if country:
            country_id = (
                self.env["res.country"].search([("code", "=", country)], limit=1).id
            )

        website_id = self._get_current_website_id(
            domain_name, country_id, fallback=fallback
        )
        return self.browse(website_id)
    ...
    def _get_current_website_id(self, domain_name, country_id, fallback=True):
        ...
        if not websites:
            ...
            country_specific_websites = websites.filtered(
                lambda website: country_id
                in website.country_group_ids.mapped("country_ids").ids
            )
            return (
                country_specific_websites[0].id
                if country_specific_websites
                else websites[0].id
            )
            ...
            for (i, (name, converter)) in enumerate(convitems):
                ...
                for val in values:
                    ...
                    for rec in converter.generate(
                        uid=self.env.uid, dom=query, args=val
                    ):
    ...
    def image_url(self, record, field, size=None):
        ...
        return "/web/image/{}/{}/{}{}?unique={}".format(
            record._name,
            record.id,
            field,
            size,
    ...
    def _get_canonical_url_localized(self, lang, canonical_params):
        ...
        canonical_query_string = (
            "?%s" % urls.url_encode(canonical_params) if canonical_params else ""
        )
    ...
    def _is_canonical_url(self, canonical_params):
        ...
        current_url = request.httprequest.url_root[:-1] + (
            hasattr(request, "rerouting")
            and request.rerouting[0]
        ...
        for search_detail in search_details:
                        ...
                        sql.SQL("{table}.{field}").format(
                            table=sql.Identifier(
                                (
                                    self.env["ir.ui.view"]
                                    if field == "arch_db"
                                    or (field == "name" and "arch_db" in fields)
                                    else model
                                )._table
                            ),
            ...
            if self.env.lang:
                ...
                if "arch_db" in fields:
                    ...
                    names = [
                        "{},{}".format(self.env["ir.ui.view"]._name, field)
                        for field in fields
                    ]
                    query = sql.SQL(
                        """
                    ...
                    names = [f"{model._name},{field}" for field in fields]
                    query = sql.SQL(
                        """
                ...
                self.env.cr.execute(
                    query, {"lang": self.env.lang, "names": names, "search": search}
                )
                ids.update(
                    row[0]
                    for row in self.env.cr.fetchall()
