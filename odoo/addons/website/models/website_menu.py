...
class Menu(models.Model):
    ...
    def write(self, values):
        ...
        if (
            "website_id" in values
            or "group_ids" in values
            or "sequence" in values
    ...
    def _compute_visible(self):
        for menu in self:
            ...
            if (
                menu.page_id
                and not menu.user_has_groups("base.group_user")
                and (
                    not menu.page_id.sudo().is_visible
                    or (
                        not menu.page_id.view_id._handle_visibility(do_raise=False)
                        and menu.page_id.view_id.visibility != "password"
                    )
                )
            ):
                visible = False
