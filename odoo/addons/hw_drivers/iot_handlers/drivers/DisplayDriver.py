...
class DisplayDriver(Driver):
    ...
    def run(self):
        while self.device_identifier != "distant_display" and not self._stopped.isSet():
            ...
            if (
                self.url
                != "http://localhost:8069/point_of_sale/display/"
...
class DisplayController(http.Controller):
    ...
    def customer_facing_display(self, html=None):
        display = DisplayDriver.get_default_display()
        if display:
            display.update_customer_facing_display(
                http.request.httprequest.remote_addr, html
            )
    ...
    @http.route(
        [
            ...
            "/point_of_sale/display/<string:display_identifier>",
        ],
        type="http",
        auth="none",
    )
    def display(self, display_identifier=None):
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "../../static/src/js/worker.js")
        ) as js:
            cust_js = js.read()
