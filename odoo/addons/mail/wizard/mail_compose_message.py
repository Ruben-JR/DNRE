...
class MailComposer(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if missing_author or missing_email_from:
            author_id, email_from = self.env["mail.thread"]._message_compute_author(
                result.get("author_id"), result.get("email_from"), raise_exception=False
            )
        ...
        if result.get("composition_mode") == "comment" and (
            set(fields) & {"model", "res_id", "partner_ids", "record_name", "subject"}
        ):
        ...
        filtered_result = {fname: result[fname] for fname in result if fname in fields}
    ...
    template_id = fields.Many2one(
        "mail.template", "Use template", index=True, domain="[('model', '=', model)]"
    )
    ...
    def action_save_as_template(self):
        ...
        for record in self:
            ...
            template_name = "{}: {}".format(model_name, tools.ustr(record.subject))
            values = {
                "name": template_name,
    ...
    def render_message(self, res_ids):
        ...
        bodies = self.with_context(
            preserve_comments=self.composition_mode == "mass_mail"
        )._render_field("body", res_ids, post_process=True)
