...
class MockEmail(common.BaseCase):
    ...
    def mock_mail_gateway(self, mail_unlink_sent=False, sim_error=None):
        ...
        with patch.object(
            ...
            autospec=True,
            wraps=IrMailServer,
            side_effect=_ir_mail_server_connect,
        ) as ir_mail_server_connect_mock, patch.object(
            IrMailServer,
            "build_email",
            autospec=True,
            wraps=IrMailServer,
            side_effect=_ir_mail_server_build_email,
        ) as ir_mail_server_build_email_mock, patch.object(
            IrMailServer,
            "send_email",
            autospec=True,
            wraps=IrMailServer,
            side_effect=_ir_mail_server_send_email,
        ) as ir_mail_server_send_email_mock, patch.object(
            MailMail,
            "create",
            autospec=True,
            wraps=MailMail,
            side_effect=_mail_mail_create,
        ) as _mail_mail_create_mock, patch.object(
            MailMail,
            "unlink",
            autospec=True,
            wraps=MailMail,
            side_effect=_mail_mail_unlink,
        ) as mail_mail_unlink_mock:
            yield
    ...
    def assertMailMail(
        ...
        for fname, fvalue in (fields_values or {}).items():
            self.assertEqual(
                found_mail[fname],
                fvalue,
                "Mail: expected {} for {}, got {}".format(
                    fvalue, fname, found_mail[fname]
                ),
            )
        if status == "sent":
    ...
    def assertMailMailWId(self, mail_id, status, content=None, fields_values=None):
        """Assert mail.mail records are created and maybe sent as emails. Allow
        asserting their content. Records to check are the one generated when
        using mock (mail.mail and outgoing emails). This method takes partners
        as source of record fetch and assert.
    ...
    def assertSentEmail(self, author, recipients, **values):
        ...
        debug_info = (
            "-".join(
                "From: {}-To: {}".format(mail["email_from"], mail["email_to"])
                for mail in self._mails
            )
            if not bool(sent_mail)
            else ""
        )
        self.assertTrue(
            bool(sent_mail),
...
class MailCase(MockEmail):
    ...
    def _reset_mail_context(cls, record):
        return record.with_context(
            mail_create_nolog=False, mail_create_nosubscribe=False, mail_notrack=False
        )
    ...
    def assertSinglePostNotifications(
        self, recipients_info, message_info=None, mail_unlink_sent=False, sim_error=None
    ):
        ...
        with self.assertPostNotifications(
            [r_info], mail_unlink_sent=mail_unlink_sent, sim_error=sim_error
        ):
            yield

    @contextmanager
    def assertPostNotifications(
        self, recipients_info, mail_unlink_sent=False, sim_error=None
    ):
        """Check content of notifications."""
        try:
            with self.mock_mail_gateway(
                mail_unlink_sent=mail_unlink_sent, sim_error=sim_error
    ...
    def assertNoNotifications(self):
        try:
            with self.mock_mail_gateway(
                mail_unlink_sent=False, sim_error=None
            ), self.mock_bus(), self.mock_mail_app():
                yield
        finally:
