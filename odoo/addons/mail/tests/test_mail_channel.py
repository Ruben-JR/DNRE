...
class TestChannelAccessRights(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.group_public = Channel.create(
            {"name": "Jobs", "description": "NotFalse", "public": "public"}
        )
    ...
    def test_access_public(self):
        ...
        with self.assertRaises(AccessError):
            self.env["mail.channel"].browse(self.group_public.id).write(
                {"name": "Broutouschnouk"}
            )
    ...
    def test_access_employee(self):
        ...
        with self.assertRaises(AccessError):
            self.env["mail.channel"].browse(self.group_private.id).write(
                {"name": "re-modified"}
            )
...
class TestChannelInternals(MailCommon):
    ...
    def test_channel_form(self):
        ...
        self.assertEqual(
            ...
            "Must be able to read the created channel",
        )
    ...
    def test_channel_user_synchronize(self):
        ...
        test_channel_private.add_members(
            (self.partner_employee | self.partner_employee_nomail).ids
        )
        test_channel_group.add_members(
            (self.partner_employee | self.partner_employee_nomail).ids
        )
        ...
        self.assertEqual(
            test_channel_private.channel_partner_ids, self.partner_employee_nomail
        )
        self.assertEqual(
            test_channel_group.channel_partner_ids, self.partner_employee_nomail
        )
        self.assertEqual(
        ...
        self.assertEqual(
            test_channel_private.channel_partner_ids, self.env["res.partner"]
        )
        self.assertEqual(
            test_channel_group.channel_partner_ids, self.env["res.partner"]
        )
        self.assertEqual(
            self.test_channel.channel_partner_ids,
            self.user_employee.partner_id | self.partner_employee_nomail,
    ...
    def test_channel_info_get(self):
        ...
        self.assertEqual(
            {p["id"] for p in initial_channel_info["members"]},
        ...
        self.assertEqual(
            {p["id"] for p in solo_channel_info["members"]},
    ...
    def test_channel_unsubscribe_auto(self):
        ...
        self.assertEqual(
            test_chat.channel_partner_ids, self.user_employee.partner_id | test_partner
        )
    ...
    def test_channel_should_generate_correct_default_avatar(self):
        ...
        expceted_avatar_channel = (
            channel_avatar.replace('fill="#875a7b"', f'fill="{bgcolor}"')
        ).encode()
        expected_avatar_group = (
            group_avatar.replace('fill="#875a7b"', f'fill="{bgcolor}"')
        ).encode()
