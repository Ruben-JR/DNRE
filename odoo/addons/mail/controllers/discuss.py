...
class DiscussController(http.Controller):
    ...
    def _response_discuss_channel_invitation(
        ...
        channel_partner_sudo = channel_sudo.env[
            "mail.channel.partner"
    ...
    def mail_channel_partner_avatar_128(self, channel_id, partner_id, **kwargs):
        ...
        if not channel_partner_sudo or not channel_partner_sudo.env[
            "mail.channel.partner"
        ].search(
            ...
            if request.env.user.share:
                placeholder = (
                    channel_partner_sudo.env["res.partner"]
                    .browse(partner_id)
                    .exists()
                    ._avatar_get_placeholder()
                )
                return channel_partner_sudo.env[
                    "ir.http"
                ]._placeholder_image_get_response(placeholder)
            return (
                channel_partner_sudo.sudo(False)
                .env["ir.http"]
                ._content_image(
                    model="res.partner", res_id=partner_id, field="avatar_128"
                )
            )
        return channel_partner_sudo.env["ir.http"]._content_image(
            model="res.partner", res_id=partner_id, field="avatar_128"
        )
    ...
    def mail_channel_guest_avatar_128(self, channel_id, guest_id, **kwargs):
        ...
        if not channel_partner_sudo or not channel_partner_sudo.env[
            ...
            if request.env.user.share:
                placeholder = (
                    channel_partner_sudo.env["mail.guest"]
                    .browse(guest_id)
                    .exists()
                    ._avatar_get_placeholder()
                )
            ...
            return (
                channel_partner_sudo.sudo(False)
                .env["ir.http"]
                ._content_image(model="mail.guest", res_id=guest_id, field="avatar_128")
            )
        return channel_partner_sudo.env["ir.http"]._content_image(
            model="mail.guest", res_id=guest_id, field="avatar_128"
        )
    ...
    def mail_channel_attachment(
        ...
        if not channel_partner_sudo.env["ir.attachment"].search(
            [
            ...
            raise NotFound()
        return channel_partner_sudo.env["ir.http"]._get_content_common(
            res_id=int(attachment_id), download=download
        )
    ...
    def fetch_image(self, channel_id, attachment_id, width=0, height=0, **kwargs):
        ...
        if not channel_partner_sudo.env["ir.attachment"].search(
            ...
            raise NotFound()
        return channel_partner_sudo.env["ir.http"]._content_image(
            res_id=int(attachment_id), height=int(height), width=int(width)
        )
    ...
    def mail_message_update_content(self, message_id, body, attachment_ids):
        ...
        if (
            ...
            and not guest.env.user.has_group("base.group_system")
        ):
            raise NotFound()
        ...
        return {
            ...
            "attachments": [
                (
                    "insert-and-replace",
                    message_sudo.attachment_ids._attachment_format(commands=True),
                )
            ],
        }
    ...
    def mail_channel_add_guest_as_member(self, channel_id, channel_uuid, **kwargs):
        ...
        if not guest:
            raise NotFound()
        channel_partner = channel_sudo.env[
            "mail.channel.partner"
        ]._get_as_sudo_from_request(request=request, channel_id=channel_id)
    ...
    def mail_channel_messages(
        ...
        return channel_partner_sudo.env["mail.message"]._message_fetch(
            domain=[
                ("res_id", "=", channel_id),
                ("model", "=", "mail.channel"),
    ...
    def channel_ping(self, channel_id, rtc_session_id=None, check_rtc_session_ids=None):
        ...
        ) = channel_partner_sudo._rtc_sync_sessions(
        ...
        return {
            "rtcSessions": [
                (
                    "insert",
                    [
                        rtc_session_sudo._mail_rtc_session_format(complete_info=False)
    ...
    def mail_thread_data(self, thread_model, thread_id, request_list, **kwargs):
        ...
        if "attachments" in request_list:
            res["attachments"] = (
                thread.env["ir.attachment"]
                .search(
                    [("res_id", "=", thread.id), ("res_model", "=", thread._name)],
                    order="id desc",
                )
    ...
    def read_followers(self, res_model, res_id):
        ...
        follower_recs = request.env["mail.followers"].search(
        ...
        return {
            "followers": followers,
            "subtypes": self.read_subscription_data(follower_id)
            if follower_id
            else None,
        }
    ...
    @http.route(
        "/mail/get_suggested_recipients", methods=["POST"], type="json", auth="user"
    )
    ...
    def channel_call_cancel_invitation(
        self, channel_id, partner_ids=None, guest_ids=None
    ):
        ...
        return channel_partner_sudo.channel_id._rtc_cancel_invitations(
