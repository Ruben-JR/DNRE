...
class ChannelPartner(models.Model):
    ...
    rtc_session_ids = fields.One2many(
        ...
        inverse_name="channel_partner_id",
    )
    ...
    def init(self):
        self.env.cr.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS mail_channel_partner_partner_unique ON %s (channel_id, partner_id) WHERE partner_id IS NOT NULL"
            % self._table
        )
        self.env.cr.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS mail_channel_partner_guest_unique ON %s (channel_id, guest_id) WHERE guest_id IS NOT NULL"
            % self._table
        )
    ...
    def write(self, vals):
        for channel_partner in self:
            for field_name in {"channel_id", "partner_id", "guest_id"}:
                if (
                    field_name in vals
                    and vals[field_name] != channel_partner[field_name].id
                ):
                    raise AccessError(
    ...
    def _get_as_sudo_from_request_or_raise(self, request, channel_id):
        channel_partner = self._get_as_sudo_from_request(
    ...
    def _get_as_sudo_from_request(self, request, channel_id):
        ...
        if guest:
            return (
                guest.env["mail.channel.partner"]
                .sudo()
                .search(
    ...
    def _rtc_join_call(self, check_rtc_session_ids=None):
        ...
        self.channel_id._rtc_cancel_invitations(
            partner_ids=self.partner_id.ids, guest_ids=self.guest_id.ids
        )
    ...
    def _rtc_leave_call(self):
        ...
        if self.rtc_session_ids:
            ...
            return self.channel_id._rtc_cancel_invitations(
                partner_ids=self.partner_id.ids, guest_ids=self.guest_id.ids
            )
            ...
            invitation_notifications.append(
                (
                    ...
                    "mail.channel/insert",
                    {
                        ...
                        "rtcInvitingSession": [
                            ("insert", self.rtc_session_ids._mail_rtc_session_format())
                        ],
                    },
                )
            )
