...
class Users(models.Model):
    ...
    def _unsubscribe_from_non_public_channels(self):
        ...
        current_cp.filtered(
            lambda cp: cp.channel_id.public != "public"
            and cp.channel_id.channel_type == "channel"
        ).unlink()
    ...
    def _init_messaging(self):
        ...
        values = {
            ...
            "public_partners": list(
                self.env.ref("base.group_public")
                .sudo()
                .with_context(active_test=False)
                .users.partner_id.mail_partner_format()
                .values()
            ),
            "shortcodes": self.env["mail.shortcode"]
            .sudo()
            .search_read([], ["source", "substitution", "description"]),
