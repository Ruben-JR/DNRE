...
atext = r"[a-zA-Z0-9!#$%&'*+\-/=?^_`{|}~]"
dot_atom_text = re.compile(rf"^{atext}+(\.{atext}+)*$")
...
class Alias(models.Model):
    ...
    def name_get(self):
        ...
        for record in self:
            if record.alias_name and record.alias_domain:
                res.append((record["id"], f"{record.alias_name}@{record.alias_domain}"))
    ...
    def _clean_and_check_unique(self, names):
        ...
        matching_alias_name = (
            f"{sanitized_alias_name}@{alias_domain}"
            if alias_domain
...
Kind Regards,</p>"""
    ...
    def _get_alias_bounced_body(self, message_dict):
        ...
        template = self.env.ref(
            "mail.mail_bounce_alias_security", raise_if_not_found=True
        )
