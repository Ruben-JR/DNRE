...
class BaseModel(models.AbstractModel):
    ...
    def _notify_get_reply_to(
        ...
        if alias_domain:
            if model and res_ids:
                ...
                for alias in mail_aliases:
                    result_email.setdefault(
                        ...
                        f"{alias.alias_name}@{alias_domain}",
                    )
            ...
            if left_ids:
                ...
                if catchall:
                    result_email.update(
                        {rid: f"{catchall}@{alias_domain}" for rid in left_ids}
                    )
    ...
    def _notify_email_header_dict(self):
        return {
            "X-Odoo-Objects": f"{self._name}-{self.id}",
        }
