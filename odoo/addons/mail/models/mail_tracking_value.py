...
class MailTracking(models.Model):
    ...
    field = fields.Many2one(
        "ir.model.fields", required=True, readonly=1, ondelete="cascade"
    )
            ...
            values.update(
                {
                    ...
                    and dict(col_info["selection"])[initial_value]
                    or "",
    ...
    def get_display_value(self, type):
        ...
        for record in self:
            if record.field_type in ["integer", "float", "char", "text", "monetary"]:
                result.append(getattr(record, f"{type}_value_{record.field_type}"))
