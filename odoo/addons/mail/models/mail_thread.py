...
class MailThread(models.AbstractModel):
    ...
    def get_empty_list_help(self, help):
        ...
        if alias:
            email_link = "<a href='mailto:{email}'>{email}</a>".format(
                email=alias.display_name
            )
            if nothing_here:
                return "<p class='o_view_nocontent_smiling_face'>{dyn_help}</p>".format(
                    dyn_help=_(
        ...
        for record in self:
            ...
            if subtype:
                if not subtype.exists():
                    ...
                    continue
                record.message_post(
                    subtype_id=subtype.id, tracking_value_ids=tracking_value_ids
                )
    ...
    def _routing_warn(self, error_message, message_id, route, raise_exception=True):
        ...
        full_message = "Routing mail with Message-Id {}: route {}: {}".format(
            message_id,
            route,
            error_message,
        )
        ...
        if email_part:
            if email_part.get_content_type() == "text/rfc822-headers":
                ...
                email_payload = message_from_string(
                    email_part.get_payload(), policy=policy.SMTP
                )
            else:
    ...
    def message_post(
        ...
        **kwargs,
        ...
        self = (
            self._fallback_lang()
        )  # add lang to context imediatly since it will be usefull in various flows latter.
    ...
    def _message_compose_with_view(self, views_or_xmlid, message_log=False, **kwargs):
        ...
        for record in self:
            ...
            rendered_template = views._render(
        ...
        for pid, active, pshare, notif, groups in res:
            ...
            if pid:
                ...
                if notif == "inbox":
                    ...
                    not pshare and notif
                ):  # has an user and is not shared, is therefore user
                    recipients_data.append(dict(pdata, notif=notif, type="user"))
    ...
    def _notify_encode_link(self, base_link, params):
        ...
        token = "{}?{}".format(
            base_link,
            " ".join("{}={}".format(key, params[key]) for key in sorted(params)),
        )
        hm = hmac.new(
    ...
    def _notify_get_action_link(self, link_type, **kwargs):
        ...
        link = "{}?{}".format(base_link, urls.url_encode(params))
        if self:
