...
class Channel(models.Model):
    ...
    def default_get(self, fields):
        ...
        if not res.get("alias_contact") and (not fields or "alias_contact" in fields):
            res["alias_contact"] = (
                "everyone" if res.get("public", "private") == "public" else "followers"
            )
        return res
    ...
    is_member = fields.Boolean(
        "Is Member", compute="_compute_is_member", compute_sudo=True
    )
    member_count = fields.Integer(
        string="Member Count",
        compute="_compute_member_count",
        compute_sudo=True,
        help="Excluding guests from count.",
    )
    ...
    def _constraint_partners_chat(self):
        for ch in self.sudo().filtered(lambda ch: ch.channel_type == "chat"):
            if (
                len(ch.channel_last_seen_partner_ids) > 2
                or len(ch.channel_partner_ids) > 2
            ):
    ...
    def _compute_member_count(self):
        read_group_res = self.env["mail.channel.partner"].read_group(
    ...
    def _subscribe_users_automatically_get_members(self):
        ...
        return {
            channel.id: (
                channel.group_ids.users.partner_id - channel.channel_partner_ids
            ).ids
            for channel in self
        }
    ...
    def add_members(self, partner_ids=None, guest_ids=None, invite_to_rtc_call=False):
        ...
        for channel in self:
            ...
            existing_guests = self.env["mail.guest"].search(
            ...
            new_members = (
                self.env["mail.channel.partner"].sudo().create(members_to_create)
            )
            members_data = []
            guest_members_data = []
            for channel_partner in new_members.filtered(
                lambda channel_partner: channel_partner.partner_id
            ):
                user = (
                    channel_partner.partner_id.user_ids[0]
                    if channel_partner.partner_id.user_ids
                ...
                if user:
                    self.env["bus.bus"]._sendone(
                        ...
                        "mail.channel/joined",
                        {
                            "channel": channel_partner.channel_id.with_user(user)
                ...
                channel_partner.channel_id.message_post(
                    body=notification,
            ...
            for channel_partner in new_members.filtered(
                lambda channel_partner: channel_partner.guest_id
            ):
                channel_partner.channel_id.message_post(
                ...
                if guest:
                    self.env["bus.bus"]._sendone(
                        ...
                        "mail.channel/joined",
                        {
                            "channel": channel_partner.channel_id.sudo().channel_info()[
                                0
                            ],
                        },
                    )
        ...
        if invite_to_rtc_call:
            ...
            for channel in self:
                ...
                if current_channel_partner and current_channel_partner.rtc_session_ids:
                    current_channel_partner._rtc_invite_members(
                        partner_ids=partners.ids, guest_ids=guests.ids
                    )
    ...
    def _action_remove_members(self, partners):
        ...
        self.env["mail.channel.partner"].sudo().search(
            [("partner_id", "in", partners.ids), ("channel_id", "in", self.ids)]
        ).unlink()
        self.invalidate_cache(
            fnames=["channel_partner_ids", "channel_last_seen_partner_ids"]
        )
    ...
    def _rtc_cancel_invitations(self, partner_ids=None, guest_ids=None):
        ...
        if invited_guests:
            channel_data["invitedGuests"] = [
                ("insert-and-unlink", [{"id": guest.id} for guest in invited_guests])
            ]
        if invited_partners:
            channel_data["invitedPartners"] = [
                (
                    "insert-and-unlink",
                    [{"id": partner.id} for partner in invited_partners],
                )
            ]
    ...
    def _alias_get_error_message(self, message, message_dict, alias):
        if alias.alias_contact == "followers" and self.ids:
            ...
            return False
        return super()._alias_get_error_message(message, message_dict, alias)
    ...
    def _message_compute_author(
        self, author_id=None, email_from=None, raise_exception=False
    ):
        return super()._message_compute_author(
            author_id=author_id, email_from=email_from, raise_exception=False
        )
    ...
    def message_post(self, *, message_type="notification", **kwargs):
        ...
        ).mapped("channel_last_seen_partner_ids").sudo().write(
            {
                "is_pinned": True,
        ...
        self.env["bus.bus"]._sendone(
            ...
            "mail.message/insert",
            {
                ...
                "attachments": [
                    (
                        "insert-and-replace",
                        message.attachment_ids._attachment_format(commands=True),
                    )
                ],
            },
        )
    ...
    def _channel_message_notifications(self, message, message_format=False):
        ...
        for channel in self:
            ...
            if channel.public == "public":
                notifications.append(
                    (channel.uuid, "mail.channel/new_message", payload)
                )
        return notifications
    ...
    def channel_info(self):
        ...
        all_needed_members = self.env["mail.channel.partner"].search(
            expression.AND(
        ...
        invited_members_by_channel = defaultdict(
            lambda: self.env["mail.channel.partner"]
        )
        ...
        for member in all_needed_members:
            ...
            if (
                ...
                and member.partner_id == self.env.user.partner_id
            ):
        ...
        for channel in self:
            ...
            if self.env.user and self.env.user.partner_id:
                ...
                partner_channel = member_of_current_user_by_channel.get(
                    channel, self.env["mail.channel.partner"]
                )
                if partner_channel:
                    ...
                    info[
                        "last_interest_dt"
                    ] = partner_channel.last_interest_dt.strftime(
                        DEFAULT_SERVER_DATETIME_FORMAT
                    )
                    if partner_channel.rtc_inviting_session_id:
                        info["rtc_inviting_session"] = {
                            "id": partner_channel.rtc_inviting_session_id.id
                        }
            # add members info
            if channel.channel_type != "channel":
                ...
                info["members"] = sorted(
                    list(
                        channel._channel_info_format_member(
                            member.partner_id,
                            partner_format_by_partner[member.partner_id],
                        )
                        for member in members_by_channel[channel]
                        if member.partner_id
                    ),
                    key=lambda p: p["id"],
                )
            ...
            info.update(
                {
                    "invitedGuests": [
                        (
                            "insert",
                            [
                                {"id": member.guest_id.id, "name": member.guest_id.name}
                                for member in invited_members_by_channel[channel]
                                if member.guest_id
                            ],
                        )
                    ],
    ...
    def _set_last_seen_message(self, last_message):
        ...
        channel_partner_domain = expression.AND(
            [channel_partner_domain, [("partner_id", "=", self.env.user.partner_id.id)]]
        )
    ...
    def notify_typing(self, is_typing):
        ...
        for channel in self:
            ...
            notifications.append(
                [channel, "mail.channel.partner/typing_status", data]
            )  # notify backend users
            notifications.append(
                [channel.uuid, "mail.channel.partner/typing_status", data]
    ...
    def get_mention_suggestions(self, search, limit=8):
        ...
        return self.search_read(
            domain, ["id", "name", "public", "channel_type"], limit=limit
        )
