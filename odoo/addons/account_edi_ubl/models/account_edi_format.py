...
class AccountEdiFormat(models.Model):
    ...
    def _import_ubl(self, tree, invoice):
        ...
        with Form(invoice) as invoice_form:
            ...
            invoice_form.invoice_date_due = (
                invoice_form.invoice_date_due or elements and elements[0].text
            )

            # Currency
            currency = self._retrieve_currency(
                _find_value("//cbc:DocumentCurrencyCode")
            )
            ...
            if elements:
                invoice_form.invoice_incoterm_id = self.env["account.incoterms"].search(
            ...
            counterpart = (
                "Customer"
                if invoice_form.move_type in ("out_invoice", "out_refund")
                else "Supplier"
            )
            invoice_form.partner_id = self._retrieve_partner(
                name=_find_value(
                    f"//cac:Accounting{counterpart}Party/cac:Party//cbc:Name"
                ),
                phone=_find_value(
                    f"//cac:Accounting{counterpart}Party/cac:Party//cbc:Telephone"
                ),
                mail=_find_value(
                    f"//cac:Accounting{counterpart}Party/cac:Party//cbc:ElectronicMail"
                ),
                vat=_find_value(
                    f"//cac:Accounting{counterpart}Party/cac:Party//cbc:CompanyID"
                ),
            )
                ...
                with invoice_form.invoice_line_ids.new() as invoice_line_form:
                    ...
                    invoice_line_form.product_id = self._retrieve_product(
                        default_code=_find_value(
                            "cac:Item/cac:SellersItemIdentification/cbc:ID", eline
                        ),
                    ...
                    invoice_line_form.price_unit = (
                        price_unit
                        or line_extension_amount / invoice_line_form.quantity
                        or 0.0
                    )
                    ...
                    if elements and elements[0].text:
                        ...
                        invoice_line_form.name = invoice_line_form.name.replace(
                            "%month%",
                            str(fields.Date.to_date(invoice_form.invoice_date).month),
                        )  # TODO: full name in locale
                        invoice_line_form.name = invoice_line_form.name.replace(
                            "%year%",
                            str(fields.Date.to_date(invoice_form.invoice_date).year),
                        )
                    else:
                        partner_name = _find_value(
                            "//cac:AccountingSupplierParty/cac:Party//cbc:Name"
                        )
                        invoice_line_form.name = "{} ({})".format(
                            partner_name or "",
                            invoice_form.invoice_date,
                        )

                    # Taxes
                    tax_element = eline.xpath(
    ...
    def _export_ubl(self, invoice):
        ...
        xml_content += self.env.ref("account_edi_ubl.export_ubl_invoice")._render(
