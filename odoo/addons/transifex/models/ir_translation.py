    ...
    def _get_transifex_url(self):
        ...
        if not base_url or not tx_sections:
            ...
            languages = (
                ...
                .search([("code", "in", translation_languages)])
            )
            ...
            for translation in self:
                if (
                    not translation.module
                    or not translation.src
                    or translation.lang == "en_US"
                ):
                ...
                src = werkzeug.urls.url_quote_plus(
                    translation.src[:50].replace("\n", "").replace("'", "\\'")
                )
                src = f"'{src}'" if "+" in src else src
                translation.transifex_url = (
                    "%(url)s/%(project)s/translate/#%(lang)s/%(module)s/42?q=%(src)s"
                    % {
                        "url": base_url,
                        "project": project,
                        "lang": lang_code,
