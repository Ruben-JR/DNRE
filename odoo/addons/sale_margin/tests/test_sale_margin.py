...
class TestSaleMargin(common.TransactionCase):
    def setUp(self):
        ...
        self.partner_invoice_address_id = (
    ...
    def test_sale_margin(self):
        ...
        self.assertEqual(
            sale_order_so11.margin, 6000.00, "Sales order profit should be 6000.00"
        )
        self.assertEqual(
            sale_order_so11.margin_percent, 0.3, "Sales order margin should be 30%"
        )
        sale_order_so11.order_line[1].purchase_price = 800
        self.assertEqual(
            sale_order_so11.margin, 5000.00, "Sales order margin should be 5000.00"
        )
    ...
    def test_sale_margin1(self):
        ...
        self.assertEqual(
            sale_order_so12.order_line[0].margin,
            -20.00,
            "Sales order profit should be -20.00",
        )
        self.assertEqual(
            sale_order_so12.order_line[0].margin_percent,
            -1,
            "Sales order margin percentage should be -100%",
        )
        self.assertEqual(
            sale_order_so12.order_line[1].margin,
            -100.00,
            "Sales order profit should be -100.00",
        )
        self.assertEqual(
            sale_order_so12.order_line[1].margin_percent,
            1.00,
            "Sales order margin should be 100% when the cost is zero and price defined",
        )
        # Verify that margin field gets bind with the value.
        self.assertEqual(
            sale_order_so12.margin, -120.00, "Sales order margin should be -120.00"
        )
        self.assertEqual(
            sale_order_so12.margin_percent, 1.5, "Sales order margin should be 150%"
        )
    ...
    def test_sale_margin2(self):
        ...
        # Verify that margin field of Sale Order Lines gets bind with the value.
        self.assertEqual(
            sale_order_so13.order_line[0].margin,
            70.00,
            "Sales order profit should be 70.00",
        )
        self.assertEqual(
            sale_order_so13.order_line[0].margin_percent,
            1.0,
            "Sales order margin percentage should be 100.00",
        )
        # Verify that margin field gets bind with the value.
        self.assertEqual(
            sale_order_so13.margin, 70.00, "Sales order profit should be 70.00"
        )
        self.assertEqual(
            sale_order_so13.margin_percent,
            1.00,
            "Sales order margin percentage should be 100.00",
        )
    ...
    def test_sale_margin3(self):
        ...
        # Verify that margin field of Sale Order Lines gets bind with the value.
        self.assertEqual(
            sale_order_so14.order_line[0].margin,
            150.00,
            "Sales order profit should be 150.00",
        )
        self.assertEqual(
            sale_order_so14.order_line[0].margin_percent,
            0.5,
            "Sales order margin should be 100%",
        )
        self.assertEqual(
            sale_order_so14.order_line[1].margin,
            -50.00,
            "Sales order profit should be -50.00",
        )
        self.assertEqual(
            sale_order_so14.order_line[1].margin_percent,
            1.0,
            "Sales order margin should be 100%",
        )
        # Verify that margin field gets bind with the value.
        self.assertEqual(
            sale_order_so14.margin, 100.00, "Sales order profit should be 100.00"
        )
        self.assertEqual(
            sale_order_so14.margin_percent, 0.4, "Sales order margin should be 40%"
        )
