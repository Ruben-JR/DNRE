...
class SaleOrder(models.Model):
    ...
    def _get_no_effect_on_threshold_lines(self):
        ...
        free_delivery_product = (
            ...
            .search([("reward_type", "=", "free_shipping")])
            .mapped("discount_line_product_id")
        )
        lines = self.order_line.filtered(
            lambda line: line.is_delivery or line.product_id in free_delivery_product
        )
...
class SalesOrderLine(models.Model):
    ...
    def unlink(self):
        ...
        applied_programs = (
            orders.mapped("no_code_promo_program_ids")
        ...
        lines_to_unlink = self.filtered(
            lambda line: line.product_id not in free_shipping_products
        )
        # Unless these lines are the last ones
        res = super(SalesOrderLine, lines_to_unlink).unlink()
        only_free_shipping_line_orders = orders.filtered(
            lambda order: len(order.order_line.ids) == 1
            and order.order_line.is_reward_line
        )
        super(
            SalesOrderLine, only_free_shipping_line_orders.mapped("order_line")
        ).unlink()
        return res
