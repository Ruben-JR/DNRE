...
class GoogleDrive(models.Model):
    ...
    @api.model
    def write_config_formula(
        self, attachment_id, spreadsheet_key, model, domain, groupbys, view_id
    ):
        ...
        if groupbys:
            fields = f"{groupbys} {fields}"
            formula = '=oe_read_group("{}";"{}";"{}";"{}")'.format(
                model,
                fields,
                groupbys,
                domain,
            ...
            formula = f'=oe_browse("{model}";"{fields}";"{domain}")'
        url = self.env["ir.config_parameter"].sudo().get_param("web.base.url")
        dbname = self._cr.dbname
        user = (
            self.env["res.users"]
            .browse(self.env.user.id)
            .read(["login", "password"])[0]
        )
        username = user["login"]
        password = user["password"]
        if not password:
            config_formula = f'=oe_settings("{url}";"{dbname}")'
        else:
            config_formula = '=oe_settings("{}";"{}";"{}";"{}")'.format(
                url,
                dbname,
                username,
                password,
        ...
        try:
            req = requests.post(
                "https://sheets.googleapis.com/v4/spreadsheets/%s/values:batchUpdate?%s"
                % (
                    spreadsheet_key,
                    werkzeug.urls.url_encode({"access_token": access_token}),
                ),
            ...
            _logger.warning(
                "An error occurred while writing the formula on the Google Spreadsheet."
            )
