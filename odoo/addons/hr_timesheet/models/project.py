...
class Project(models.Model):
    ...
    has_planned_hours_tasks = fields.Boolean(
        compute="_compute_remaining_hours",
        compute_sudo=True,
    ...
    @api.depends(
        "allow_timesheets", "task_ids.planned_hours", "task_ids.remaining_hours"
    )
    def _compute_remaining_hours(self):
        group_read = self.env["project.task"].read_group(
            ...
            fields=["planned_hours:sum", "remaining_hours:sum"],
            groupby="project_id",
        )
    ...
    def _compute_timesheet_count(self):
        ...
        if self.env["account.analytic.line"].check_access_rights(
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            allow_timesheets = values.get(
                "allow_timesheets", defaults.get("allow_timesheets")
            )
            analytic_account_id = values.get(
...
class Task(models.Model):
    ...
    def _fields_view_get(
        ...
        result["arch"] = (
            ...
            .sudo()
            ._apply_timesheet_label(result["arch"])
        )
        ...
        if view_type in [
            ...
            result["arch"] = self.env["account.analytic.line"]._apply_time_label(
                result["arch"], related_model=self._name
            )
