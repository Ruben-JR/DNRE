...
class AccountAnalyticLine(models.Model):
    ...
    def name_get(self):
        ...
        def _get_display_name(project_id, task_id):
            ...
            """
            if task_id:
                return "{} - {}".format(project_id[1], task_id[1])
            return project_id[1]
    ...
    def _compute_user_id(self):
        for line in self:
            line.user_id = (
                line.employee_id.user_id if line.employee_id else line._default_user()
            )
    ...
    def create(self, vals_list):
        ...
        user_ids = list(
            map(
                lambda x: x.get("user_id", default_user_id),
                filter(
                    lambda x: not x.get("employee_id") and x.get("project_id"),
                    vals_list,
                ),
            )
        )
        ...
        for employee in employees:
            employee_for_user_company[employee.user_id.id][
        ...
        for vals in vals_list:
            ...
            if not vals.get("employee_id") and vals.get("project_id"):
                employee_for_company = employee_for_user_company.get(
                    vals.get("user_id", default_user_id), False
                )
    ...
    def fields_view_get(
        ...
        result["arch"] = self._apply_timesheet_label(
    ...
    def _timesheet_preprocess(self, vals):
        ...
        if (vals.get("project_id") or vals.get("task_id")) and not vals.get(
            "partner_id"
        ):
            partner_id = False
            if vals.get("task_id"):
                partner_id = (
                    self.env["project.task"].browse(vals["task_id"]).partner_id.id
                )
            else:
                partner_id = (
                    self.env["project.project"].browse(vals["project_id"]).partner_id.id
                )
            if partner_id:
