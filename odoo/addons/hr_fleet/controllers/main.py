...
class HrFleet(Controller):
    ...
    def get_claim_report_user(self, employee_id, **post):
        ...
        partner_ids = (
            employee.user_id.partner_id | employee.sudo().address_home_id
        ).ids
        if not employee or not partner_ids:
            return request.not_found()

        car_assignation_logs = request.env["fleet.vehicle.assignation.log"].search(
        ...
        for document in doc_list:
            ...
            try:
                reader = PdfFileReader(
                    io.BytesIO(base64.b64decode(document.datas)),
                    strict=False,
