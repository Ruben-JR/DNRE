...
class IapAccount(models.Model):
    ...
    def get_credits_url(self, service_name, base_url="", credit=0, trial=False):
        ...
        return "{}?{}".format(base_url, werkzeug.urls.url_encode(d))
    ...
    def get_account_url(self):
        ...
        return "{}?{}".format(endpoint + route, werkzeug.urls.url_encode(d))
    ...
    def get_config_account_url(self):
        ...
        if account:
            url = "/web#id={}&action={}&model=iap.account&view_type=form&menu_id={}".format(
                account.id,
                action.id,
                menu.id,
            )
        else:
            url = "/web#action={}&model=iap.account&view_type=form&menu_id={}".format(
                action.id,
                menu.id,
            )
