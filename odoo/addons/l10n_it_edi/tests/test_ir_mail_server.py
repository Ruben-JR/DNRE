...
class PecMailServerTests(AccountEdiTestCommon):
    ...
    def setUpClass(cls):
        ...
        cls.server = (
            cls.env["fetchmail.server"]
            .sudo()
            .create(
                {"name": "test_server", "server_type": "imap", "l10n_it_is_pec": True}
            )
        )
    ...
    def _create_invoice(self, content, filename):
        ...
        with patch.object(self.server._cr, "commit", return_value=None):
            ...
            return self.server._create_invoice_from_mail(
                content, filename, "fake@address.be"
            )
    ...
    def test_receive_signed_vendor_bill(self):
        ...
        with freeze_time("2020-04-06"):
            invoices = self._create_invoice(
                self.signed_invoice_content, self.signed_invoice_filename
            )
    ...
    def _test_receipt(self, receipt_type, source_state, destination_state):
        ...
        receipt_mail_attachment = create_mail_attachment(
            receipt_filename, receipt_content, {}
        )
