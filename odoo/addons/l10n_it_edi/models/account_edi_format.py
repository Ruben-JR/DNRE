...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_it_edi_generate_electronic_invoice_filename(self, invoice):
        ...
        return "{country_code}{codice}_{progressive_number}.xml".format(
    ...
    def _l10n_it_edi_check_ordinary_invoice_configuration(self, invoice):
        ...
        if (
            seller.l10n_it_has_tax_representative
    ...
    def _l10n_it_edi_is_required_for_invoice(self, invoice):
        ...
        return (
            ...
            and invoice.l10n_it_send_state
            not in ("sent", "delivered", "delivered_accepted")
    ...
    def _post_fattura_pa(self, invoices):
        ...
        if invoice.l10n_it_einvoice_id and invoice.l10n_it_send_state not in [
            "invalid",
            "to_send",
        ]:
            return {
                "error": _(
                    "You can't regenerate an E-Invoice when the first one is sent and there are no errors"
                )
            }
    ...
    def _check_filename_is_fattura_pa(self, filename):
        return re.search(
            "([A-Z]{2}[A-Za-z0-9]{2,28}_[A-Za-z0-9]{0,5}.(xml.p7m|xml))", filename
        )
    ...
    def _create_invoice_from_binary(self, filename, content, extension):
        ...
        if extension.lower() == ".xml.p7m":
            ...
            if decoded_content is not None and self._is_fattura_pa(
        ...
        for body_tree in tree.xpath("//FatturaElettronicaBody"):
            ...
            with Form(invoice_ctx) as invoice_form:
                ...
                if not partner:
                    ...
                    if elements:
                        ...
                        if re.match(r"^[0-9]{11}$", codice):
                            domains.append(
                                [("l10n_it_codice_fiscale", "=", "IT" + codice)]
                            )
                ...
                for element in elements:
                    invoice_form.narration = "{}{}<br/>".format(
                        ...
                        with invoice_form.invoice_line_ids.new() as invoice_line_form:
                            ...
                            if percentage is not None:
                                ...
                                if tax:
                                    ...
                                    if l10n_it_kind_exoneration:
                                        ...
                                        message_to_log.append(
                                            _(
                                                "Tax not found with percentage: %s for the article: %s"
                                            )
                                            % (percentage, invoice_line_form.name)
                                        )
                            ...
                            if discount_elements:
                                ...
                                # Discounts in cascade summarized in 1 percentage
                                    ...
                                    total = float(
                                        element.xpath(".//PrezzoTotale")[0].text
                                    )
                ...
                if discount_elements:
                    taxable_amount = float(
                        json.loads(invoice_form.tax_totals_json)["amount_untaxed"]
                    )
