...
class FetchmailServer(models.Model):
    ...
    def fetch_mail(self):
        ...
        for server in self.filtered(lambda s: s.l10n_it_is_pec):
            _logger.info(
                "start checking for new emails on %s PEC server %s",
                server.server_type,
                server.name,
            )
            ...
            try:
                ...
                bypass_incoming_address_filter = self.env[
                    "ir.config_parameter"
                ].get_param("l10n_it_edi.bypass_incoming_address_filter", False)
        ...
        return super(
            FetchmailServer, self.filtered(lambda s: not s.l10n_it_is_pec)
        ).fetch_mail()
        ...
        for attachment in attachments:
            ...
            if attachment_ext != "zip":
                if split_underscore[1] in [
                    ...
                    if match:
                        ...
                        if match.groups()[1] == "xml.p7m":
                            ...
                            if not att_content_data:
                                _logger.warning(
                                    "E-invoice couldn't be read: %s", att_filename
                                )
                                continue
                    ...
                    self._create_invoice_from_mail(
                        att_content_data, att_filename, from_address
                    )
            else:
    ...
    def _create_invoice_from_mail(self, att_content_data, att_name, from_address):
        ...
        invoices = self.env.ref(
            "l10n_it_edi.edi_fatturaPA"
        )._create_invoice_from_xml_tree(att_name, tree)
        if not invoices:
        ...
        for invoice in invoices:
            invoice.with_context(
                no_new_invoice=True, default_res_id=invoice.id
            ).message_post(
                body=(_("Original E-invoice XML file")), attachment_ids=[attachment.id]
            )
            ...
            for attachment_name in z.namelist():
                ...
                if split_underscore[1] == "AT":
                    ...
                    try:
                        ...
                        _logger.info(
                            "Error in decoding new receipt file: %s", attachment_name
                        )
                        return
                    ...
                    if not related_invoice:
                        _logger.info(
                            "Error: invoice not found for receipt file: %s", filename
                        )
                        return
    ...
    def _message_receipt_invoice(self, receipt_type, attachment):
        ...
        if receipt_type == "RC":
            ...
            if not related_invoice:
                _logger.info(
                    "Error: invoice not found for receipt file: %s", attachment.fname
                )
                return
            ...
            related_invoice.activity_schedule(
                ...
                if related_invoice.invoice_user_id
    ...
    def _return_error_xml(self, tree):
        ...
        for element in elements:
            ...
            if descrizione:
                output_str += "<li>Errore {}: {}</li>".format(
                    element[0].text, descrizione
                )
        return output_str + "</ul>"
...
class IrMailServer(models.Model):
    ...
    def _get_test_email_addresses(self):
        ...
        company = self.env["res.company"].search(
            [("l10n_it_mail_pec_server_id", "=", self.id)], limit=1
        )
        if not company:
        ...
        if not email_to:
            raise UserError(
                _("Please configure Government PEC-mail	in company settings")
            )
