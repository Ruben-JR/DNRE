...
class AccountMove(models.Model):
    ...
    l10n_it_send_state = fields.Selection(
        [
            ...
            (
                "delivered_accepted",
                "This invoice is delivered and accepted by destinatory",
            ),
            (
                "delivered_refused",
                "This invoice is delivered and refused by destinatory",
            ),
            (
                "delivered_expired",
                "This invoice is delivered and expired (expiry of the maximum term for communication of acceptance/refusal)",
            ),
            (
                "failed_delivery",
                "Delivery impossible, ES certify that it has received the invoice and that the file \
    ...
    def _check_before_xml_exporting(self):
        ...
        errors = self.env[
            "account.edi.format"
        ]._l10n_it_edi_check_invoice_configuration(self)
        if errors:
            raise UserError(
                self.env["account.edi.format"]._format_error_message(
    ...
    def _prepare_fatturapa_export_values(self):
        ...
        for _tax_name, tax_dict in tax_details["tax_details"].items():
            ...
            if (
                tax_dict["tax"].price_include
                and tax_dict["tax"].amount_type == "percent"
            ):
                expected_base_amount = (
                    tax_amount * 100 / tax_rate if tax_rate else False
                )
        ...
        template_values = {
            ...
            "normalize_codice_fiscale": self.env[
                "res.partner"
    ...
    def _post(self, soft=True):
        ...
        for move in posted.filtered(
            lambda m: m.l10n_it_send_state == "to_send"
    ...
    def send_pec_mail(self):
        ...
        if (
            ...
            self.message_post(
                body=(
                    _(
                        "Error when sending mail with E-Invoice: Your company must have a mail PEC server and must indicate the mail PEC that will send electronic invoice."
                    )
                )
            )
        ...
        if self.l10n_it_send_state not in allowed_state:
            raise UserError(
                _(
                    "%s isn't in a right state. It must be in a 'Not yet send' or 'Invalid' state."
                )
                % (self.display_name)
            )
        ...
        message = self.env["mail.message"].create(
            {
                ...
                "body": _("Sending file: %s to ES: %s")
                % (
                    self.l10n_it_einvoice_name,
                    self.env.company.l10n_it_address_recipient_fatturapa,
                ),
        ...
        mail_fattura = (
            self.env["mail.mail"]
            .sudo()
            .with_context(wo_bounce_return_path=True)
            .create(
                {
        ...
        for element in elements:
            ...
            for line in element.iter():
                if line.text:
                    ...
                    if text:
                        output_str += f"<li>{line.tag}: {text}</li>"
            output_str += "</ul>"
        return output_str
    ...
    def _compose_multi_info_message(self, tree, element_tags):
        ...
        for element_tag in element_tags:
            ...
            for element in elements:
                ...
                if text:
                    output_str += f"<li>{element.tag}: {text}</li>"
