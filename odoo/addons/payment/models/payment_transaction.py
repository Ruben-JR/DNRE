...
class PaymentTransaction(models.Model):
    ...
    def _check_state_authorized_supported(self):
        ...
        illegal_authorize_state_txs = self.filtered(
            ...
            and not tx.acquirer_id.support_authorization
        )
        if illegal_authorize_state_txs:
            raise ValidationError(
                _(
                    ...
                    ", ".join(
                        set(illegal_authorize_state_txs.mapped("acquirer_id.name"))
                    ),
                )
            )
    ...
    def _get_processing_values(self):
        ...
        if self.operation in ("online_redirect", "validation"):
            ...
            if redirect_form_view:  # Some acquirer don't need a redirect form
                ...
                redirect_form_html = redirect_form_view._render(
        ...
        return self.create(
            {
                ...
                "reference": self._compute_reference(
                    self.provider, prefix=f"R-{self.reference}"
                ),
    ...
    def _update_state(self, allowed_states, target_state, state_message):
        ...
        def _classify_by_state(_transactions):
            ...
            """
            _txs_to_process = _transactions.filtered(
                lambda _tx: _tx.state in allowed_states
            )
            _txs_already_processed = _transactions.filtered(
        ...
        txs_to_process, txs_already_processed, txs_wrong_state = _classify_by_state(
            self
        )
    ...
    def _execute_callback(self):
        ...
        for tx in self.filtered(lambda t: not t.sudo().callback_is_done):
            ...
            if not consteq(ustr(valid_callback_hash), callback_hash):
                _logger.warning(
                    "invalid callback signature for transaction with id %s", tx.id
                )
    ...
    def _create_payment(self, **extra_create_values):
        ...
        payment_method_line = (
            self.acquirer_id.journal_id.inbound_payment_method_line_ids.filtered(
                lambda l: l.code == self.provider
            )
        )
        payment_values = {
            "amount": abs(
