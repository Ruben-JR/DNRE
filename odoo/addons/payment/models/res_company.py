...
class ResCompany(models.Model):
    ...
    def _run_payment_onboarding_step(self, menu_id):
        ...
        if (
            stripe_acquirer.state == "disabled"
        ):  # The onboarding step has never been run
            # Configure PayPal
            paypal_acquirer = new_env.ref("payment.payment_acquirer_paypal")
            if not paypal_acquirer.paypal_email_account:
                paypal_acquirer.paypal_email_account = (
                    new_env.user.email or new_env.company.email
                )
            if (
                paypal_acquirer.state == "disabled"
                and paypal_acquirer.paypal_email_account
            ):
