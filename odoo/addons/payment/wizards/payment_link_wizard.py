...
class PaymentLinkWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        amount_field = (
            "amount_residual" if res_model == "account.move" else "amount_total"
        )
    ...
    def _compute_payment_acquirer_selection(self):
        for link in self:
            link.payment_acquirer_selection = (
                link.acquirer_id.id if link.acquirer_id else "all"
            )
    ...
    def _inverse_payment_acquirer_selection(self):
        for link in self:
            link.acquirer_id = (
                link.payment_acquirer_selection
                if link.payment_acquirer_selection != "all"
                else False
            )
    ...
    def _get_payment_acquirer_available(
        self, company_id=None, partner_id=None, currency_id=None
    ):
