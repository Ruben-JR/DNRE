...
class PaymentWizard(models.TransientModel):
    ...
    payment_method = fields.Selection(
        ...
        default=lambda self: self._get_default_payment_acquirer_onboarding_value(
            "payment_method"
        ),
    )
    ...
    def _get_default_payment_acquirer_onboarding_value(self, key):
        ...
        self._payment_acquirer_onboarding_cache[
            "payment_method"
        ...
        if "payment_paypal" in installed_modules:
            ...
            self._payment_acquirer_onboarding_cache["paypal_email_account"] = (
                acquirer["paypal_email_account"] or self.env.user.email or ""
            )
            self._payment_acquirer_onboarding_cache["paypal_seller_account"] = acquirer[
                "paypal_seller_account"
            ]
            self._payment_acquirer_onboarding_cache["paypal_pdt_token"] = acquirer[
                "paypal_pdt_token"
            ]

        if "payment_stripe" in installed_modules:
            acquirer = self.env.ref("payment.payment_acquirer_stripe")
            self._payment_acquirer_onboarding_cache["stripe_secret_key"] = acquirer[
                "stripe_secret_key"
            ]
            self._payment_acquirer_onboarding_cache[
                "stripe_publishable_key"
            ] = acquirer["stripe_publishable_key"]
        ...
        self._payment_acquirer_onboarding_cache["manual_post_msg"] = manual_payment[
            "pending_msg"
        ]
        self._payment_acquirer_onboarding_cache["journal_name"] = (
