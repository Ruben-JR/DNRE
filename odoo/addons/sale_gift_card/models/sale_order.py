...
class SaleOrder(models.Model):
    ...
    def _compute_gift_card_count(self):
        for record in self:
            record.gift_card_count = len(
                record.order_line.mapped("generated_gift_card_ids")
            )
    ...
    def _constrains_state(self):
        ...
        for record in self.filtered(lambda so: so.state == "sale"):
            for gift_card_order_line in record.order_line.filtered(
                lambda ol: ol.product_id.detailed_type == "gift"
            ):
    ...
    def _pay_with_gift_card(self, gift_card):
        ...
        if not error and amount > 0:
            ...
            gift_card.redeem_line_ids.filtered(
                lambda redeem: redeem.state != "sale"
            ).unlink()
    ...
    def _send_gift_card_mail(self):
        template = self.env.ref(
            "sale_gift_card.mail_template_gift_card", raise_if_not_found=False
        )
    ...
    def _recompute_gift_card_lines(self):
        for record in self:
            ...
            for gift_card_line in gift_payment_lines.sorted(
                lambda line: line.gift_card_id.expired_date
            ):
                amount = min(
                    to_pay,
                    gift_card_line.gift_card_id.balance_converted(record.currency_id),
                )
                if amount:
                    to_pay -= amount
                    if (
                        gift_card_line.price_unit != -amount
                        or gift_card_line.product_uom_qty != 1
                    ):
                        lines_to_update.append(
                            fields.Command.update(
                                gift_card_line.id,
                                {"price_unit": -amount, "product_uom_qty": 1},
                            )
                        )
                else:
...
class SaleOrderLine(models.Model):
    ...
    generated_gift_card_ids = fields.One2many(
        "gift.card", "buy_line_id", string="Bought Gift Card"
    )
