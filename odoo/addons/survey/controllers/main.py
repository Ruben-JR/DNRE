...
class Survey(http.Controller):
    ...
    def survey_start(self, survey_token, answer_token=None, email=False, **post):
        ...
        * a token linked to an answer or generate a new token if access is allowed;
        ...
        return request.redirect(
            f"/survey/{survey_sudo.access_token}/{answer_sudo.access_token}"
        )
    ...
    def _prepare_question_html(self, survey_sudo, answer_sudo, **post):
        ...
        if answer_sudo.state == "done":
            survey_content = request.env.ref("survey.survey_fill_form_done")._render(
                survey_data
            )
        else:
    ...
    @http.route(
        "/survey/get_background_image/<string:survey_token>/<string:answer_token>",
        type="http",
        auth="public",
        website=True,
        sitemap=False,
    ...
    def survey_get_question_image(
        ...
        if not survey_sudo.question_ids.filtered(
    ...
    def _generate_report(self, user_input, download=True):
        report = (
            request.env.ref("survey.certification_report")
