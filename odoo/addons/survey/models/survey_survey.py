...
class Survey(models.Model):
    ...
    def _compute_survey_statistic(self):
        ...
        read_group_res = UserInput.read_group(
            base_domain,
            ["survey_id", "state"],
    ...
    def _check_answer_creation(
        ...
        if test_entry:
            ...
            if self.access_mode == "internal" and (
                not user or not user.has_group("base.group_user")
            ):
                raise exceptions.UserError(
                    _(
    ...
    def action_survey_user_input(self):
        ...
        ctx = dict(self.env.context)
        ctx.update(
            {"search_default_survey_id": self.ids[0], "search_default_not_test": 1}
        )
        action["context"] = ctx
        return action
