...
class PaymentTransaction(models.Model):
    ...
    def _get_tx_from_feedback_data(self, provider, data):
        ...
        if sign != sign_check:
            raise ValidationError(
                "Alipay: "
                + _(
                    "Expected signature %(sc) but received %(sign)s.",
                    sc=sign_check,
                    sign=sign,
                )
            )
    ...
    def _process_feedback_data(self, data):
        ...
        if (
            float_compare(
                float(data.get("total_fee", "0.0")), (self.amount + self.fees), 2
            )
            != 0
        ):
            ...
            raise ValidationError(
                "Alipay: " + _("The amount does not match the total + fees.")
            )
        if self.acquirer_id.alipay_payment_method == "standard_checkout":
            ...
            raise ValidationError(
                "Alipay: "
                + _("The seller email does not match the configured Alipay account.")
            )
