...
class FleetVehicle(models.Model):
    ...
    description = fields.Html(
        "Vehicle Description", help="Add a note about this vehicle"
    )
    ...
    car_value = fields.Float(
        string="Catalog Value (VAT Incl.)", help="Value of the bought vehicle"
    )
    net_car_value = fields.Float(
        string="Purchase Value", help="Purchase value of the vehicle"
    )
    ...
    def _compute_count_all(self):
        ...
        for record in self:
            ...
            ].search_count([("vehicle_id", "=", record.id)])
    ...
    def toggle_active(self):
        self.env["fleet.vehicle.log.contract"].with_context(active_test=False).search(
            [("vehicle_id", "in", self.ids)]
        ).toggle_active()
        self.env["fleet.vehicle.log.services"].with_context(active_test=False).search(
            [("vehicle_id", "in", self.ids)]
        ).toggle_active()
        super().toggle_active()
