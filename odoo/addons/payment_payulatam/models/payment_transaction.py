...
class PaymentTransaction(models.Model):
    ...
    def _compute_reference(self, provider, prefix=None, separator="-", **kwargs):
        ...
        if provider == "payulatam":
            if not prefix:
                ...
                prefix = (
                    self.sudo()._compute_reference_prefix(provider, separator, **kwargs)
                    or None
                )
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        payulatam_values = {
            ...
            "responseUrl": urls.url_join(
                self.get_base_url(), PayuLatamController._return_url
            ),
            "confirmationUrl": urls.url_join(
                self.get_base_url(), PayuLatamController._webhook_url
            ),
            "api_url": api_url,
        }
        if self.acquirer_id.state != "enabled":
