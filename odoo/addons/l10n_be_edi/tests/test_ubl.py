...
class TestL10nBeEdi(AccountEdiTestCommon):
    ...
    def test_efff_simple_case(self):
        ...
        self.assert_generated_file_equal(
            self.invoice, self.expected_invoice_efff_values
        )
    ...
    def test_efff_group_of_taxes(self):
        self.invoice.write(
            {
                "invoice_line_ids": [
                    (
                        ...
                        {"tax_ids": [Command.set(self.tax_group.ids)]},
                    )
                ],
            }
        )
        ...
        self.assert_generated_file_equal(
            self.invoice, self.expected_invoice_efff_values, applied_xpath
        )
    ...
    def test_invoice_edi_xml_update(self):
        ...
        self.update_invoice_from_file(
            "l10n_be_edi", "test_xml_file", "efff_test.xml", invoice
        )
    ...
    def test_invoice_edi_xml_create(self):
        invoice_count = len(self.env["account.move"].search([]))
        invoice = self.create_invoice_from_file(
            "l10n_be_edi", "test_xml_file", "efff_test.xml"
        )
