...
class TestSaleMrpFlow(ValuationReconciliationTestCommon):
    ...
    def test_06_kit_qty_delivered_mixed_uom(self):
        ...
        for ml in move_lines:
            corr_bom_line = bom_kit_uom_1.bom_line_ids.filtered(
                lambda b: b.product_id.id == ml.product_id.id
            )
            computed_qty = ml.product_uom._compute_quantity(
                ml.product_uom_qty, corr_bom_line.product_uom_id
            )
            self.assertEqual(
    ...
    def test_11_sale_mrp_explode_kits_uom_quantities(self):
        ...
        move_component_unit = order.picking_ids[0].move_lines.filtered(
    ...
    def test_anglo_saxo_return_and_credit_note(self):
        ...
        stock_out_aml = amls.filtered(
            lambda aml: aml.account_id == stock_output_account
        )
        self.assertEqual(
