...
class Project(models.Model):
    ...
    def _compute_purchase_orders_count(self):
        purchase_orders_data = self.env["purchase.order.line"].read_group(
            ...
            ["account_analytic_id", "order_id:count_distinct"],
            ["account_analytic_id"],
        )
        mapped_data = {
            data["account_analytic_id"][0]: data["order_id"]
            for data in purchase_orders_data
        }
        for project in self:
            project.purchase_orders_count = mapped_data.get(
