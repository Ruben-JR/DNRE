...
class WebsiteEventMeetController(EventCommunityController):
    ...
    def _sort_event_rooms(self, room):
        return (
            room.website_published,
            room.is_pinned,
            room.room_last_activity,
            room.id,
        )
    ...
    def _event_meeting_rooms_get_values(self, event, lang=None):
        ...
        meeting_rooms_all = (
            request.env["event.meeting.room"].sudo().search(search_domain)
        )
        if lang:
        ...
        return {
            ...
            "current_lang": request.env["res.lang"].browse(int(lang))
            if lang
            else False,
    ...
    def create_meeting_room(self, event, **post):
        if (
            ...
            or (
                ...
                and not request.env.user.user_has_groups("base.group_user")
            )
        ...
        return request.redirect(
            f"/event/{slug(event)}/meeting_room/{slug(meeting_room)}"
        )
    ...
    def _event_meeting_room_page_get_values(self, event, meeting_room):
        ...
        if not request.env.user.has_group("event.group_event_manager"):
            ...
            meeting_rooms_other = meeting_rooms_other.filtered(
                lambda m: not m.room_is_full
            )

        meeting_rooms_other = meeting_rooms_other.sorted(
            self._sort_event_rooms, reverse=True
        )
