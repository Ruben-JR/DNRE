...
class AccountEdiFormat(models.Model):
    ...
    def _get_bis3_values(self, invoice):
        ...
        for line_vals in values["invoice_line_vals_list"]:
            if (
                len(
                    values["tax_details"]["invoice_line_tax_details"][
                        line_vals["line"]
                    ]["tax_details"]
                )
                > 1
                ...
                raise UserError(
                    "Multiple vat percentage not supported on the same invoice line"
                )

        tax_details_no_tax_closing = invoice._prepare_edi_tax_details(
            filter_to_apply=lambda x: not x[
                "tax_repartition_line_id"
            ].use_in_tax_closing,
        )
        for line_vals in values["invoice_line_vals_list"]:
            line_vals["price_subtotal_with_no_tax_closing"] = line_vals[
                "line"
            ].price_subtotal
            for tax_detail in tax_details_no_tax_closing["invoice_line_tax_details"][
                line_vals["line"]
            ]["tax_details"].values():
                line_vals["price_subtotal_with_no_tax_closing"] += tax_detail[
                    "tax_amount_currency"
                ]

        values["total_untaxed_amount"] = sum(
            x["price_subtotal_with_no_tax_closing"]
            for x in values["invoice_line_vals_list"]
        )
        ...
        for partner_vals in (values["customer_vals"], values["supplier_vals"]):
            ...
            if partner.country_id.code in COUNTRY_EAS:
                ...
                partner_vals["bis3_endpoint_scheme"] = COUNTRY_EAS[
    ...
    def _decode_bis3(self, tree, invoice):
        ...
        with Form(
            invoice.with_context(
                default_move_type=move_type, default_journal_id=default_journal.id
            )
        ) as invoice_form:
            # Reference
                ...
                with invoice_form.invoice_line_ids.new() as invoice_line_form:
                    ...
                    invoice_line_form.product_id = self._retrieve_product(
                        default_code=_find_value(
                            "./{*}Item/{*}SellersItemIdentification/{*}ID", eline
                        ),
                    ...
                    line_extension_amount = (
                        element is not None and float(element.text) or 0.0
                    )
