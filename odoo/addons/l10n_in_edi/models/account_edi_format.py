...
class AccountEdiFormat(models.Model):
    ...
    def _is_required_for_invoice(self, invoice):
        ...
        if self.code == "in_einvoice_1_03":
            return (
                ...
                and invoice.country_code == "IN"
                and invoice.l10n_in_gst_treatment
                in (
                    "regular",
                    "composition",
                    "overseas",
    ...
    def _check_move_configuration(self, move):
        ...
        for line in move.invoice_line_ids.filtered(
            lambda line: not (line.display_type or line.is_rounding_line)
        ):
            if line.product_id:
        ...
        if response.get("error"):
            ...
            if "2150" in error_codes:
                ...
                response = self._l10n_in_edi_get_irn_by_details(
                    invoices.company_id,
                    {
                        "doc_type": invoices.move_type == "out_refund"
                        and "CRN"
                        or "INV",
                        "doc_num": invoices.name,
                        "doc_date": invoices.invoice_date
                        and invoices.invoice_date.strftime("%d/%m/%Y")
                        or False,
                    },
                )
            ...
            if "no-credit" in error_codes:
                ...
                error_message = "<br/>".join(
                    [
                        "[{}] {}".format(e.get("code"), html_escape(e.get("message")))
                        for e in error
                    ]
                )
                res[invoices] = {
        ...
        for invoice in invoices:
            ...
            if response.get("error"):
                ...
                if "1005" in error_codes:
                    ...
                    if not authenticate_response.get("error"):
                        error = []
                        response = self._l10n_in_edi_cancel(
                            invoice.company_id, cancel_json
                        )
                        if response.get("error"):
                            error = response["error"]
                            error_codes = [e.get("code") for e in error]
                ...
                if "no-credit" in error_codes:
                    ...
                    error_message = "<br/>".join(
                        [
                            "[{}] {}".format(
                                e.get("code"), html_escape(e.get("message"))
                            )
                            for e in error
                        ]
                    )
    ...
    def _get_l10n_in_edi_line_details(self, index, line, line_tax_details, sign):
        ...
        if full_discount_or_zero_quantity:
            ...
            unit_price_in_inr = (
                (line.balance / (1 - (line.discount / 100))) / line.quantity
            ) * sign
        return {
            "SlNo": str(index),
            ...
            "Discount": self._l10n_in_round_value(
                (unit_price_in_inr * line.quantity) * (line.discount / 100)
            ),
            ...
            "IgstAmt": self._l10n_in_round_value(
                tax_details_by_code.get("igst_amount", 0.00) * sign
            ),
            "CgstAmt": self._l10n_in_round_value(
                tax_details_by_code.get("cgst_amount", 0.00) * sign
            ),
            "SgstAmt": self._l10n_in_round_value(
                tax_details_by_code.get("sgst_amount", 0.00) * sign
            ),
            "CesRt": self._l10n_in_round_value(
                tax_details_by_code.get("cess_rate", 0.00), 3
            ),
            "CesAmt": self._l10n_in_round_value(
                tax_details_by_code.get("cess_amount", 0.00) * sign
            ),
            ...
            "StateCesAmt": self._l10n_in_round_value(
                tax_details_by_code.get("state_cess_amount", 0.00) * sign
            ),
            ...
            "OthChrg": self._l10n_in_round_value(
                tax_details_by_code.get("other_amount", 0.00) * sign
            ),
        ...
        json_payload = {
            ...
            "ItemList": [
                self._get_l10n_in_edi_line_details(
                    index, line, invoice_line_tax_details.get(line, {}), sign
                )
                for index, line in enumerate(lines, start=1)
            ],
            "ValDtls": {
                "AssVal": self._l10n_in_round_value(
                    tax_details.get("base_amount") * sign
                ),
            ...
            json_payload.update(
                {
                    "ShipDtls": self._get_l10n_in_edi_partner_details(
                        saler_buyer.get("ship_to_details")
                    )
                }
            )
        if is_overseas:
            json_payload.update(
                {
                    "ExpDtls": {
                        ...
                        and invoice.l10n_in_shipping_bill_date.strftime("%d/%m/%Y")
                        or "",
    ...
    def _l10n_in_prepare_edi_tax_details(self, move, in_foreign=False):
        def l10n_in_grouping_key_generator(tax_values):
            ...
            line_code = "other"
            tax_report_line_sc = self.env.ref(
                "l10n_in.tax_report_line_state_cess", False
            )
            if any(
                tag in tax_line.tax_tag_ids
                for tag in self.env.ref("l10n_in.tax_report_line_cess").sudo().tag_ids
            ):
                if tax_line.tax_line_id.amount_type != "percent":
                    line_code = "cess_non_advol"
                else:
                    line_code = "cess"
            elif tax_report_line_sc and any(
                tag in tax_line.tax_tag_ids for tag in tax_report_line_sc.sudo().tag_ids
            ):
                if tax_line.tax_line_id.amount_type != "percent":
                ...
                for gst in ["cgst", "sgst", "igst"]:
                    tag_ids = (
                        self.env.ref("l10n_in.tax_report_line_%s" % (gst))
                        .sudo()
                        .tag_ids
                    )
