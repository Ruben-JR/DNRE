...
class PosConfig(models.Model):
    ...
    iface_orderline_customer_notes = fields.Boolean(
    ...
    barcode_nomenclature_id = fields.Many2one(
        ...
        default=lambda self: self.env.company.nomenclature_id,
        required=True,
    )
    ...
    route_id = fields.Many2one(
        "stock.location.route", string="Spefic route for products delivered later."
    )
    ...
    def _check_currencies(self):
        ...
        if any(
            self.payment_method_ids.filtered(lambda pm: pm.is_cash_count).mapped(
                lambda pm: self.currency_id
                not in (self.company_id.currency_id | pm.journal_id.currency_id)
            )
        ):
            raise ValidationError(
                _(
    ...
    @api.onchange(
        "limit_categories", "iface_available_categ_ids", "iface_start_categ_id"
    )
    def _onchange_limit_categories(self):
        ...
        if (
            self.iface_available_categ_ids
            and self.iface_start_categ_id.id not in self.iface_available_categ_ids.ids
        ):
            self.iface_start_categ_id = False
        return res
    ...
    def name_get(self):
        ...
        for config in self:
            ...
            if (not last_session) or (last_session.state == "closed"):
                ...
                result.append(
                    (config.id, f"{config.name} ({last_session.user_id.name})")
                )
        return result
    ...
    def _set_fiscal_position(self):
        for config in self:
            if (
                config.tax_regime
                and config.default_fiscal_position_id.id
                not in config.fiscal_position_ids.ids
            ):
                config.fiscal_position_ids = [(4, config.default_fiscal_position_id.id)]
            elif (
                not config.tax_regime_selection
                and not config.tax_regime
                and config.fiscal_position_ids.ids
            ):
    ...
    def _check_groups_implied(self):
        for pos_config in self:
            for field_name in [
                f for f in pos_config.fields_get_keys() if f.startswith("group_")
            ]:
