...
class PosSession(models.Model):
        ...
        for session in self:
            ...
            if cash_payment_method:
                ...
                session.cash_register_total_entry_encoding = (
                    session.cash_register_id.total_entry_encoding
    ...
    def _compute_order_count(self):
        orders_data = self.env["pos.order"].read_group(
    ...
    def _check_bank_statement_state(self):
        for session in self:
            closed_statement_ids = session.statement_ids.filtered(
            ...
            if closed_statement_ids:
                raise UserError(
                    _(
                        ...
                        "Cash Registers: %r",
                        list(statement.name for statement in closed_statement_ids),
                    )
                )
    ...
    def create(self, values):
        ...
        cash_payment_methods = pos_config.payment_method_ids.filtered(
        ...
        for cash_journal in cash_payment_methods.mapped("journal_id"):
            ctx["journal_id"] = (
                cash_journal.id
                if pos_config.cash_control and cash_journal.type == "cash"
                else False
            )
            st_values = {
        ...
        for session in self.filtered(
            ...
            if session.config_id.cash_control and not session.rescue:
                ...
                session.cash_register_id.balance_start = (
                    last_session.cash_register_id.balance_end_real
                    if last_session
                    else 0
                )
    ...
    def _validate_session(
        ...
        if self.order_ids or self.statement_ids.line_ids:
            ...
            if self.update_stock_at_closing:
                ...
                )._compute_total_cost_at_session_closing(self.picking_ids.move_lines)
            try:
    ...
    def _create_cash_statement_lines_and_cash_move_lines(self, data):
        ...
        statements_by_journal_id = {
            statement.journal_id.id: statement for statement in self.statement_ids
        }
        ...
        for payment, amounts in split_receivables_cash.items():
            ...
            split_cash_statement_line_vals[statement].append(
                self._get_split_statement_line_vals(
                    statement, amounts["amount"], payment
                )
            )
            ...
            if not float_is_zero(
                ...
                combine_cash_statement_line_vals[statement].append(
                    self._get_combine_statement_line_vals(
                        statement, amounts["amount"], payment_method
                    )
                )
        ...
        for statement in self.statement_ids:
            split_cash_statement_lines[statement] = (
                BankStatementLine.create(split_cash_statement_line_vals[statement])
            ...
            combine_cash_statement_lines[statement] = (
                BankStatementLine.create(combine_cash_statement_line_vals[statement])
                .mapped("move_id.line_ids")
                .filtered(lambda line: line.account_id.internal_type == "receivable")
            )
            split_cash_receivable_lines[statement] = MoveLine.create(
                split_cash_receivable_vals[statement]
            )
            combine_cash_receivable_lines[statement] = MoveLine.create(
                combine_cash_receivable_vals[statement]
            )
    ...
    def _get_combine_receivable_vals(self, payment_method, amount, amount_converted):
        partial_vals = {
            ...
            "move_id": self.move_id.id,
            "name": f"{self.name} - {payment_method.name}",
        }
        return self._debit_amounts(partial_vals, amount, amount_converted)
    ...
    def open_frontend_cb(self):
        ...
        return {
            ...
            "target": "self",
            "url": self.config_id._get_pos_base_url()
            + "?config_id=%d" % self.config_id.id,
        }
    ...
    def try_cash_in_out(self, _type, amount, reason, extras):
        ...
        ).create({"amount": sign * amount, "name": reason}).run()
        message_content = [
