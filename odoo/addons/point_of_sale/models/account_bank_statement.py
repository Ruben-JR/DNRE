...
class AccountBankStatement(models.Model):
    ...
    def button_validate_or_action(self):
        ...
        for statement in self:
            if (
                statement.pos_session_id.state in ("opened", "closing_control")
                and statement.state == "open"
            ):
                raise UserError(
                    _(
                        "You can't validate a bank statement that is used in an opened Session of a Point of Sale."
                    )
                )
    ...
    def _unlink_except_linked_to_pos_session(self):
        for bs in self:
            if bs.pos_session_id:
                raise UserError(
                    _(
                        "You cannot delete a bank statement linked to Point of Sale session."
                    )
                )
