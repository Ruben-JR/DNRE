...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_eg_eta_prepare_eta_invoice(self, invoice):
        def group_tax_retention(tax_values):
            return {
                "l10n_eg_eta_code": tax_values["tax_id"].l10n_eg_eta_code.split("_")[0]
            }
        ...
        invoice_line_data, totals = self._l10n_eg_eta_prepare_invoice_lines_data(
            invoice, grouped_taxes["invoice_line_tax_details"]
        )
        eta_invoice = {
            ...
            lines.append(
                {
                    ...
                    "taxableItems": [
                        {
                            "taxType": tax["tax_id"]
                            .l10n_eg_eta_code.split("_")[0]
                            .upper()
                            .upper(),
                            "amount": self._l10n_eg_edi_round(abs(tax["tax_amount"])),
                            "subType": tax["tax_id"]
                            .l10n_eg_eta_code.split("_")[1]
                            .upper(),
    ...
    def _check_move_configuration(self, invoice):
        ...
        if not all(
            ...
            for aml in invoice.invoice_line_ids.filtered(lambda x: not x.display_type)
        ):
            errors.append(
                _(
