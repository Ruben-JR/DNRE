...
from odoo.addons.sale_coupon.tests.test_program_numbers import (
    TestSaleCouponProgramNumbers,
)
    ...
    def test_01_activate_coupon_with_existing_program(self):
        ...
        with MockRequest(
            ...
            self.assertEqual(
                order.amount_total,
                864,
                "The order total should equal 864: 3*320 - 10% discount ",
            )
            ...
            self.assertEqual(
                order.amount_total,
                576,
                "The order total should equal 576: 3*320 - 320 (free product) - 10%",
            )
