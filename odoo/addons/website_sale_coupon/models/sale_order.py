...
class SaleOrder(models.Model):
        ...
        if pending_coupon_code:
            try:
                self.env["sale.coupon.apply.code"].with_context(
                    active_id=self.id
                ).create({"coupon_code": pending_coupon_code}).process_coupon()
    ...
    def _compute_website_order_line(self):
        ...
        for order in self:
            ...
            for program in programs:
                program_lines = order.order_line.filtered(
    ...
    def _get_free_shipping_lines(self):
        self.ensure_one()
        free_shipping_prgs_ids = (
            self._get_applied_programs_with_rewards_on_current_order().filtered(
                lambda p: p.reward_type == "free_shipping"
            )
        )
        if not free_shipping_prgs_ids:
            return self.env["sale.order.line"]
        free_shipping_product_ids = free_shipping_prgs_ids.mapped(
            "discount_line_product_id"
        )
        return self.order_line.filtered(
            lambda l: l.product_id in free_shipping_product_ids
        )
