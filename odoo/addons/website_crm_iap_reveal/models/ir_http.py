...
class IrHttp(models.AbstractModel):
    ...
    def _serve_page(cls):
        ...
        if (
            ...
            if (
                not visitor_sudo
                or "lead_ids" not in visitor_sudo
                or not visitor_sudo.lead_ids
            ):
                country_code = "geoip" in request.session and request.session[
                    "geoip"
                ].get("country_code")
                state_code = "geoip" in request.session and request.session[
                    "geoip"
                ].get("region")
                if country_code:
                    try:
                        ...
                        if new_rules_excluded:
                            response.set_cookie(
                                "rule_ids", ",".join(new_rules_excluded)
                            )
                    except Exception:
