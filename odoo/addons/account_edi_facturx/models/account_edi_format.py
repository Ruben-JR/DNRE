...
class AccountEdiFormat(models.Model):
    ...
    def _is_embedding_to_invoice_pdf_needed(self):
        ...
        return (
            True
            if self.code == "facturx_1_0_05"
            else super()._is_embedding_to_invoice_pdf_needed()
        )
    ...
    def _export_facturx(self, invoice):
        ...
        # Create file content.
        seller_siret = (
            "siret" in invoice.company_id._fields
            and invoice.company_id.siret
            or invoice.company_id.company_registry
        )
        buyer_siret = (
        ...
        tax_detail_vals = invoice._prepare_edi_tax_details(
            grouping_key_generator=lambda tax_values: {
                "unece_tax_category_code": tax_values[
                    "tax_id"
                ]._get_unece_category_code(
                    invoice.commercial_partner_id, invoice.company_id
                ),
                "amount": tax_values["tax_id"].amount,
                "amount_type": tax_values[
                    "tax_id"
                ].amount_type,  # We need to check the type
            }
        )
        ...
        xml_content += self.env.ref(
            "account_edi_facturx.account_invoice_facturx_export"
    ...
    def _import_facturx(self, tree, invoice):
        ...
        if invoice._context.get("default_journal_id"):
            ...
            default_move_type = (
                "out_invoice" if journal.type == "sale" else "in_invoice"
            )
        ...
        elements = tree.xpath(
            "//rsm:ExchangedDocument/ram:TypeCode", namespaces=tree.nsmap
        )
        ...
        if type_code == "381":
            default_move_type = (
                "out_refund" if default_move_type == "out_invoice" else "in_refund"
            )
            refund_sign = -1
        else:
        ...
        with Form(
            invoice.with_context(default_move_type=default_move_type)
        ) as invoice_form:
            partner_type = (
                invoice_form.journal_id.type == "purchase"
            ...
            # Reference.
            elements = tree.xpath(
                "//rsm:ExchangedDocument/ram:ID", namespaces=tree.nsmap
            )
            if elements:
            ...
            # Comment.
            elements = tree.xpath(
                "//ram:IncludedNote/ram:Content", namespaces=tree.nsmap
            )
            if elements:
            ...
            if currency_str:
                ...
                if currency and not currency.active:
                    ...
                    raise RedirectWarning(
                        error_msg, error_action, _("Display the currency")
                    )
            ...
            # Invoice lines.
            elements = tree.xpath(
                "//ram:IncludedSupplyChainTradeLineItem", namespaces=tree.nsmap
            )
            if elements:
                for element in elements:
                    with invoice_form.invoice_line_ids.new() as invoice_line_form:
                        ...
                        invoice_line_form.product_id = self._retrieve_product(
                            ...
                            barcode=_find_value(
                                ".//ram:SpecifiedTradeProduct/ram:GlobalID", element
                            ),
                        )
                            ...
                            if discount_elements:
                                ...
                                if discount_percent_elements:
                                    ...
                                    if net_price_elements:
                                        ...
                                        net_unit_price = (
                                            float(net_price_elements[0].text)
                                            / float(quantity_elements[0].text)
                                            if quantity_elements
                                            else float(net_price_elements[0].text)
                                        )
