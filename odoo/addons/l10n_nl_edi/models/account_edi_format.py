...
class AccountEdiFormat(models.Model):
    ...
    def _is_nlcius(self, filename, tree):
        ...
        return (
            ...
            and customization_id is not None
            and "nlcius" in customization_id.text
        )
    ...
    def _export_nlcius(self, invoice):
        ...
        xml_content += self.env.ref("l10n_nl_edi.export_nlcius_invoice")._render(
            self._get_nlcius_values(invoice)
        )
        vat = invoice.company_id.partner_id.commercial_partner_id.vat
        xml_name = "nlcius-{}{}{}.xml".format(
            vat or "",
            "-" if vat else "",
    ...
    def _check_move_configuration(self, invoice):
        ...
        if not supplier.street or not supplier.zip or not supplier.city:
            errors.append(
                _(
                    "The supplier's address must include street, zip and city (%s).",
                    supplier.display_name,
                )
            )
        if (
            supplier.country_code == "NL"
            and not supplier.l10n_nl_kvk
            and not supplier.l10n_nl_oin
        ):
            errors.append(
                _(
                    "The supplier %s must have a KvK-nummer or OIN.",
                    supplier.display_name,
                )
            )
        if not supplier.vat:
            errors.append(
                _("Please define a VAT number for '%s'.", supplier.display_name)
            )

        customer = invoice.commercial_partner_id
        if customer.country_code == "NL" and (
            not customer.street or not customer.zip or not customer.city
        ):
            errors.append(
                _(
                    "Customer's address must include street, zip and city (%s).",
                    customer.display_name,
                )
            )
        if (
            customer.country_code == "NL"
            and not customer.l10n_nl_kvk
            and not customer.l10n_nl_oin
        ):
            errors.append(
                _(
                    "The customer %s must have a KvK-nummer or OIN.",
                    customer.display_name,
                )
            )
        ...
        if not invoice.partner_bank_id:
            errors.append(
                _("The supplier %s must have a bank account.", supplier.display_name)
            )

        if invoice.invoice_line_ids.filtered(
            lambda l: not (l.product_id.name or l.name)
        ):
            errors.append(_("Each invoice line must have a product or a label."))

        if (
            invoice.invoice_line_ids.tax_ids.invoice_repartition_line_ids.filtered(
                lambda r: r.use_in_tax_closing
            )
            and not supplier.vat
        ):
            errors.append(
                _("When vat is present, the supplier must have a vat number.")
            )
