...
class StockPicking(models.Model):
    ...
    sale_id = fields.Many2one(
        related="group_id.sale_id", string="Sales Order", store=True, readonly=False
    )
    ...
    def _log_less_quantities_than_expected(self, moves):
        ...
        def _render_note_exception_quantity(moves_information):
            ...
            return self.env.ref("sale_stock.exception_on_picking")._render(
                values=values
            )

        documents = self._log_activity_get_documents(
            moves, "sale_line_id", "DOWN", _keys_in_sorted, _keys_in_groupby
        )
