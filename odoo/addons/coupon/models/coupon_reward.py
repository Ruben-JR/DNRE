...
class CouponReward(models.Model):
    ...
    reward_product_id = fields.Many2one(
        "product.product", string="Free Product", help="Reward Product"
    )
    ...
    def _check_discount_percentage(self):
        if self.filtered(
            lambda reward: reward.discount_type == "percentage"
            and (reward.discount_percentage < 0 or reward.discount_percentage > 100)
        ):
