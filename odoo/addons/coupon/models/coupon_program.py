...
class CouponProgram(models.Model):
    ...
    active = fields.Boolean(
        "Active",
        default=True,
        help="A program is available for the customers when active",
    )
    rule_id = fields.Many2one(
        "coupon.rule", string="Coupon Rule", ondelete="restrict", required=True
    )
    reward_id = fields.Many2one(
        "coupon.reward", string="Reward", ondelete="restrict", required=True, copy=False
    )
    ...
    promo_code_usage = fields.Selection(
        [("no_code_needed", "Automatically Applied"), ("code_needed", "Use a code")],
    ...
    promo_applicability = fields.Selection(
        ...
        default="on_current_order",
        string="Applicability",
    )
    coupon_ids = fields.One2many(
        "coupon.coupon", "program_id", string="Generated Coupons", copy=False
    )
    ...
    def _compute_coupon_count(self):
        coupon_data = self.env["coupon.coupon"].read_group(
    ...
    def toggle_active(self):
        ...
        coupons = self.filtered(
            lambda p: not p.active and p.promo_code_usage == "code_needed"
        ).mapped("coupon_ids")
    ...
    def _compute_program_amount(self, field, currency_to):
        ...
        return self.currency_id._convert(
            self[field], currency_to, self.company_id, fields.Date.today()
        )
