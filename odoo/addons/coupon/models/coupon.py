...
class Coupon(models.Model):
    ...
    code = fields.Char(
        default=lambda self: self._generate_code(), required=True, readonly=True
    )
    ...
    discount_line_product_id = fields.Many2one(
        "product.product",
        related="program_id.discount_line_product_id",
        readonly=False,
    ...
    def _compute_expiration_date(self):
        ...
        for coupon in self.filtered(lambda x: x.program_id.validity_duration > 0):
            coupon.expiration_date = (
                coupon.create_date
                + relativedelta(days=coupon.program_id.validity_duration)
            ).date()
    ...
    def _check_coupon_code(self, order_date, partner_id, **kwargs):
        ...
        if self.state == "used":
            ...
            message = {
                "error": _(
                    "This coupon %s exists but the origin sales order is not validated yet."
                )
                % (self.code)
            }
