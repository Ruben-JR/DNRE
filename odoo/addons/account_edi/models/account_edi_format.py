...
class AccountEdiFormat(models.Model):
    ...
    def _cancel_invoice_edi(self, invoices):
        ...
        * blocking_level:   (optional) How bad is the error (how should the edi flow be blocked ?)
        ...
        self.ensure_one()
        return {
            invoice: {"success": True} for invoice in invoices
        }  # By default, cancel succeeds doing nothing.
    ...
    def _cancel_payment_edi(self, payments):
        ...
        * blocking_level: (optional) How bad is the error (how should the edi flow be blocked ?)
        ...
        self.ensure_one()
        return {
            payment: {"success": True} for payment in payments
        }  # By default, cancel succeeds doing nothing.
        ...
        for file_data in self._decode_attachment(attachment):
            for edi_format in self:
                ...
                try:
                    ...
                    _logger.exception(
                        'Error importing attachment "%s" as invoice with format "%s"',
                        file_data["filename"],
                        edi_format.name,
                        str(e),
                    )
            ...
            if not partner:
                ...
                if vat_only_numeric:
                    ...
                    self._cr.execute(
                        ...
                        LIMIT 1
                    """,
                        where_params + [f"^{vat_prefix_regex}0*{vat_only_numeric}$"],
                    )
