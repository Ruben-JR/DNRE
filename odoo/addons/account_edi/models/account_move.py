...
class AccountMove(models.Model):
    ...
    def _compute_edi_show_abandon_cancel_button(self):
        for move in self:
            move.edi_show_abandon_cancel_button = any(
                doc.edi_format_id._needs_web_services()
    ...
    def _add_edi_tax_values(
        ...
        if serialized_grouping_key not in results["tax_details"]:
            ...
            tax_details = results["tax_details"][serialized_grouping_key]
            if key_by_tax:
                add_to_base_amount = key_by_tax[tax_values["tax_id"]] != key_by_tax.get(
                    tax_values["src_line_id"].tax_line_id
                )
            else:
                add_to_base_amount = tax_values["base_line_id"] not in {
                    x["base_line_id"] for x in tax_details["group_tax_details"]
                }
    ...
    def _prepare_edi_tax_details(
        ...
        def compute_invoice_lines_tax_values_dict_from_tax_details(invoice_lines):
            ...
            (
                tax_details_query,
                tax_details_params,
            ) = invoice_lines._get_query_tax_details_from_domain(
                [("move_id", "=", self.id)]
            )
            ...
            for row in self._cr.dictfetchall():
                ...
                src_tax = (
                    self.env["account.tax"].browse(row["group_tax_id"])
                    if row["group_tax_id"]
                    else tax
                )
        ...
        def compute_invoice_lines_tax_values_dict_from_compute_all(invoice_lines):
            ...
            for invoice_line in invoice_lines:
                taxes_res = invoice_line.tax_ids.compute_all(
                    ...
                    is_refund=invoice_line.move_id.move_type
                    ...
                    invoice_lines_tax_values_dict[invoice_line].append(
                        {
                            ...
                            ].browse(tax_res["tax_repartition_line_id"]),
                            "base_amount": sign
                            * invoice_line.company_currency_id.round(
                                tax_res["base"] * rate
                            ),
                            "tax_amount": sign
                            * invoice_line.company_currency_id.round(
        ...
        if compute_mode == "compute_all":
            invoice_lines_tax_values_dict = (
                compute_invoice_lines_tax_values_dict_from_compute_all(invoice_lines)
            )
        else:
            invoice_lines_tax_values_dict = (
                compute_invoice_lines_tax_values_dict_from_tax_details(invoice_lines)
            )
        ...
        for invoice_line in invoice_lines:
            ...
            for tax_values in tax_values_list:
                ...
                if not filter_to_apply or filter_to_apply(tax_values):
                    filtered_invoice_lines_tax_values_dict[invoice_line].append(
                        tax_values
                    )
            ...
            for zero_tax in taxes_set:
                ...
                for tax_rep in zero_tax[tax_rep_lines_field].filtered(
        ...
        for invoice_line in invoice_lines:
            ...
            for tax_values in tax_values_list:
                ...
                if (
                    serialized_grouping_key
                    not in invoice_global_tax_details["invoice_line_tax_details"][
                        invoice_line
                    ]
                ):
                    invoice_line_global_tax_details = invoice_global_tax_details[
    ...
    def _prepare_edi_vals_to_export(self):
        ...
        for index, line in enumerate(
            self.invoice_line_ids.filtered(lambda line: not line.display_type), start=1
        ):
