        ...
        for doc in self:
            ...
            if doc.state in ("to_send", "to_cancel"):
                ...
                if config_errors:
                    ...
                    res = base64.b64encode(
                        doc.edi_format_id._get_invoice_edi_content(doc.move_id)
                    )
                elif move.payment_id and doc.edi_format_id._is_required_for_payment(
                    move
                ):
                    res = base64.b64encode(
                        doc.edi_format_id._get_payment_edi_content(doc.move_id)
                    )
            doc.edi_content = res
        ...
        for key, documents in to_process.items():
            ...
            for doc in documents:
                if edi_format._support_batching(
                    move=doc.move_id, state=state, company=company_id
                ):
                    batch |= doc
                else:
