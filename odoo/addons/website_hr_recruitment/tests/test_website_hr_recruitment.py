...
class TestWebsiteHrRecruitmentForm(odoo.tests.HttpCase):
    def test_tour(self):
        ...
        # check result
        guru_applicant = self.env["hr.applicant"].search(
            [
                ("description", "=", "### [GURU] HR RECRUITMENT TEST DATA ###"),
                ("job_id", "=", job_guru.id),
            ]
        )
        ...
        internship_applicant = self.env["hr.applicant"].search(
            [
                ("description", "=", "### HR [INTERN] RECRUITMENT TEST DATA ###"),
