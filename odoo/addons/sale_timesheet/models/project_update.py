...
class ProjectUpdate(models.Model):
    ...
    def _get_services_values(self, project):
        ...
        for sol in sols:
            ...
            if (
                sol.product_uom.category_id
                == self.env.company.timesheet_encode_uom_id.category_id
                or is_unit
            ):
    ...
    def _get_profitability_values(self, project):
        ...
        return {
            ...
            "revenues": format_amount(
                self.env, profitability["revenues"], self.env.company.currency_id
            ),
            "margin": profitability["margin"],
            "margin_formatted": format_amount(
                self.env, profitability["margin"], self.env.company.currency_id
            ),
            "margin_percentage": formatLang(
                self.env,
                not float_utils.float_is_zero(
                    profitability["costs"], precision_digits=2
                )
