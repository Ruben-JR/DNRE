...
class Project(models.Model):
    ...
    def _compute_warning_employee_rate(self):
        ...
        employees = self.env["account.analytic.line"].read_group(
        ...
        action.update(
            {
                ...
                "views": [
                    ...
                    [
                        self.env.ref(
                            "hr_timesheet.view_kanban_account_analytic_line"
                        ).id,
                        "kanban",
                    ],
                    [
                        self.env.ref("hr_timesheet.view_hr_timesheet_line_pivot").id,
                        "pivot",
                    ],
                    [
                        self.env.ref(
    ...
    def _get_sold_items(self):
        ...
        for sol in sols:
            name = (
                [x[1] for x in sol.name_get()] if number_sale_orders > 1 else sol.name
            )
            ...
            if qty_delivered > 0 or product_uom_qty > 0:
                sold_items["data"].append(
                    {
                        ...
                        "value": "%s / %s %s"
                        % (
                            formatLang(self.env, qty_delivered, 1),
                            formatLang(self.env, product_uom_qty, 1),
                            product_uom_convert.name,
                        ),
                ...
                if sol.product_uom.category_id == timesheet_encode_uom.category_id or (
                    sol.product_uom == product_uom_unit
                    and sol.product_id.service_policy != "delivered_manual"
                ):
        ...
        if self.allow_billable:
            ...
            data += [
                ...
                },
                {
                    "name": _("Costs"),
                    "value": format_amount(
                        self.env, profitability["costs"], self.env.company.currency_id
                    ),
                },
                {
                    "name": _("Margin"),
                    "color": margin_color,
                    "value": format_amount(
                        self.env, profitability["margin"], self.env.company.currency_id
                    ),
                },
            ]
        return {
            "action": self.allow_billable
    ...
    def _get_profitability_common(self):
        self.ensure_one()
        result = {"costs": 0.0, "margin": 0.0, "revenues": 0.0}
        ...
        if profitability:
            ...
            result.update(
                {
                    "costs": profitability["timesheet_cost"]
                    + profitability["expense_cost"],
                    "margin": profitability["margin"],
                    "revenues": (
                        profitability["amount_untaxed_invoiced"]
                        + profitability["amount_untaxed_to_invoice"]
                        + profitability["expense_amount_untaxed_invoiced"]
                        + profitability["expense_amount_untaxed_to_invoice"]
            ...
            buttons.append(
                {
                    ...
                    "additional_context": json.dumps(
                        {"active_id": self.id, "default_project_id": self.id}
                    ),
