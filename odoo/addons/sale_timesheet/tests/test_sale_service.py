...
class TestSaleService(TestCommonSaleTimesheet):
    ...
    def test_sale_service(self):
        ...
        task = project.task_ids.filtered(
            lambda t: t.name
            == f"{self.sale_order.name}: {self.product_delivery_timesheet2.name}"
        )
        ...
        subtask = self.env["project.task"].create(
            {
                "parent_id": task.id,
                "project_id": project.id,
                "name": f"{task.name}: substask1",
            }
        )
        task2 = self.env["project.task"].create(
            {"project_id": project.id, "name": f"{task.name}: substask1"}
        )
    ...
    def test_add_product_analytic_account(self):
        ...
        self.assertEqual(
            ...
            self.project_global.analytic_account_id,
