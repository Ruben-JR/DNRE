...
class WebsiteEventController(http.Controller):
    ...
    def _create_attendees_from_registration_post(self, event, registration_data):
        ...
        for registration_values in registration_data:
            ...
            if visitor_sudo:
                ...
                if (
                    registration_values.get("name")
                    and not visitor_sudo.name
                    and not visitor_values.get("name")
                ):
    ...
    @http.route(
        "/event/add_event", type="json", auth="user", methods=["POST"], website=True
    )
    def add_event(self, name, event_start, event_end, address_values, **kwargs):
        values = self._prepare_event_values(
            name, event_start, event_end, address_values
        )
    ...
    def get_country_events(self, **post):
        ...
        if not events:
            events = Event.search(
                domain + [("date_begin", ">=", "%s 00:00:00" % fields.Date.today())],
                order="date_begin",
            )
        for event in events:
