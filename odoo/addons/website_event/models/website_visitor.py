...
class WebsiteVisitor(models.Model):
    ...
    parent_id = fields.Many2one(
        "website.visitor", string="Parent", ondelete="set null", help="Main identity"
    )
    ...
    def _compute_event_registered_ids(self):
        ...
        for visitor in self:
            all_registrations = (
                ...
                | visitor.parent_id.event_registration_ids
            )
    ...
    def _search_event_registered_ids(self, operator, operand):
        ...
        if all_registrations:
            ...
            visitors = all_registrations.with_context(active_test=False).mapped(
                "visitor_id"
            )
            children = (
                ...
                .sudo()
                .search([("parent_id", "in", visitors.ids)])
            )
    ...
    def _link_to_partner(self, partner, update_values=None):
        ...
        super()._link_to_partner(partner, update_values=update_values)
    ...
    def _link_to_visitor(self, target, keep_unique=True):
        ...
        res = super()._link_to_visitor(target, keep_unique=False)
    ...
    def _get_visitor_from_request(self, force_create=False):
        ...
        visitor = super()._get_visitor_from_request(force_create=force_create)
        ...
        if visitor and visitor.parent_id.partner_id:
            if self.env.user._is_public():
                ...
                visitor = (
                    ...
                    .sudo()
                    .with_context(active_test=False)
                    .search([("partner_id", "=", self.env.user.partner_id.id)])
                )
