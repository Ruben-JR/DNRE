...
class TestIsMultiLang(odoo.tests.HttpCase):
    ...
    def test_02_url_lang_code_underscore(self):
        ...
        self.assertTrue(
            r.url.endswith("/test_lang_url/my-super-country-%s" % country1.id)
        )
        ...
        self.assertTrue(
            r.url.endswith(
                "/%s/test_lang_url/my-super-country-italia-%s"
                % (it.url_code, country1.id)
            )
        )
        ...
        self.assertTrue(
            fr_href.endswith(
                "/%s/test_lang_url/my-super-country-belgium-%s"
                % (be.url_code, country1.id)
            )
        )
        self.assertTrue(
            en_href.endswith("/test_lang_url/my-super-country-%s" % country1.id)
        )
