...
class TestRedirect(HttpCase):
    def setUp(self):
        ...
        self.base_url = "http://{}:{}".format(HOST, odoo.tools.config["http_port"])
    ...
    def test_02_redirect_308_RequestUID(self):
        ...
        with patch.object(WebsiteHttp, "_get_error_html", _get_error_html):
            ...
            # published
            resp = self.url_open(
                "/test_website/200/name-%s" % rec_published.id, allow_redirects=False
            )
            self.assertEqual(resp.status_code, 308)
            self.assertEqual(
                resp.headers.get("Location"),
                self.base_url + "/test_website/308/name-%s" % rec_published.id,
            )

            resp = self.url_open(
                "/test_website/308/name-%s" % rec_published.id, allow_redirects=False
            )
            self.assertEqual(resp.status_code, 200)

            resp = self.url_open(
                "/test_website/200/xx-%s" % rec_published.id, allow_redirects=False
            )
            self.assertEqual(resp.status_code, 308)
            self.assertEqual(
                resp.headers.get("Location"),
                self.base_url + "/test_website/308/xx-%s" % rec_published.id,
            )

            resp = self.url_open(
                "/test_website/308/xx-%s" % rec_published.id, allow_redirects=False
            )
            ...
            resp = self.url_open(
                "/test_website/200/xx-%s" % rec_published.id, allow_redirects=True
            )
            self.assertEqual(resp.status_code, 200)
            self.assertEqual(
                resp.url, self.base_url + "/test_website/308/name-%s" % rec_published.id
            )
            ...
            self.assertEqual(
                resp.headers.get("Location"),
                self.base_url + "/test_website/308/name-100",
            )
            ...
            self.assertEqual(resp.status_code, 308)
            self.assertEqual(
                resp.headers.get("Location"), self.base_url + "/test_website/308/xx-100"
            )
            ...
            # unpublish
            resp = self.url_open(
                "/test_website/200/name-%s" % rec_unpublished.id, allow_redirects=False
            )
            self.assertEqual(resp.status_code, 308)
            self.assertEqual(
                resp.headers.get("Location"),
                self.base_url + "/test_website/308/name-%s" % rec_unpublished.id,
            )

            resp = self.url_open(
                "/test_website/308/name-%s" % rec_unpublished.id, allow_redirects=False
            )
            self.assertEqual(resp.status_code, 403)
            self.assertEqual(resp.text, "CUSTOM 403")

            resp = self.url_open(
                "/test_website/200/xx-%s" % rec_unpublished.id, allow_redirects=False
            )
            self.assertEqual(resp.status_code, 308)
            self.assertEqual(
                resp.headers.get("Location"),
                self.base_url + "/test_website/308/xx-%s" % rec_unpublished.id,
            )

            resp = self.url_open(
                "/test_website/308/xx-%s" % rec_unpublished.id, allow_redirects=False
            )
            self.assertEqual(resp.status_code, 403)
            self.assertEqual(resp.text, "CUSTOM 403")
            ...
            resp = self.url_open(
                "/test_website/200/seo-name-%s" % rec_published.id,
                allow_redirects=False,
            )
            self.assertEqual(resp.status_code, 308)
            self.assertEqual(
                resp.headers.get("Location"),
                self.base_url + "/test_website/308/seo-name-%s" % rec_published.id,
            )

            resp = self.url_open(
                "/test_website/308/seo-name-%s" % rec_published.id,
                allow_redirects=False,
            )
            self.assertEqual(resp.status_code, 200)
