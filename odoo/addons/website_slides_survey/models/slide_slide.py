...
class Slide(models.Model):
    ...
    _sql_constraints = [
        (
            "check_survey_id",
            "CHECK(slide_type != 'certification' OR survey_id IS NOT NULL)",
            "A slide of type 'certification' requires a certification.",
        ),
        (
            "check_certification_preview",
            "CHECK(slide_type != 'certification' OR is_preview = False)",
    ...
    def _ensure_challenge_category(self, old_surveys=None, unlink=False):
        ...
        if not unlink:
            certification_challenges = (
                self.mapped("survey_id").mapped("certification_badge_id").challenge_ids
            )
    ...
    def _generate_certification_url(self):
        ...
        for slide in self.filtered(
            lambda slide: slide.slide_type == "certification" and slide.survey_id
        ):
