...
class AccountChartTemplate(models.Model):
    ...
    def process_translations(self, langs, in_field, in_ids, out_ids):
        ...
        for lang in langs:
            ...
            value = xlat_obj._get_ids(
                in_ids._name + "," + in_field, "model", lang, in_ids.ids
            )
            counter = 0
...
class BaseLanguageInstall(models.TransientModel):
    ...
    def lang_install(self):
        self.ensure_one()
        already_installed = self.lang in [
        ...
        for coa in self.env["account.chart.template"].search(
            ...
            if self.lang in coa.spoken_languages.split(";"):
                ...
                for company in self.env["res.company"].search(
                    ...
                    # write account.group translations
                    coa._process_account_group_translations(
                        company.id, [self.lang], "name"
                    )
                    # copy account.tax name translations
                    coa._process_taxes_translations(company.id, [self.lang], "name")
                    # copy account.tax description translations
                    coa._process_taxes_translations(
                        company.id, [self.lang], "description"
                    )
                    # copy account.fiscal.position translations
                    coa._process_fiscal_pos_translations(
                        company.id, [self.lang], "name"
                    )
        return res
