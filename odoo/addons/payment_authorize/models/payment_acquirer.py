...
class PaymentAcquirer(models.Model):
    ...
    def _check_payment_method_type(self):
        for acquirer in self.filtered(lambda acq: acq.provider == "authorize"):
            if self.env["payment.token"].search(
                [("acquirer_id", "=", acquirer.id)], limit=1
            ):
    ...
    def action_update_merchant_details(self):
        ...
        if self.state == "disabled":
            raise UserError(
                _("This action cannot be performed while the acquirer is disabled.")
            )
    ...
    def _get_compatible_acquirers(self, *args, currency_id=None, **kwargs):
        ...
        if currency:
            acquirers = acquirers.filtered(
                lambda a: a.provider != "authorize"
                or currency == a.authorize_currency_id
            )
