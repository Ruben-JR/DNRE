...
class TestStockFlow(TestStockCommon):
    ...
    def test_00_picking_create_and_transfer_quantity(self):
        ...
        total_qty = sum(
            self.StockPackObj.search(
                [("move_id", "in", picking_in.move_lines.ids)]
            ).mapped("qty_done")
        )
        ...
        self.assertEqual(
            len(back_order_in.move_lines), 2, "Wrong number of move lines."
        )
        ...
        # Check total move lines of back order.
        self.assertEqual(
            len(back_order_out.move_lines), 2, "Wrong number of move lines"
        )
    ...
    def test_10_pickings_transfer_with_different_uom(self):
        ...
        self.StockPackObj.search(
            [("product_id", "=", self.kgB.id), ("picking_id", "=", picking_in_B.id)]
        ).write({"product_uom_qty": 0.020, "product_uom_id": self.uom_kg.id})
        self.StockPackObj.search(
            [("product_id", "=", self.gB.id), ("picking_id", "=", picking_in_B.id)]
        ).write({"product_uom_qty": 526, "product_uom_id": self.uom_gm.id})
        self.StockPackObj.search(
            [("product_id", "=", self.DozA.id), ("picking_id", "=", picking_in_B.id)]
        ).write({"product_uom_qty": 4, "product_uom_id": self.uom_dozen.id})
        self.StockPackObj.create(
            {
                "product_id": self.DozA.id,
                "product_uom_qty": 48,
        ...
        self.assertEqual(
            picking_in.state,
            "done",
            "Incoming shipment state: {} instead of {}".format(
                picking_in.state, "done"
            ),
        )
        # Check incoming shipment move lines state.
        for move in picking_in.move_lines:
    ...
    def test_transit_multi_companies_ultimate(self):
        ...
        self.assertEqual(incoming_picking.company_id, self.env.company)
        self.assertEqual(
            incoming_picking.move_lines.mapped("company_id"), self.env.company
        )
        self.assertEqual(outgoing_picking.company_id, company_2)
        self.assertEqual(outgoing_picking.move_lines.company_id, company_2)
    ...
    def test_receive_tracked_product(self):
        ...
        move_form = Form(
            receipt.move_lines, view="stock.view_stock_move_nosuggest_operations"
        )
