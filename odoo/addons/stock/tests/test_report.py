...
class TestReports(TestReportsCommon):
    ...
    def test_report_forecast_8_delivery_to_receipt_link(self):
        ...
        self.assertEqual(
            delivery2.move_lines.forecast_expected_date, receipt.move_lines.date
        )
    ...
    def test_report_forecast_9_delivery_to_receipt_link_over_received(self):
        ...
        self.assertEqual(delivery2.move_lines.forecast_availability, 200)
        self.assertEqual(
            delivery.move_lines.forecast_expected_date, receipt.move_lines.date
        )
        self.assertEqual(
            delivery2.move_lines.forecast_expected_date, receipt.move_lines.date
        )
    ...
    def test_report_forecast_11_non_reserved_order(self):
        ...
        self.assertEqual(
            all_delivery.move_lines.mapped("forecast_availability"), [0, 0, 0, 0]
        )
        ...
        self.assertEqual(
            all_delivery.move_lines.mapped("forecast_availability"), [3, 3, 0, 0]
        )
    ...
    def test_report_reception_1_one_receipt(self):
        ...
        self.assertEqual(
            len(receipt.move_lines[0].move_dest_ids.ids),
            2,
            "Demand qty of first and last moves should now be linked to incoming.",
        )
        self.assertEqual(
            len(receipt.move_lines[1].move_dest_ids.ids),
            1,
            "Demand qty of second move should now be linked to incoming.",
        )
        self.assertEqual(
            len(receipt.move_lines[2].move_dest_ids.ids),
            0,
            "product3 should have no moves linked to it.",
        )
        self.assertEqual(
            len(delivery1.move_lines.filtered(lambda m: m.product_id == product2)),
            2,
