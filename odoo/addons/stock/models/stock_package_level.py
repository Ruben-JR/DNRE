...
class StockPackageLevel(models.Model):
                    ...
                    for quant in package_level.package_id.quant_ids:
                        ...
                        if corresponding_mls:
                            for ml in corresponding_mls:
                                qty = (
                                    min(to_dispatch, ml.product_qty)
    ...
    def _check_move_lines_map_quant_package(self, package, field="qty_done"):
        ...
        grouped_quants = {}
        for k, g in groupby(
            sorted(package.quant_ids, key=sorted_key), key=itemgetter(*keys)
        ):
        ...
        grouped_ops = {}
        for k, g in groupby(
            sorted(pack_move_lines, key=sorted_key), key=itemgetter(*keys)
        ):
            grouped_ops[k] = sum(
                self.env["stock.move.line"].concat(*list(g)).mapped(field)
            )
        if any(
            grouped_quants.get(key, 0) - grouped_ops.get(key, 0) != 0
            for key in grouped_quants
        ) or any(
            grouped_ops.get(key, 0) - grouped_quants.get(key, 0) != 0
