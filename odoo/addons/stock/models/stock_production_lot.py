...
class ProductionLot(models.Model):
    ...
    def generate_lot_names(self, first_lot, count):
        ...
        for i in range(0, count):
            lot_names.append(
                "{}{}{}".format(prefix, str(initial_number + i).zfill(padding), suffix)
            )
        return lot_names
    ...
    def _get_next_serial(self, company, product):
        ...
        if product.tracking == "serial":
            ...
            if last_serial:
                return self.env["stock.production.lot"].generate_lot_names(
                    last_serial.name, 2
                )[1]
        return False
    ...
    def create(self, vals_list):
        self._check_create()
        return super(
            ProductionLot, self.with_context(mail_create_nosubscribe=True)
        ).create(vals_list)
        ...
        for lot in self:
            ...
            for line in move_lines.filtered(lambda ml: ml.lot_id.id == lot.id):
                if line.produce_line_ids:
                    ...
                    for (
                        delivery_ids_set
                    ) in line.produce_line_ids.lot_id._find_delivery_ids_by_lot(
