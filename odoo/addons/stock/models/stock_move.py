...
class StockMove(models.Model):
    ...
    move_line_nosuggest_ids = fields.One2many(
        "stock.move.line",
        "move_id",
        domain=["|", ("product_qty", "=", 0.0), ("qty_done", "!=", 0.0)],
    )
    ...
    def _compute_forecast_information(self):
        ...
        for move in product_moves:
            if (
                move.picking_type_id.code in self._consuming_picking_types()
    ...
    def _propagate_date_log_note(self, move_orig):
        ...
        msg = _(
            "The deadline has been automatically updated due to a delay on <a href='#' data-oe-model='%s' data-oe-id='%s'>%s</a>."
    ...
    def action_show_details(self):
        ...
        if self.product_id.tracking == "serial" and self.state == "assigned":
            self.next_serial = self.env["stock.production.lot"]._get_next_serial(
    ...
    def _set_quantity_done_prepare_vals(self, qty):
        ...
        for ml in self.move_line_ids:
            if float_is_zero(
                ml.product_uom_qty, precision_rounding=ml.product_uom_id.rounding
            ) and float_is_zero(
