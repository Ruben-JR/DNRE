...
class StockWarehouseOrderpoint(models.Model):
    ...
    qty_to_order = fields.Float(
        "To Order", compute="_compute_qty_to_order", store=True, readonly=False
    )
    ...
    _sql_constraints = [
        ...
        (
            ...
            "The combination of product and location must be unique.",
        ),
    ]
    ...
    def _get_orderpoint_action(self):
        ...
        for (product, warehouse) in to_refill.keys():
            product = (
                ...
                .browse(product)
                .with_prefetch(all_product_ids)
            )
            warehouse = (
                self.env["stock.warehouse"]
                .browse(warehouse)
                .with_prefetch(all_warehouse_ids)
            )
        ...
        dummy, qty_by_product_wh = (
            self.env["product.product"]
            .browse(product_ids)
            ._get_quantity_in_progress(warehouse_ids=warehouse_ids)
        )
        ...
        orderpoint_by_product_warehouse = {
            (record.get("product_id")[0], record.get("warehouse_id")[0]): record.get(
                "qty_to_order"
            )
        ...
        lot_stock_id_by_warehouse = {
            w["id"]: w["lot_stock_id"][0] for w in lot_stock_id_by_warehouse
        }
        ...
        for (product, warehouse), product_qty in to_refill.items():
            ...
            if orderpoint_id:
                ...
                orderpoint_values = self.env[
                    "stock.warehouse.orderpoint"
                ]._get_orderpoint_values(product, lot_stock_id)
        ...
        for orderpoint in orderpoints:
            ...
            orderpoint.route_id = next(
                (
                    r
                    for r in orderpoint.product_id.route_ids
                    if not r.supplied_wh_id or r.supplied_wh_id == orderpoint_wh
                ),
                orderpoint.route_id,
            )
