...
class StockMoveLine(models.Model):
    ...
    def _onchange_product_id(self):
        if self.product_id:
            ...
            if (
                ...
                or self.product_uom_id.category_id != self.product_id.uom_id.category_id
            ):
            ...
            if package.package_type_id:
                ...
                for sml in smls:
                    ...
                    sml.location_dest_id = sml.move_id.location_dest_id.with_context(
                        ...
                        sml.product_id, quantity=sml.product_uom_qty
                    )
                    excluded_smls -= sml
    ...
    def create(self, vals_list):
        ...
        for move_line in mls:
            ...
            if move_line.picking_id.state != "done":
                moves = move_line.picking_id.move_lines.filtered(
        ...
        for ml in mls_todo:
            if ml.product_id.type == "product":
                ...
                if (
                    ...
                    and float_compare(
                        ml.qty_done, ml.product_uom_qty, precision_rounding=rounding
                    )
                    > 0
                ):
    ...
    def _create_and_assign_production_lot(self):
        ...
        for key, mls in key_to_mls.items():
            mls._assign_production_lot(
                lots[key_to_index[key]].with_prefetch(lots._ids)
    ...
    def _log_message(self, record, move, template, vals):
        ...
        if "lot_id" in vals and vals["lot_id"] != move.lot_id.id:
            data["lot_name"] = (
                self.env["stock.production.lot"].browse(vals.get("lot_id")).name
            )
    ...
    def _free_reservation(
        ...
        if quantity > available_quantity:
            ...
            for candidate in outdated_candidates:
                if (
                    float_compare(
                        candidate.product_qty, quantity, precision_rounding=rounding
                    )
                    <= 0
                ):
