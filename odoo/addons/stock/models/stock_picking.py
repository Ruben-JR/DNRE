...
class PickingType(models.Model):
    ...
    warehouse_id = fields.Many2one(
        "stock.warehouse", "Warehouse", ondelete="cascade", check_company=True
    )
    ...
    def create(self, vals):
        ...
        picking_type = super().create(vals)
        return picking_type
...
class Picking(models.Model):
    ...
    product_id = fields.Many2one(
        "product.product", "Product", related="move_lines.product_id", readonly=True
    )
    ...
    def _compute_has_tracking(self):
        for picking in self:
            picking.has_tracking = any(
                m.has_tracking != "none" for m in picking.move_lines
            )
    ...
    def _compute_delay_alert_date(self):
        delay_alert_date_data = self.env["stock.move"].read_group(
            [("id", "in", self.move_lines.ids), ("delay_alert_date", "!=", False)],
    ...
    @api.depends(
        ...
        "scheduled_date",
        "move_lines",
        "move_lines.forecast_availability",
    ...
    def _compute_products_availability(self):
        ...
        for picking in pickings:
            ...
            if any(
                ...
                forecast_date = max(
                    picking.move_lines.filtered("forecast_expected_date").mapped(
            ...
            picking.json_popover = json.dumps(
                {
                    ...
                    "late_elements": [
                        ...
                        for late_move in picking.move_lines.filtered(
    ...
    def _compute_scheduled_date(self):
        for picking in self:
            moves_dates = picking.move_lines.filtered(
        ...
        for picking in self:
            if picking.move_type == "direct":
                picking.date_deadline = min(
                    picking.move_lines.filtered("date_deadline").mapped(
                        "date_deadline"
                    ),
                    default=False,
                )
    ...
    def _compute_has_packages(self):
        ...
        cnt_by_picking = self.env["stock.move.line"].read_group(
    ...
    def create(self, vals):
        ...
        if vals.get("partner_id"):
            for picking in res.filtered(
                ...
                or p.location_dest_id.usage == "customer"
            ):
    ...
    def write(self, vals):
        ...
        if after_vals:
            self.mapped("move_lines").filtered(lambda move: not move.scrapped).write(
                after_vals
            )
    ...
    def unlink(self):
        ...
        self.with_context(prefetch_fields=False).mapped(
            "move_lines"
        ).unlink()  # Checks if moves are not done
        return super().unlink()
    ...
    def action_assign(self):
        ...
        moves = (
            ...
            .filtered(lambda move: move.state not in ("draft", "cancel", "done"))
            .sorted(
                key=lambda move: (
                    -int(move.priority),
    ...
    def _action_done(self):
        ...
        ).move_lines.filtered(lambda m: m.state == "done")
    ...
    def _check_move_lines_map_quant_package(self, package):
        ...
        keys_ids = [f"{fname}.id" for fname in keys]
        ...
        grouped_quants = {}
        for k, g in groupby(
            sorted(package.quant_ids, key=attrgetter(*keys_ids)), key=itemgetter(*keys)
        ):
        ...
        grouped_ops = {}
        for k, g in groupby(
            sorted(pack_move_lines, key=attrgetter(*keys_ids)), key=itemgetter(*keys)
        ):
            grouped_ops[k] = sum(
    ...
    def _check_backorder(self):
        ...
        for picking in self:
            quantity_todo = {}
            quantity_done = {}
            for move in picking.mapped("move_lines").filtered(
    ...
    def _autoconfirm_picking(self):
        ...
        for picking in self:
            ...
            if picking.immediate_transfer or any(
                move.additional for move in picking.move_lines
            ):
        ...
        if stream == "DOWN":
            if sorted_method and groupby_method:
                grouped_moves = groupby(
                    sorted(origin_objects.mapped(stream_field), key=sorted_method),
                    key=groupby_method,
                )
            else:
                raise UserError(
    ...
    def _log_less_quantities_than_expected(self, moves):
        ...
        documents = self._log_activity_get_documents(
            moves, "move_dest_ids", "DOWN", _keys_in_sorted, _keys_in_groupby
        )
