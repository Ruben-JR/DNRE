...
class ResConfigSettings(models.TransientModel):
    ...
    group_stock_production_lot = fields.Boolean(
        "Lots & Serial Numbers", implied_group="stock.group_production_lot"
    )
    ...
    stock_mail_confirmation_template_id = fields.Many2one(
        related="company_id.stock_mail_confirmation_template_id", readonly=False
    )
    ...
    group_stock_auto_reception_report = fields.Boolean(
        "Show Reception Report at Validation",
        implied_group="stock.group_auto_reception_report",
    )
    ...
    def set_values(self):
        ...
        if (
            ...
            and location_grp in base_user.implied_ids
            and warehouse_grp in base_user.implied_ids
        ):
            raise UserError(
                _(
                    "You can't desactivate the multi-location if you have more than once warehouse by company"
                )
            )
        ...
        if (
            ...
            and storage_cate_grp not in base_user.implied_ids
        ):
            ...
            and storage_cate_grp in base_user.implied_ids
        ):
        ...
        if self.group_stock_multi_locations and not previous_group.get(
            ...
            warehouse_obj.with_context(active_test=True).search([]).mapped(
                "int_type_id"
            ).write({"active": True})
        ...
        if (
            ...
            picking_types = (
                ...
                .search([("code", "!=", "incoming"), ("show_operations", "=", False)])
            )
