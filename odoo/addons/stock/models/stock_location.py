...
class Location(models.Model):
    ...
    outgoing_move_line_ids = fields.One2many(
        "stock.move.line", "location_id", help="Technical: used to compute weight."
    )
    incoming_move_line_ids = fields.One2many(
        "stock.move.line", "location_dest_id", help="Technical: used to compute weight."
    )
    ...
    @api.depends(
        "outgoing_move_line_ids.product_qty",
        "incoming_move_line_ids.product_qty",
    ...
    def _get_putaway_strategy(
        ...
        putaway_rules |= self.putaway_rule_ids.filtered(
            ...
            and (
                package_type in x.package_type_ids or package_type == x.package_type_ids
            )
        )
        ...
        while categ:
            putaway_rules |= self.putaway_rule_ids.filtered(
                ...
                and (
                    package_type in x.package_type_ids
                    or package_type == x.package_type_ids
                )
            )
                    ...
                    for values in move_line_data:
                        ...
                        qty_done = sum(
                            ...
                            for qty_reserved, qty, ml_uom in zip(
                                values["product_qty"], values["qty_done"], list(uoms)
                            )
                        )
        ...
        if self.storage_category_id:
            ...
            if package and package.package_type_id:
                ...
                if self.storage_category_id.max_weight < self.forecast_weight + sum(
                    package_smls.mapped(
                        lambda sml: sml.product_qty * sml.product_id.weight
                    )
                ):
                    return False
                # check if enough space
