...
class Warehouse(models.Model):
    ...
    def _default_name(self):
        ...
        return (
            "{} - warehouse # {}".format(self.env.company.name, count + 1)
            if count
    ...
    crossdock_route_id = fields.Many2one(
        "stock.location.route", "Crossdock Route", ondelete="restrict"
    )
    reception_route_id = fields.Many2one(
        "stock.location.route", "Receipt Route", ondelete="restrict"
    )
    delivery_route_id = fields.Many2one(
        "stock.location.route", "Delivery Route", ondelete="restrict"
    )
    ...
    def _check_multiwarehouse_group(self):
        cnt_by_company = (
            self.env["stock.warehouse"]
            .sudo()
            .read_group([("active", "=", True)], ["company_id"], groupby=["company_id"])
        )
        if cnt_by_company:
    ...
    def _format_rulename(self, from_loc, dest_loc, suffix):
        rulename = f"{self.code}: {from_loc.name}"
        if dest_loc:
