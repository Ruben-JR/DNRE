...
class Product(models.Model):
    ...
    stock_quant_ids = fields.One2many(
        "stock.quant", "product_id", help="Technical: used to compute quantities."
    )
    stock_move_ids = fields.One2many(
        "stock.move", "product_id", help="Technical: used to compute quantities."
    )
    ...
    def _get_domain_locations(self):
        ...
        return self._get_domain_locations_new(
            location_ids, compute_child=self.env.context.get("compute_child", True)
        )
    ...
    def _get_domain_locations_new(
        ...
        if other_locations:
            ...
            dest_loc_domain = dest_loc_domain + [
                ("location_dest_id", operator, other_locations.ids)
            ]
        return (
            domain + loc_domain,
            domain + dest_loc_domain + ["!"] + loc_domain
            if loc_domain
            else domain + dest_loc_domain,
            domain + loc_domain + ["!"] + dest_loc_domain
            if dest_loc_domain
            else domain + loc_domain,
        )
    ...
    def _get_rules_from_location(self, location, route_ids=False, seen_rules=False):
        ...
        rule = self.env["procurement.group"]._get_rule(
            self,
            location,
            {"route_ids": route_ids, "warehouse_id": location.warehouse_id},
        )
        if not rule:
            return seen_rules
    ...
    def _filter_to_unlink(self):
        domain = [("product_id", "in", self.ids)]
        lines = self.env["stock.production.lot"].read_group(
...
class ProductTemplate(models.Model):
    ...
    has_available_route_ids = fields.Boolean(
        ...
        default=lambda self: self.env["stock.location.route"].search_count(
    ...
    def _compute_has_available_route_ids(self):
        self.has_available_route_ids = self.env["stock.location.route"].search_count(
            [("product_selectable", "=", True)]
        )
    ...
    def _compute_quantities_dict(self):
        variants_available = {
            p["id"]: p
            for p in self.product_variant_ids.read(
...
class ProductPackaging(models.Model):
    ...
    route_ids = fields.Many2many(
        "stock.location.route",
        "stock_location_route_packaging",
        "packaging_id",
        "route_id",
        "Routes",
