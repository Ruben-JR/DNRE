...
class GoogleDrive(models.Model):
    ...
    def get_access_token(self, scope=None):
        ...
        if not google_drive_refresh_token:
            if user_is_admin:
                action_id = self.env["ir.model.data"]._xmlid_lookup(
        ...
        try:
            req = requests.post(
                GOOGLE_TOKEN_ENDPOINT, data=data, headers=headers, timeout=TIMEOUT
            )
    ...
    def copy_doc(self, res_id, template_id, name_gdocs, res_model):
        ...
        request_url = (
            "https://www.googleapis.com/drive/v2/files/%s?fields=parents/id&access_token=%s"
        ...
        record_url = (
            "Click on link to open Record in Odoo\n %s/?db=%s#id=%s&model=%s"
        ...
        request_url = (
            "https://www.googleapis.com/drive/v2/files/%s/copy?access_token=%s"
            % (template_id, access_token)
        )
        headers = {"Content-type": "application/json", "Accept": "text/plain"}
        # resp, content = Http().request(request_url, "POST", data_json, headers)
        req = requests.post(
            request_url, data=json.dumps(data), headers=headers, timeout=TIMEOUT
        )
        req.raise_for_status()
        content = req.json()
        res = {}
        if content.get("alternateLink"):
            ...
            try:
                ...
                raise self.env["res.config.settings"].get_config_warning(
                    _(
                        "The permission 'reader' for 'anyone with the link' has not been written on the document"
                    )
                )
    ...
    def get_google_drive_config(self, res_model, res_id):
        ...
        for config in configs.sudo():
            if config.filter_id:
                if (
                    config.filter_id.user_id
                    and config.filter_id.user_id.id != self.env.user.id
                ):
                    # Private
                    continue
                try:
                    domain = [("id", "in", [res_id])] + ast.literal_eval(
                        config.filter_id.domain
                    )
                except:
                    raise UserError(
                        _(
                            "The document filter must not include any 'dynamic' part, so it should not be based on the current time or current user, for example."
                        )
                    )
                additionnal_context = ast.literal_eval(config.filter_id.context)
                google_doc_configs = (
                    self.env[config.filter_id.model_id]
    ...
    google_drive_resource_id = fields.Char(
            ...
            if drive.filter_id and drive.model_id.model != drive.filter_id.model_id:
                raise ValidationError(
                    _(
                        ...
                        drive=drive.name,
                        filter=drive.filter_id.name,
                        filter_model=drive.filter_id.model_id.model,
                        drive_model=drive.model_id.model,
                    )
                )
