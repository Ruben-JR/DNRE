...
class Lead(models.Model):
        ...
        for lead in self:
            ...
            if email_search:
                duplicate_lead_ids |= return_if_relevant(
                    "crm.lead",
                    common_lead_domain + [("email_from", "ilike", email_search)],
                )
    ...
    def copy(self, default=None):
        ...
        default = default or {}
        default["date_open"] = (
            fields.Datetime.now() if self.type == "opportunity" else False
        )
    ...
    def _fields_view_get(
        ...
        if self._context.get("opportunity_id"):
            ...
            if action.get("views") and any(
                view_id for view_id in action["views"] if view_id[1] == view_type
            ):
                view_id = next(
                    view_id[0] for view_id in action["views"] if view_id[1] == view_type
                )
    ...
    def get_empty_list_help(self, help):
        ...
        if alias_record and alias_record.alias_domain and alias_record.alias_name:
            ...
            email_link = f"<b><a href='mailto:{email}'>{email}</a></b>"
            sub_title = _(
        ...
        for lead in self:
            ...
            for field in _fields:
                ...
                if field.ttype == "selection":
                    ...
                    if value:
                        value = ",".join(val.display_name for val in value.sudo())
                body.append("{}: {}".format(field.field_description, value or ""))
    ...
    def _merge_notify(self, opportunities):
        ...
        self.ensure_one()
        merge_message = (
            _("Merged leads") if self.type == "lead" else _("Merged opportunities")
        )
    ...
    def _merge_get_fields(self):
        return list(CRM_LEAD_FIELDS_TO_MERGE) + list(
            self._merge_get_fields_specific().keys()
        )
    ...
    def _notify_get_reply_to(
        ...
        if leftover:
            res.update(
                super(Lead, leftover)._notify_get_reply_to(
            ...
            for lead in self:
                if lead.partner_id:
                    lead._message_add_suggested_recipient(
                        recipients, partner=lead.partner_id, reason=_("Customer")
                    )
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        if "company_id" not in defaults and "team_id" in defaults:
            defaults["company_id"] = (
                self.env["crm.team"].browse(defaults["team_id"]).company_id.id
            )
    ...
    def _pls_get_naive_bayes_probabilities(self, batch_mode=False):
        ...
        frequencies = self.env["crm.lead.scoring.frequency"].search(
            [("variable", "in", list(leads_fields))], order="team_id asc"
        )
        ...
        result = {
            team_id: {field: dict(won_total=0, lost_total=0) for field in leads_fields}
    ...
    def _pls_prepare_frequencies(
        ...
        stage_ids = self.env["crm.stage"].search_read(
            [], ["sequence", "name", "id"], order="sequence"
        )
