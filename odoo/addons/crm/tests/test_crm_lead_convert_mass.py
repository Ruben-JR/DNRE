...
class TestLeadConvertMass(crm_common.TestLeadConvertMassCommon):
    ...
    def test_assignment_salesmen(self):
        ...
        with self.assertQueryCount(
            user_sales_manager=254
        ):  # crm only: 251 - often 251, sometimes +3 on runbot
    ...
    def test_assignment_salesmen_wteam(self):
        ...
        with self.assertQueryCount(
            user_sales_manager=222
        ):  # crm only: 217 - generally 219, sometimes +2/+3 on runbot
    ...
    def test_mass_convert_performances(self):
        ...
        with self.assertQueryCount(
            user_sales_manager=1336
        ):  # still some randomness (1266 generally on runbot) - crm only: 1257
            mass_convert = (
                self.env["crm.lead2opportunity.partner.mass"]
                .with_context(
                    {
