...
class AccountAnalyticGroup(models.Model):
    ...
    children_ids = fields.One2many(
        "account.analytic.group", "parent_id", string="Childrens"
    )
...
class AccountAnalyticAccount(models.Model):
    ...
    def _compute_debit_credit_balance(self):
        ...
        if self._context.get("tag_ids"):
            tag_domain = expression.OR(
                [[("tag_ids", "in", [tag])] for tag in self._context["tag_ids"]]
            )
    ...
    name = fields.Char(
        string="Analytic Account", index=True, required=True, tracking=True
    )
    ...
    group_id = fields.Many2one(
        "account.analytic.group", string="Group", check_company=True
    )
...
class AccountAnalyticLine(models.Model):
    ...
    product_uom_category_id = fields.Many2one(
        related="product_uom_id.category_id", string="UoM Category", readonly=True
    )
    account_id = fields.Many2one(
