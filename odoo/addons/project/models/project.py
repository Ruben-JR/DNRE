...
class ProjectTaskType(models.Model):
    ...
    def unlink_wizard(self, stage_view=False):
        ...
        readgroup = (
            ...
            .env["project.task"]
            .read_group([("stage_id", "in", self.ids)], ["project_id"], ["project_id"])
        )
        project_ids = list(
            set(
...
class Project(models.Model):
    ...
    def _compute_task_count(self):
        task_data = self.env["project.task"].read_group(
            ...
            ["project_id", "display_project_id:count"],
            ["project_id"],
        )
    ...
    label_tasks = fields.Char(
        string="Use Tasks as",
        default="Tasks",
        help="Label used for the tasks of the project.",
        translate=True,
    )
    ...
    collaborator_count = fields.Integer(
        "# Collaborators", compute="_compute_collaborator_count"
    )
    ...
    def map_tasks(self, new_project_id):
        ...
        for task in all_tasks:
            ...
            new_child_ids = [
                old_to_new_tasks[child.id]
                for child in task.child_ids
                if child.id in old_to_new_tasks
            ]
            ...
            if task.allow_task_dependencies:
                depend_on_ids = [
                    t.id if t not in all_tasks else old_to_new_tasks.get(t.id, None)
                    for t in task.depend_on_ids
                ]
                new_task.write(
                    {
                        "depend_on_ids": [
                            Command.link(tid) for tid in depend_on_ids if tid
                        ]
                    }
                )
                dependent_ids = [
                    t.id if t not in all_tasks else old_to_new_tasks.get(t.id, None)
                    for t in task.dependent_ids
                ]
                new_task.write(
                    {
                        "dependent_ids": [
                            Command.link(tid) for tid in dependent_ids if tid
                        ]
                    }
                )
            tasks += new_task
    ...
    def action_unlink(self):
        wizard = self.env["project.delete.wizard"].create({"project_ids": self.ids})
    ...
    def _unlink_except_contains_tasks(self):
        ...
        for project in self.with_context(active_test=False):
            if project.tasks:
                raise UserError(
                    _(
                        "You cannot delete a project containing tasks. You can either archive it or first delete all of its tasks."
                    )
                )
    ...
    def action_view_tasks(self):
        action = (
            self.with_context(active_id=self.id, active_ids=self.ids)
    ...
    def _action_open_all_projects(self):
        action = self.env["ir.actions.act_window"]._for_xml_id(
            ...
            if not self.user_has_groups("project.group_project_stages")
            else "project.open_view_project_all_group_stage"
        )
        return action
            ...
            buttons.append(
                {
                    ...
                    and self.rating_percentage_satisfaction > -1,
                    "sequence": 5,
                }
            )
            ...
            buttons.append(
                {
                    ...
                    "number": format_amount(
                        self.env,
                        self.analytic_account_balance,
...
class Task(models.Model):
    ...
    def _get_default_stage_id(self):
        ...
        return self.stage_find(
            project_id, [("fold", "=", False), ("is_closed", "=", False)]
        )
    ...
    date_last_stage_update = fields.Datetime(
        string="Last Stage Update", index=True, copy=False, readonly=True
    )
    ...
    def _compute_recurring_count(self):
        ...
        count = self.env["project.task"].read_group(
    ...
    def _compute_attachment_ids(self):
        for task in self:
            attachment_ids = (
                ...
                .search([("res_id", "=", task.id), ("res_model", "=", "project.task")])
                .ids
            )
    ...
    def _ensure_fields_are_accessible(
        ...
        if (
            ...
            if unauthorized_fields:
                raise AccessError(
                    _(
                        "You cannot %s %s fields in task.",
                        operation if operation == "read" else "%s on" % operation,
        ...
        if (
            ...
            if parent_ids:
                ...
                child_subtype = self._track_subtype(
                    {col_name: initial_values[col_name] for col_name in changes}
                )
                child_subtype_info = (
    ...
    def _notify_get_reply_to(
        ...
        if leftover:
            res.update(
                super(Task, leftover)._notify_get_reply_to(
    ...
    def _notify_email_header_dict(self):
        headers = super()._notify_email_header_dict()
        if self.project_id:
            current_objects = [
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype_xmlid=None):
        return super().rating_apply(
            rate, token=token, feedback=feedback, subtype_xmlid="project.mt_task_rating"
        )
