...
class ProjectCustomerPortal(CustomerPortal):
    ...
    def _project_get_page_view_values(
        ...
        **kwargs,
        ...
        if group:
            grouped_tasks = [
                Task.concat(*g) for k, g in groupbyelem(tasks, itemgetter(group))
            ]
        else:
            grouped_tasks = [tasks]
    ...
    def portal_my_tasks(
        ...
        **kw,
        ...
        searchbar_sortings = dict(
            sorted(
                self._task_get_searchbar_sortings().items(),
        ...
        domain = AND(
            [domain, request.env["ir.rule"]._compute_domain(TaskSudo._name, "read")]
        )
