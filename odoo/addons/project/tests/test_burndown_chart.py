...
class TestBurndownChart(TransactionCase):
    ...
    def test_burndown_chart(self):
        ...
        self.set_create_date(
            "project_task", task_f.id, datetime(current_year - 1, 12, 20)
        )
        ...
        read_group_result = (
            self.env["project.task.burndown.chart.report"]
            .with_context(fill_temporal=True)
            .read_group(
                [("project_id", "=", project.id), ("display_project_id", "!=", False)],
                ["date", "stage_id", "nb_tasks"],
                ["date:month", "stage_id"],
                lazy=False,
            )
        )
        read_group_result_dict = {
            (res["date:month"], res["stage_id"][0]): res["nb_tasks"]
        ...
        for (month, stage_id), nb_tasks in read_group_result_dict.items():
            ...
            expected_nb_tasks = expected_dict.get(
                (month, stage_id), 5 if stage_id != todo_stage.id else 1
            )
            self.assertEqual(
                nb_tasks,
                expected_nb_tasks,
                "In %s, the number of tasks should be equal to %s in %s stage."
                % (month, expected_nb_tasks, stages_dict.get(stage_id, "Unknown")),
            )
