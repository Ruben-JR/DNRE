...
class Post(models.Model):
    ...
    child_ids = fields.One2many(
        ...
        "parent_id",
        string="Post Answers",
        domain=lambda self: [("forum_id", "=", self.forum_id.id)],
    )
    ...
    closed_uid = fields.Many2one(
        "res.users", string="Closed by", index=True, readonly=True, copy=False
    )
    ...
    def _get_mail_message_access(self, res_ids, operation, model_name=None):
        ...
        return super()._get_mail_message_access(
    ...
    def mark_as_offensive_batch(self, key, values):
        ...
        _logger.info(f"User {self.env.uid} marked as spams (in batch): {spams}")
    ...
    def get_access_action(self, access_uid=None):
        ...
        return {
            "type": "ir.actions.act_url",
            "url": f"/forum/{self.forum_id.id}/{self.id}",
            "target": "self",
            "target_type": "public",
            "res_id": self.id,
        }
