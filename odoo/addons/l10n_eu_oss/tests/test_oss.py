...
class TestOSSBelgium(AccountTestInvoicingCommon):
    ...
    def setUpClass(self, chart_template_ref="l10n_be.l10nbe_chart_template"):
        try:
            ...
            if (
                e.args[0]
                == "External ID not found in the system: l10n_be.l10nbe_chart_template"
            ):
                self.skipTest(
                    self,
                    reason="Belgian CoA is required for this testSuite but l10n_be isn't installed",
                )
            else:
                raise e
    ...
    def test_country_tag_from_belgium(self):
        ...
        for doc_type, report_line_xml_id in (
            ...
            with self.subTest(doc_type=doc_type, report_line_xml_id=report_line_xml_id):
                ...
                expected_tag_id = self.env.ref(report_line_xml_id).tag_ids.filtered(
                    lambda t: not t.tax_negate
                )
