...
class MailChannel(models.Model):
    ...
    livechat_operator_id = fields.Many2one(
        "res.partner", string="Operator", help="""Operator for this specific channel"""
    )
    ...
    def _channel_message_notifications(self, message, message_format=False):
        ...
        if not message.author_id:
            unpinned_channel_partner = self.channel_last_seen_partner_ids.filtered(
                lambda cp: not cp.is_pinned
            )
            if unpinned_channel_partner:
                unpinned_channel_partner.write({"is_pinned": True})
                notifications = (
                    self._channel_channel_notifications(
                        unpinned_channel_partner.mapped("partner_id").ids
                    )
                    + notifications
                )
        ...
        for channel in self:
            ...
            if channel.channel_type == "livechat":
                ...
                channel_infos_dict[channel.id][
                    "livechat_visitor"
                ] = channel._channel_get_livechat_visitor_info()
    ...
    def _channel_info_format_member(self, partner, partner_info):
        ...
        if self.channel_type == "livechat":
            return {
                ...
                "id": partner.id,
                "name": partner.user_livechat_username
                or partner.name,  # for API compatibility in stable
        ...
        return super()._channel_info_format_member(
            partner=partner, partner_info=partner_info
        )
    ...
    def _notify_typing_partner_data(self):
        ...
        if (
            ...
            and self.env.user.partner_id.user_livechat_username
        ):
            data[
                "partner_name"
            ] = (
                self.env.user.partner_id.user_livechat_username
            )  # for API compatibility in stable
    ...
    def _channel_get_livechat_visitor_info(self):
        ...
        if first_partner and (
            not first_partner.user_ids
            or not any(user._is_public() for user in first_partner.user_ids)
        ):
            # legit non-public partner
            return {
                "country": first_partner.country_id.name_get()[0]
                if first_partner.country_id
                else False,
                "id": first_partner.id,
