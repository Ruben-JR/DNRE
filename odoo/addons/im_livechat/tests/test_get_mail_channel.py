...
class TestGetMailChannel(TransactionCase):
    ...
    def test_channel_get_livechat_visitor_info(self):
        ...
        self.assertEqual(
            sorted(channel_info["members"], key=lambda m: m["id"]),
            sorted(
                [
                    {
                        "active": True,
        ...
        channel_info = self.livechat_channel.with_user(
            test_user
        )._open_livechat_mail_channel(anonymous_name="whatever", user_id=test_user.id)
        visitor_info = channel_info["livechat_visitor"]
    ...
    def test_channel_not_pinned_for_operator_before_first_message(self):
        ...
        operator_channel_partner = self.env["mail.channel.partner"].search(
            [
        ...
        self.assertEqual(
            len(operator_channel_partner), 1, "operator should be member of channel"
        )
        self.assertFalse(
            operator_channel_partner.is_pinned,
        ...
        self.assertIn(
            channel_info["id"],
            operator_channel_partner.partner_id._get_channels_as_member().ids,
