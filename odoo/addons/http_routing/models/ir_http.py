...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_resolve(cls):
        if "geoip" not in request.session:
            ...
            if odoo._geoip_resolver and request.httprequest.remote_addr:
                record = (
                    odoo._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
                )
    ...
    def _add_dispatch_parameters(cls, func):
        ...
        if request.routing_iteration == 1:
            ...
            nearest_lang = not func and cls.get_nearest_lang(
                Lang._lang_get_code(path[1])
            )
            ...
            if nearest_lang:
                ...
                nearest_ctx_lg = not is_a_bot and cls.get_nearest_lang(
    ...
    def _dispatch(cls):
        ...
        try:
            ...
            request.is_frontend = (
                ...
                or path_components[2] != "static"
                or "." not in path_components[-1]
            )
            routing_error = e

        request.is_frontend_multilang = not func or (
            func
        ...
        if request.is_frontend:
            ...
            if request.routing_iteration == 1:
                ...
                wrong_url_lg = url_lg and (
                    url_lg != request.lang.url_code or url_lg == default_lg_id.url_code
                )
                ...
                missing_url_lg = (
                    not url_lg
                    and request.is_frontend_multilang
                    and request.lang != default_lg_id
                ...
                if wrong_url_lg or (missing_url_lg and not is_a_bot):
                    ...
                    if request.httprequest.path == "/%s/" % url_lg:
                        ...
                        if request.httprequest.query_string:
                            path += "?" + request.httprequest.query_string.decode(
                                "utf-8"
                            )
        ...
        if (
            request.is_frontend
            and cook_lang != request.lang._get_cached("code")
