...
class EventRegistration(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", string="Booked by", states={"done": [("readonly", True)]}
    )
    name = fields.Char(
    ...
    def _onchange_partner_id(self):
        ...
        for registration in self:
            if registration.partner_id:
                registration.update(
                    registration._synchronize_partner_values(registration.partner_id)
                )
    ...
    def _check_ticket_seats_limit(self):
        for record in self:
            if (
                record.event_ticket_id.seats_max
                and record.event_ticket_id.seats_available < 0
            ):
    ...
    def write(self, vals):
        ...
        if vals.get("state") == "open":
            pre_draft = self.filtered(
                lambda registration: registration.state == "draft"
            )

        ret = super().write(vals)

        if vals.get("state") == "open" and not self.env.context.get(
            "install_mode", False
        ):
            # running the scheduler for demo data can cause an issue where wkhtmltopdf runs during
            # server start and hangs indefinitely, leading to serious crashes
        ...
        for registration in self:
            if registration.partner_id.name:
                if (
                    ...
                    name = f"{registration.partner_id.name}, {registration.name}"
                else:
