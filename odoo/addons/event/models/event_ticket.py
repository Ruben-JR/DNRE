...
class EventTicket(models.Model):
    ...
    seats_unconfirmed = fields.Integer(
        string="Unconfirmed Seats", compute="_compute_seats", store=True
    )
    ...
    def _compute_sale_available(self):
        for ticket in self:
            if (
                not ticket.is_launched()
                or ticket.is_expired
                or (ticket.seats_max and ticket.seats_available <= 0)
            ):
    ...
    def _constrains_dates_coherency(self):
        for ticket in self:
            if (
                ...
                raise UserError(
                    _("The stop date cannot be earlier than the start date.")
                )
