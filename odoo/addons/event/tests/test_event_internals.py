...
class TestEventData(TestEventCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.patcher = patch(
            "odoo.addons.event.models.event_event.fields.Datetime", wraps=FieldsDatetime
        )
    ...
    def test_event_date_computation(self):
        ...
        self.assertEqual(
            registration.get_date_range_str(), "on Mar 1, 2020, 11:00:00 AM"
        )
...
class TestEventTicketData(TestEventCommon):
    def setUp(self):
        super().setUp()
        self.ticket_date_patcher = patch(
            "odoo.addons.event.models.event_ticket.fields.Date", wraps=FieldsDate
        )
        ...
        self.ticket_datetime_patcher = patch(
            "odoo.addons.event.models.event_ticket.fields.Datetime",
            wraps=FieldsDatetime,
        )
        self.ticket_datetime_patcher_mock = self.ticket_datetime_patcher.start()
        self.ticket_datetime_patcher_mock.now.return_value = datetime(
            2020, 1, 31, 10, 0, 0
        )
