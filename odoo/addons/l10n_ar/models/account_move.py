...
class AccountMove(models.Model):
        ...
        for inv in self.filtered(lambda x: x.company_id.l10n_ar_company_requires_vat):
            ...
            for line in inv.mapped("invoice_line_ids").filtered(
                ...
                if len(vat_taxes) != 1:
                    raise UserError(
                        _(
                            'There must be one and only one VAT tax per line. Check line "%s"',
                            line.name,
                        )
                    )
                elif (
    ...
    def _get_last_sequence(self, relaxed=False, with_prefix=None):
        ...
        if (
            res
            and self.journal_id.l10n_ar_share_sequences
            and self.l10n_latam_document_type_id.doc_code_prefix not in res
        ):
            res = self._get_formatted_sequence(
                number=self._l10n_ar_get_document_number_parts(
                    res.split()[-1], self.l10n_latam_document_type_id.code
                )["invoice_number"]
            )
        return res
    ...
    def _get_last_sequence_domain(self, relaxed=False):
        ...
        if (
            ...
            if not self.journal_id.l10n_ar_share_sequences:
                ...
                param["l10n_latam_document_type_ids"] = tuple(
                    self.l10n_latam_document_type_id.search(
                        [
                            (
                                "l10n_ar_letter",
                                "=",
