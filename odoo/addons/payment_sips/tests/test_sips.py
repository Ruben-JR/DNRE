...
class SipsTest(SipsCommon):
    ...
    def test_redirect_form_values(self):
        ...
        self.patch(
            type(self.env["base"]), "get_base_url", lambda _: "http://localhost:8069"
        )
    ...
    def test_feedback_processing(self):
        ...
        self.assertEqual(
            tx.state, "done", "Sips: validation did not put tx into done state"
        )
        self.assertEqual(
            tx.acquirer_reference,
            self.reference,
            "Sips: validation did not update tx id",
        )
        ...
        self.assertEqual(
            tx2.state,
            "cancel",
            "Sips: erroneous validation did not put tx into error state",
        )
