...
class HrExpense(models.Model):
    ...
    approved_by = fields.Many2one(
        "res.users", string="Approved By", related="sheet_id.user_id"
    )
    ...
    label_total_amount_company = fields.Char(
    ...
    def _compute_label_total_amount_company(self):
        for expense in self:
            expense.label_total_amount_company = (
                _("Total %s", expense.company_currency_id.name)
    ...
    def create_expense_from_attachments(self, attachment_ids=None, view_type="tree"):
        ...
        if product:
            product = (
                product.filtered(lambda p: p.default_code == "EXP_GEN") or product[0]
            )
        else:
            raise UserError(
                _(
    ...
    def get_empty_list_help(self, help_message):
        return super().get_empty_list_help(
            help_message + self._get_empty_list_mail_alias()
        )
...
Or send your receipts at <a href="mailto:%(email)s?subject=Lunch%%20with%%20customer%%3A%%20%%2412.32">%(email)s</a>.
    ...
    def _prepare_move_values(self):
        ...
        self.ensure_one()
        journal = (
            self.sheet_id.bank_journal_id
            if self.payment_mode == "company_account"
            else self.sheet_id.journal_id
        )
    ...
    def _get_expense_account_source(self):
        ...
        if self.account_id:
            ...
            if not account:
                raise UserError(
                    _(
                        "Please configure Default Expense account for Category expense: `property_account_expense_categ_id`."
                    )
                )
        return account
