...
class TestExpenses(TestExpenseCommon):
    ...
    def test_account_entry_multi_currency(self):
        ...
        self.assertTrue(
            expense.account_move_id.id, "Expense Journal Entry is not created"
        )
        ...
        for line in expense.account_move_id.line_ids:
            if line.credit:
                ...
                self.assertEqual(
                    len(line.analytic_line_ids),
                    0,
                    "The credit move line should not have analytic lines",
                )
                self.assertFalse(
                    line.product_id, "Product of credit move line should be false"
                )
            else:
                if not line.tax_line_id == tax:
                    ...
                    self.assertEqual(
                        len(line.analytic_line_ids),
                        1,
                        "The debit move line should have 1 analytic lines",
                    )
                    self.assertEqual(
                        line.product_id,
                        self.product_a,
                        "Product of debit move line should be the one from the expense",
                    )
                else:
                    ...
                    self.assertEqual(
                        len(line.analytic_line_ids),
                        0,
                        "The tax move line should not have analytic lines",
                    )
                    self.assertFalse(
                        line.product_id, "Product of tax move line should be false"
                    )
    ...
    def test_reconcile_payment(self):
        ...
        self.assertEqual(
            sheet.state,
            "cancel",
            "Sheet state must be cancel when the payment linked to that sheet is canceled",
        )
