...
class WebsiteEventBoothController(WebsiteEventController):
    ...
    def event_booth_registration_confirm(
        ...
        return request.redirect(
            ("/event/%s/booth/success?" % event.id)
            + werkzeug.urls.url_encode(
                {
    ...
    @http.route(
        '/event/<model("event.event"):event>/booth/success',
        type="http",
        auth="public",
        methods=["GET"],
        website=True,
    ...
    def event_booth_registration_complete(self, event, booths):
        ...
        return request.render(
            "website_event_booth.event_booth_registration_complete",
            {
                ...
                "contact_email": booth_ids[0].contact_email
                or booth_ids.partner_id.email,
                "contact_mobile": booth_ids[0].contact_mobile
                or booth_ids.partner_id.mobile,
                "contact_phone": booth_ids[0].contact_phone
                or booth_ids.partner_id.phone,
            },
        )
