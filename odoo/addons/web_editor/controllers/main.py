...
class Web_Editor(http.Controller):
    ...
    def update_checklist(
        ...
        if not li or not self._update_checklist_recursive(
            li, checked, children=True, ancestors=True
        ):
            return value
    ...
    def _update_checklist_recursive(self, li, checked, children=False, ancestors=False):
        ...
        if ancestors:
            ...
            for child in ul.getchildren():
                if (
                    child.tag == "li"
                    and "checklist-id" in child.get("id", "")
                    and "o_checked" not in child.get("class", "")
                ):
                    allSelected = False
    ...
    def get_image_info(self, src=""):
        ...
        if id_match:
            ...
            if "." in url_segment:  # xml-id
                attachment = request.env["ir.http"]._xmlid_to_obj(
                    request.env, url_segment
                )
    ...
    def _load_resources(
        ...
        for bundle_data in files_data_by_bundle:
            for i in range(0, len(bundle_data[1])):
                ...
                content = AssetsUtils.get_asset_content(
    ...
    @http.route(
        "/web_editor/public_render_template", type="json", auth="public", website=True
    )
    def public_render_template(self, args):
        ...
        len_args = len(args)
        assert (
            len_args >= 1 and len_args <= 2
        ), "Need a xmlID and potential rendering values to render a template"
        ...
        return View.render_public_asset(
            xmlid, {k: values[k] for k in values if k in trusted_value_keys}
        )
    ...
    def save_library_media(self, media):
        ...
        for id, url in response.json().items():
            ...
            if media[id]["is_dynamic_svg"]:
                ...
                attachment["url"] = "/web_editor/shape/illustration/{}?{}".format(
                    slug(attachment),
                    colorParams,
                )
