...
class TestWebsiteSaleProductAttributeValueConfig(TestSaleProductAttributeValueCommon):
    def test_get_combination_info(self):
        ...
        self.assertEqual(
            pricelist.currency_id.compare_amounts(
                combination_info["price_extra"], 222 * currency_ratio * tax_ratio
            ),
            0,
        )
    ...
    def test_get_combination_info_with_fpos(self):
        ...
        self.assertEqual(
            combination_info["price"],
            500,
            "500% + 0% tax (mapped from fp 15% -> 0% for BE)",
        )
        self.assertEqual(
            combination_info["list_price"],
            500,
            "500% + 0% tax (mapped from fp 15% -> 0% for BE) (2)",
        )
        self.assertEqual(
            combination_info["price_extra"],
            200,
            "200% + 0% tax (mapped from fp 15% -> 0% for BE)",
        )
        ...
        self.assertEqual(
            ...
            434.78,
            "434.78$ + 0% tax (mapped from fp 15% -> 0% for BE)",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            combination_info["price_extra"],
            173.91,
            "173.91$ + 0% tax (mapped from fp 15% -> 0% for BE)",
        )
...
class TestWebsiteSaleProductPricelist(TestSaleProductAttributeValueCommon):
    def test_cart_update_with_fpos(self):
        ...
        with MockRequest(self.env, website=current_website, sale_order_id=so.id):
            so._cart_update(
                product_id=test_product.product_variant_id.id, line_id=sol.id, set_qty=1
            )
        self.assertEqual(
