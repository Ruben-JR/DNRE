    ...
    def _website_product_id_change(self, order_id, product_id, qty=0, **kwargs):
        ...
        if order.pricelist_id.discount_policy == "without_discount":
            ...
            price, rule_id = order.pricelist_id.with_context(
                product_context
            ).get_product_price_rule(product, qty or 1.0, order.partner_id)
            pu, currency = (
            ...
            if order.pricelist_id and order.partner_id:
                ...
                if order_line:
                    price = self.env["account.tax"]._fix_tax_included_price_company(
                        price, product.taxes_id, order_line[0].tax_id, self.company_id
                    )
            ...
            if pu != 0:
                if order.pricelist_id.currency_id != currency:
                    ...
                    pu = currency._convert(
                        pu, order.pricelist_id.currency_id, order.company_id, date
                    )
    ...
    def _cart_update(
        ...
        if not order_line:
            ...
            values = self._website_product_id_change(
                self.id, product_id, qty=1, **kwargs
            )
        ...
        if quantity <= 0:
            ...
            )._website_product_id_change(self.id, product_id, qty=quantity, **kwargs)
...
class SaleOrderLine(models.Model):
    ...
    def get_sale_order_line_multiline_description_sale(self, product):
        description = super().get_sale_order_line_multiline_description_sale(product)
