...
class Website(models.Model):
    ...
    def _compute_pricelist_id(self):
        for website in self:
            website.pricelist_id = website.with_context(
        ...
        if country_code:
            for cgroup in self.env["res.country.group"].search(
                ...
                pricelists |= cgroup.pricelist_ids.filtered(
                    lambda pl: pl._is_available_on_website(self.id)
                    and _check_show_visible(pl)
                )
        ...
        if not is_public:
            ...
            if country_code:
                ...
                partner_pl = partner_pl.filtered(
                    lambda pl: pl.country_group_ids
                    and country_code in pl.country_group_ids.mapped("country_ids.code")
                    or not pl.country_group_ids
                )
    ...
    def _get_pricelist_available(self, req, show_visible=False):
        ...
        isocountry = (
            req and req.session.geoip and req.session.geoip.get("country_code") or False
        )
    ...
    def _prepare_sale_order_values(self, partner, pricelist):
        ...
        values = {
            ...
            or partner.parent_id.team_id.id
            or partner.team_id.id,
    ...
    def sale_get_order(
        ...
        if check_fpos and sale_order:
            fpos_id = (
                ...
                .get_fiscal_position(
                    ...
                    delivery_id=sale_order.partner_shipping_id.id,
                )
            ).id
        ...
        if self.env["product.pricelist"].browse(force_pricelist).exists():
            ...
            pricelist_id = (
                ...
                or self.get_current_pricelist().id
            )
        ...
        if (
            sale_order.partner_id.id != partner.id
            and request.website.partner_id.id != partner.id
        ):
            ...
            if (
                flag_pricelist or recent_fiscal_position != fiscal_position
        ...
        if code and code != sale_order.pricelist_id.code:
            ...
            code is not None
            and sale_order.pricelist_id.code
            and code != sale_order.pricelist_id.code
        ):
