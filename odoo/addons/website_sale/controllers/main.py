...
class Website(main.Website):
    ...
    def get_switchable_related_views(self, key):
        ...
        if key == "website_sale.product":
            if not request.env.user.has_group("product.group_product_variant"):
                view_product_variants = request.website.viewref(
    ...
    def toggle_switchable_view(self, view_key):
        super().toggle_switchable_view(view_key)
        if view_key in (
...
class WebsiteSale(http.Controller):
    ...
    def pricelist_change(self, pl_id, **post):
        ...
        if (
            pl_id.selectable
            or pl_id == request.env.user.partner_id.property_product_pricelist
    ...
    def pricelist(self, promo, **post):
        ...
        if promo:
            ...
            if not pricelist or (
                pricelist and not request.website.is_pricelist_available(pricelist.id)
            ):
    ...
    def cart_update_json(
        ...
        value = order._cart_update(
            ...
            add_qty=add_qty,
            set_qty=set_qty,
            product_custom_attribute_values=json_scriptsafe.loads(pcav)
            if pcav
            else None,
    ...
    def _get_shop_payment_values(self, order, **kwargs):
        ...
        show_tokenize_input = logged_in and not request.env[
            "payment.acquirer"
        ].sudo()._is_tokenization_required(sale_order_id=order.id)
        return {
            "website_sale_order": order,
            "errors": [],
    ...
    @http.route(
        ["/shop/add_product"], type="json", auth="user", methods=["POST"], website=True
    )
    ...
    def products_recently_viewed_update(self, product_id, **kwargs):
        ...
        if visitor_sudo:
            if (
                request.httprequest.cookies.get("visitor_uuid", "")
