...
class TestStockValuationWithCOA(AccountTestInvoicingCommon):
    ...
    def test_fifo_anglosaxon_return(self):
        ...
        self.product1.property_account_creditor_price_difference = (
            self.price_diff_account
        )
    ...
    def test_realtime_anglo_saxon_valuation_multicurrency_different_dates(self):
        ...
        price_diff_line = product_lines.filtered(
    ...
    def test_average_realtime_with_delivery_anglo_saxon_valuation_multicurrency_different_dates(
        ...
        stock_line = product_lines.filtered(
            lambda l: l.account_id == self.stock_input_account
        )
        ...
        stock_journal_line = reconciled_lines.filtered(
            lambda l: l.journal_id == self.stock_journal
        )
        ...
        exchange_stock_line = reconciled_lines.filtered(
            lambda l: l.journal_id == exhange_diff_journal
        )
    ...
    def test_average_realtime_with_two_delivery_anglo_saxon_valuation_multicurrency_different_dates(
        ...
        (
            backorder_picking.move_lines.filtered(
                lambda l: l.purchase_line_id == line_product_avg
            ).write({"quantity_done": 5.0})
        )
        ...
        # DELIVERY DIFFERENCE (AVERAGE)
        stock_lines = move_lines.filtered(
            lambda l: l.account_id == self.stock_input_account
        )
        ...
        price_diff_line = move_lines.filtered(
            lambda l: l.account_id == self.price_diff_account
        )
        ...
        exchange_stock_line = reconciled_lines.filtered(
            lambda l: l.journal_id == exchange_diff_journal
        )
        self.assertEqual(exchange_stock_line.amount_currency, 0.00)
        ...
        exchange_stock_lines = reconciled_lines.filtered(
            lambda l: l.journal_id == exchange_diff_journal
        )
        self.assertAlmostEqual(
            sum(exchange_stock_lines.mapped("amount_currency")), 0.00
        )
    ...
    def test_anglosaxon_valuation_price_total_diff_discount(self):
        ...
        self.assertAlmostEqual(
            price_diff_aml.credit, 20, "Price difference should be equal to 20 (110-90)"
        )
    ...
    def test_anglosaxon_valuation_discount(self):
        ...
        self.assertAlmostEqual(
            price_diff_aml.credit, 10, "Price difference should be equal to 10 (100-90)"
        )
    ...
    def test_anglosaxon_valuation_price_unit_diff_avco(self):
        ...
        self.product1.categ_id.property_cost_method = "average"
        self.product1.categ_id.property_valuation = "real_time"
        self.product1.categ_id.property_account_creditor_price_difference_categ = (
            self.price_diff_account
        )
