...
class StockRule(models.Model):
    ...
    def _get_lead_days(self, product, **values):
        ...
        if not bypass_delay_description:
            delay_description.append(
                (_("Days to Purchase"), _("+ %d day(s)", days_to_purchase))
            )
        return (
            delay + supplier_delay + security_delay + days_to_purchase,
            delay_description,
        )
    ...
    def _get_procurements_to_merge(self, procurements):
        ...
        for k, procurements in groupby(
            sorted(procurements, key=self._get_procurements_to_merge_sorted),
