...
class TestOutOfOffice(TestHrHolidaysCommon):
    ...
    def test_leave_ooo(self):
        ...
        self.assertEqual(
            ...
            2,
            "Channel info should get info for the 2 members",
        )
        partner_info = next(
            c for c in channel_info["members"] if c["email"] == partner.email
        )
        partner2_info = next(
            c for c in channel_info["members"] if c["email"] == partner2.email
        )
        self.assertFalse(
            partner2_info["out_of_office_date_end"],
