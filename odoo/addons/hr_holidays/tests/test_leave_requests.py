...
class TestLeaveRequests(TestHrHolidaysCommon):
            ...
            self.env["hr.leave"].with_user(self.user_employee_id).create(
                {
                    ...
                    "date_from": (datetime.today() + relativedelta(days=1)).strftime(
                        "%Y-%m-%d %H:%M"
                    ),
    ...
    def test_several_allocations(self):
        ...
        self.assertEqual(
            allocation1.leaves_taken,
            0.0,
            "As this allocation does not have enough days, it should not be affected",
        )
        self.assertEqual(
            allocation2.leaves_taken,
            0.0,
            "As this allocation does not have enough days, it should not be affected",
        )
        self.assertEqual(
            allocation3.leaves_taken,
            10.0,
            "As this allocation has enough days, the leave days should be taken",
        )
