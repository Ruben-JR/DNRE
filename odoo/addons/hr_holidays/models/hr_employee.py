...
class HrEmployeeBase(models.AbstractModel):
    ...
    allocation_used_count = fields.Float(
        "Total number of days off used", compute="_compute_total_allocation_used"
    )
    ...
    def _compute_total_allocation_used(self):
        for employee in self:
            employee.allocation_used_count = float_round(
                employee.allocation_count - employee.remaining_leaves,
                precision_digits=2,
            )
    ...
    def _compute_presence_icon(self):
        ...
        employees_absent = self.filtered(
            lambda employee: employee.hr_icon_display
            not in ["presence_present", "presence_absent_active"]
            and employee.is_absent
        )
        employees_absent.update({"hr_icon_display": "presence_holiday_absent"})
        employees_present = self.filtered(
            lambda employee: employee.hr_icon_display
            in ["presence_present", "presence_absent_active"]
            and employee.is_absent
        )
