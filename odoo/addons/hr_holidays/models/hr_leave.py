...
class HolidaysRequest(models.Model):
    ...
    def _default_get_request_parameters(self, values):
        ...
        if values.get("date_from"):
            ...
            localized_dt = (
                timezone("UTC")
                .localize(values["date_from"])
                .astimezone(timezone(user_tz))
            )
            global_from = (
                localized_dt.time().hour == 7 and localized_dt.time().minute == 0
            )
        ...
        if values.get("date_to"):
            ...
            localized_dt = (
                timezone("UTC")
                .localize(values["date_to"])
                .astimezone(timezone(user_tz))
            )
            global_to = (
                localized_dt.time().hour == 19 and localized_dt.time().minute == 0
            )
    ...
    request_unit_custom = fields.Boolean(
        "Days-long custom hours",
        compute="_compute_request_unit_custom",
        store=True,
        readonly=False,
    )
    # view
    ...
    def _check_allocation_duration(self):
        for holiday in self:
            if (
                holiday.holiday_status_id.requires_allocation == "yes"
                and holiday.holiday_allocation_id
                and holiday.number_of_days
                > holiday.holiday_allocation_id.number_of_days
            ):
    ...
    def _compute_from_holiday_status_id(self):
        invalid_self = self.filtered(
            lambda leave: not leave.date_to or not leave.date_from
        )
        if invalid_self:
        ...
        for allocation in allocations:
            allocation["taken_leaves"] = (
                self.env["hr.leave"]
                .browse(allocation.pop("taken_leave_ids"))
                .filtered(
                    lambda leave: leave.state in ["confirm", "validate", "validate1"]
                )
            )
            allocations_dict[
                (allocation["holiday_status_id"][0], allocation["employee_id"][0])
            ].append(allocation)
        ...
        for leave in self:
            if (
                ...
                leave_unit = "number_of_%s_display" % (
                    "hours" if leave.leave_type_request_unit == "hour" else "days"
                )
                for allocation in allocations_dict[
                    (leave.holiday_status_id.id, leave.employee_id.id)
                ]:
                    date_to_check = (
                        allocation["date_to"] >= date_to
                    ...
                    if date_to_check and date_from_check:
                        ...
                        allocation_taken_number_of_units = sum(
                            allocation_taken_leaves.mapped(leave_unit)
                        )
                        leave_number_of_units = leave[leave_unit]
                        if (
                            allocation["max_leaves"]
                            >= allocation_taken_number_of_units + leave_number_of_units
                        ):
                            found_allocation = allocation["id"]
                            break
                leave.holiday_allocation_id = (
                    self.env["hr.leave.allocation"].browse(found_allocation)
                    if found_allocation
                    else False
                )
            else:
        ...
        for holiday in self:
            ...
            if not holiday.request_date_from:
                ...
                if resource_calendar_id.two_weeks_calendar:
                    ...
                    ].get_week_type(holiday.request_date_from)
                    ...
                    attendance_filtred = [
                        ...
                        if int(att.dayofweek) >= holiday.request_date_from.weekday()
                    ]
                    ...
                    end_week_type = self.env[
                        "resource.calendar.attendance"
                    ].get_week_type(holiday.request_date_to)
                    attendance_actual_week = [
                        att
                        for att in attendances
                    ...
                    attendance_filtred_reversed = list(
                        reversed(
                            [
                                att
                                for att in attendance_actual_week
                                if int(att.dayofweek)
                                <= holiday.request_date_to.weekday()
                            ]
                        )
                    )
                    attendance_filtred_reversed += list(
                    ...
                    attendance_to = next(
                        (
                            ...
                            if int(att.dayofweek) <= holiday.request_date_to.weekday()
                        ),
                ...
                if holiday.request_unit_half:
                    ...
                    compensated_request_date_from = holiday._adjust_date_based_on_tz(
                        holiday.request_date_from, hour_from
                    )
                    compensated_request_date_to = holiday._adjust_date_based_on_tz(
                        holiday.request_date_to, hour_to
                    )
                else:
                    hour_from = float_to_time(attendance_from.hour_from)
                    hour_to = float_to_time(attendance_to.hour_to)
                ...
                holiday.date_from = (
                    timezone(holiday.tz)
                    .localize(
                        datetime.combine(compensated_request_date_from, hour_from)
                    )
                    .astimezone(UTC)
                    .replace(tzinfo=None)
                )
                holiday.date_to = (
                    timezone(holiday.tz)
                    .localize(datetime.combine(compensated_request_date_to, hour_to))
                    .astimezone(UTC)
                    .replace(tzinfo=None)
                )
    ...
    def _compute_request_unit_half(self):
        for holiday in self:
            if (
                ...
                or holiday.request_unit_hours
                or holiday.request_unit_custom
            ):
    ...
    def _compute_from_employee_id(self):
        for holiday in self:
            ...
            if (
                ...
                and self._origin.employee_id != holiday.employee_id
            ):
    ...
    def _compute_duration_display(self):
        for leave in self:
            leave.duration_display = "{:g} {}".format(
                (
    ...
    def _compute_number_of_hours_text(self):
        ...
        for leave in self:
            leave.number_of_hours_text = "{}{:g} {}{}".format(
    ...
    def _check_holidays(self):
        for holiday in self:
            if (
                ...
                or not holiday.holiday_status_id
                or holiday.holiday_status_id.requires_allocation == "no"
            ):
                continue
            mapped_days = holiday.holiday_status_id.get_employees_days(
                [holiday.employee_id.id], holiday.date_from
            )
    ...
    def _check_allocation_id(self):
        for leave in self:
            if (
                ...
                and not leave.multi_employee
                and leave.holiday_status_id.requires_allocation == "yes"
                and not leave.holiday_allocation_id
            ):
                raise ValidationError(
                    _(
    ...
    def create(self, vals_list):
        ...
        holidays.filtered(lambda holiday: not holiday.holiday_allocation_id).with_user(
            SUPERUSER_ID
        )._compute_from_holiday_status_id()
