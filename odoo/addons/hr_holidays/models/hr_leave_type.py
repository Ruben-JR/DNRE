...
class HolidaysType(models.Model):
    ...
    def get_employees_days(self, employee_ids, date=None):
        ...
        for request in requests:
            ...
            if (
                not request.holiday_allocation_id
                or request.holiday_allocation_id in allocations
            ):
            ...
            if request.holiday_status_id.requires_allocation == "no":
                status_dict["virtual_leaves_taken"] += (
                    request.number_of_hours_display
                    if request.leave_type_request_unit == "hour"
                    else request.number_of_days
                )
    ...
    def _compute_accrual_count(self):
        accrual_allocations = self.env["hr.leave.accrual.plan"].read_group(
