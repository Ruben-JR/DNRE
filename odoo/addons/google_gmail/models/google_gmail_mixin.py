...
class GoogleGmailMixin(models.AbstractModel):
    ...
    def _compute_gmail_uri(self):
        ...
        if not google_gmail_client_id or not google_gmail_client_secret:
            ...
            google_gmail_uri = self.env["google.service"]._get_google_token_uri(
                "gmail", scope=self._SERVICE_SCOPE
            )
    ...
    def create(self, values):
        if values.get("google_gmail_authorization_code"):
            ...
            values["google_gmail_refresh_token"] = self.env[
                "google.service"
            ].generate_refresh_token("gmail", values["google_gmail_authorization_code"])
    ...
    def write(self, values):
        ...
        if authorization_code and not all(
            authorization_code == code
            for code in self.mapped("google_gmail_authorization_code")
        ):
            # Update the refresh token
            values["google_gmail_refresh_token"] = self.env[
                "google.service"
            ].generate_refresh_token("gmail", authorization_code)
            values["google_gmail_access_token"] = False
            values["google_gmail_access_token_expiration"] = False
    ...
    def _generate_oauth2_string(self, user, refresh_token):
        ...
        if (
            ...
            self.google_gmail_access_token, expires_in = self.env[
                "google.service"
            ]._get_access_token(refresh_token, "gmail", self._SERVICE_SCOPE)
            self.google_gmail_access_token_expiration = now_timestamp + expires_in

            _logger.info(
                "Google Gmail: fetch new access token. Expire in %i minutes",
                expires_in // 60,
            )
        else:
            _logger.info(
        ...
        return f"user={user}\1auth=Bearer {self.google_gmail_access_token}\1\1"
