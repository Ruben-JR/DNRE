...
class StripeController(http.Controller):
    ...
    def stripe_return_from_checkout(self, **data):
        ...
        tx_sudo = (
            ...
            .sudo()
            ._get_tx_from_feedback_data("stripe", data)
        )
    ...
    def stripe_return_from_validation(self, **data):
        ...
        self._include_setup_intent_in_feedback_data(
    ...
    def stripe_webhook(self):
        ...
        try:
                ...
                if self._verify_webhook_signature(
                    ...
                    if checkout_session.get("payment_intent"):  # Can be None
                        payment_intent = tx_sudo.acquirer_id._stripe_make_request(
                            f"payment_intents/{tx_sudo.stripe_payment_intent}",
                            method="GET",
                        )
                        _logger.info(
                            "received payment_intents response:\n%s",
                        ...
                        self._include_payment_intent_in_feedback_data(
                            payment_intent, data
                        )
            ...
            _logger.exception(
                "unable to handle the event data; skipping to acknowledge"
            )
        return ""
    ...
    def _verify_webhook_signature(self, webhook_secret):
        ...
        if datetime.utcnow().timestamp() - event_timestamp > self.WEBHOOK_AGE_TOLERANCE:
            _logger.warning(
                "ignored webhook event due to age tolerance: %s", event_timestamp
            )
            return False
