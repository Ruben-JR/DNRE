...
class PaymentTransaction(models.Model):
    ...
    def _send_payment_request(self):
        ...
        StripeController._include_payment_intent_in_feedback_data(
            payment_intent, feedback_data
        )
        _logger.info(
    ...
    def _process_feedback_data(self, data):
        ...
        if intent_status in INTENT_STATUS_MAPPING["draft"]:
            ...
            self._set_error(
                "Stripe: "
                + _("Received data with invalid intent status: %s", intent_status)
            )
    ...
    def _stripe_tokenize_from_feedback_data(self, data):
        ...
        if not payment_method_id or not payment_method:
            _logger.warning(
                "requested tokenization with payment method missing from feedback data"
            )
            return
