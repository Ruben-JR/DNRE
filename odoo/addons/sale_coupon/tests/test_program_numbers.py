...
class TestSaleCouponProgramNumbers(TestSaleCouponCommon):
    ...
    def test_program_numbers_free_and_paid_product_qty(self):
        ...
        order.recompute_coupon_lines()
        self.assertEqual(
            order.order_line.filtered(lambda x: x.is_reward_line).product_uom_qty,
            25,
    ...
    def test_program_numbers_one_discount_line_per_tax(self):
        ...
        self.assertEqual(
            order.amount_total, 1619, "The order total with programs should be 1619"
        )
        self.assertEqual(
            order.amount_untaxed,
            1355.46,
        ...
        self.assertEqual(
            len(order.order_line.ids),
            10,
            "Order should contains 7 lines: 4 products lines,"
    ...
    def test_program_fixed_price(self):
        ...
        self.assertEqual(
            len(sol2.tax_id.ids), 1, "One tax should be present on the reward line"
        )
        self.assertEqual(
            sol2.tax_id.id, self.tax_0pc_excl.id, "The tax should be 0% Tax excl"
        )
    ...
    def test_program_next_order(self):
        ...
        self.assertEqual(
            len(order.generated_coupon_ids),
            0,
        ...
        self.assertEqual(
            generated_coupon.state, "reserved", "The coupon should be reserved"
        )
    ...
    def test_program_percentage_discount_on_product_included_tax(self):
        ...
        self.assertEqual(
            order.amount_total, 0.0, "Order should be 0 as it is a 100% discount"
        )
    ...
    def test_program_numbers_free_prod_with_min_amount_and_qty_on_same_prod(self):
        ...
        self.assertEqual(
            order.amount_total,
            self.conferenceChair.lst_price * (sol1.product_uom_qty - 1)
    ...
    def test_program_free_product_different_than_rule_product_with_multiple_application(
        ...
        self.assertEqual(
            order.order_line.filtered(lambda x: x.is_reward_line).product_uom_qty,
            1,
            "Only one free Large Meeting Table should be offered, as only one paid Large Meeting Table is in cart. You can't have more free product than paid product.",
        )
        ...
        self.assertEqual(
            order.order_line.filtered(lambda x: x.is_reward_line).product_uom_qty,
            2,
            "The 2 Large Meeting Table should be offered, as the promotion says 1 Black Drawer = 1 free Large Meeting Table and there are 2 Black Drawer",
        )
    ...
    def test_program_maximum_use_number_last_order(self):
        ...
        self.assertDictEqual(
            self.env["sale.coupon.apply.code"].sudo().apply_coupon(order, "promo1"),
            {"error": "Promo code promo1 has been expired."},
        )
    ...
    def test_order_promo(self):
        ...
        self.assertEqual(
            order.amount_total,
            3,
            "Price should be 10$ - 2$(20% of 10$) - 5$(flat discount) = 3$",
        )
    ...
    def test_fixed_amount_change_promo_amount(self):
        ...
        self.assertEqual(
            len(order.order_line), 1, "Promotion line should not be present"
        )
