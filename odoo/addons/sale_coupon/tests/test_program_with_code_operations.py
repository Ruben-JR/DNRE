...
class TestProgramWithCodeOperations(TestSaleCouponCommon):
    ...
    def test_program_basic_operation_coupon_code(self):
        ...
        with self.assertRaises(UserError):
            self.env["sale.coupon.apply.code"].with_context(
                active_id=wrong_partner_order.id
            ).create({"coupon_code": coupon.code}).process_coupon()
    ...
    def test_apply_program_no_reward_link(self):
        ...
        self.assertFalse(
            self.empty_order.code_promo_program_id,
            "The program should not be linked to the order",
        )
        ...
        self.assertFalse(
            self.empty_order.applied_coupon_ids,
            "No coupon should be linked to the order",
        )
