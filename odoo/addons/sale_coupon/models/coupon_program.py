...
class CouponProgram(models.Model):
    ...
    def action_view_sales_orders(self):
        ...
        orders = (
            self.env["sale.order.line"]
            .search([("product_id", "=", self.discount_line_product_id.id)])
            .mapped("order_id")
        )
        return {
            "name": _("Sales Orders"),
    ...
    def _check_promo_code(self, order, coupon_code):
        message = {}
        if (
            self.maximum_use_number != 0
            and self.total_order_count >= self.maximum_use_number
        ):
            ...
            message = {
                "error": _("The promotional offer is already applied on this order")
            }
            ...
            message = {
                "error": _("This coupon is not yet usable. It will be starting from %s")
                % (
                    format_datetime(
                        self.rule_date_from,
                        format="short",
            ...
            self._is_global_discount_program()
            ...
            self.promo_applicability == "on_current_order"
            and not self.env.context.get("applicable_coupon")
        ):
    ...
    def _filter_on_validity_dates(self, order):
        return self.filtered(
            lambda program: (
                not program.rule_date_from
                or program.rule_date_from <= fields.Datetime.now()
            )
            and (
                not program.rule_date_to
                or program.rule_date_to >= fields.Datetime.now()
            )
        )
    ...
    def _filter_promo_programs_with_code(self, order):
        ...
        return self.filtered(
            lambda program: program.promo_code_usage == "code_needed"
            and program.promo_code != order.promo_code
        )
    ...
    def _filter_programs_on_partners(self, order):
        return self.filtered(
            lambda program: program._is_valid_partner(order.partner_id)
        )
    ...
    def _filter_not_ordered_reward_programs(self, order):
        ...
        for program in self:
            if (
                ...
                and program.reward_product_id not in order_products
            ):
                continue
            elif (
                ...
                and not any(
                    discount_product in order_products
                    for discount_product in program.discount_specific_product_ids
                )
            ):
                continue
            programs += program
