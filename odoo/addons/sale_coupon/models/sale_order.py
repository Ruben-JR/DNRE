...
class SaleOrder(models.Model):
    ...
    applied_coupon_ids = fields.One2many(
        "coupon.coupon", "sales_order_id", string="Applied Coupons", copy=False
    )
    generated_coupon_ids = fields.One2many(
        "coupon.coupon", "order_id", string="Offered Coupons", copy=False
    )
    ...
    no_code_promo_program_ids = fields.Many2many(
        ...
        domain="[('promo_code_usage', '=', 'no_code_needed'), '|', ('company_id', '=', False), ('company_id', '=', company_id)]",
        copy=False,
    )
    ...
    promo_code = fields.Char(
        related="code_promo_program_id.promo_code",
    ...
    def _get_paid_order_lines(self):
        ...
        return self.order_line.filtered(
            lambda x: not x._is_not_sellable_line()
    ...
    def _get_base_order_lines(self, program):
        ...
        return self.order_line.filtered(
            lambda x: not x._is_not_sellable_line()
            or (x.is_reward_line and x.product_id != program.discount_line_product_id)
        )
    ...
    def _get_max_reward_values_per_tax(self, program, taxes):
        lines = self.order_line.filtered(
            lambda l: l.tax_id == taxes
            and l.product_id != program.discount_line_product_id
        )
    ...
    def _get_reward_values_fixed_amount(self, program):
        ...
        if program.discount_line_product_id.taxes_id:
            line_taxes = (
                self.fiscal_position_id.map_tax(
                    program.discount_line_product_id.taxes_id
                )
                if self.fiscal_position_id
                else program.discount_line_product_id.taxes_id
            )
            return [
                {
        ...
        tax_groups = {line.tax_id for line in lines}
        max_discount_per_tax_groups = {
            tax_ids: self._get_max_reward_values_per_tax(program, tax_ids)
            for tax_ids in tax_groups
        }

        for tax_ids in sorted(
            tax_groups,
            key=lambda tax_ids: max_discount_per_tax_groups[tax_ids],
            reverse=True,
        ):
            ...
            lines_price = sum(
                curr_lines.mapped(lambda l: l.price_reduce * l.product_uom_qty)
            )
            lines_total = sum(curr_lines.mapped("price_total"))

            discount_line_amount_price = min(
                max_discount_per_tax_groups[tax_ids],
    ...
    def _get_reward_values_percentage_amount(self, program):
        ...
        fixed_price_products = (
            self._get_applied_programs()
            .filtered(lambda p: p.discount_type == "fixed_amount")
            .mapped("discount_line_product_id")
        )
        self.order_line.filtered(lambda l: l.product_id in fixed_price_products).write(
            {"price_unit": 0}
        )
        ...
        programs = (
            ...
            .search(
                [
                    ...
                    ("rule_date_from", "<=", fields.Datetime.now()),
                    "|",
                    ("rule_date_to", "=", False),
    ...
    def _update_existing_reward_lines(self):
        ...
        for program in applied_programs.sorted(
            lambda ap: (
                ap.discount_type == "fixed_amount",
                ap.discount_apply_on == "on_order",
            )
        ):
            ...
            if program.reward_type == "discount":
                ...
                for value in values:
                    ...
                    for line in lines:
                        # Case 1.
                        if not len(
                            set(line.tax_id.mapped("id")).symmetric_difference(
                                {v[1] for v in value["tax_id"]}
                            )
                        ):
                            value_found = True
                            # Working on Case 3.
    ...
    def _remove_invalid_reward_lines(self):
        ...
        if applied_programs:
            ...
            applicable_programs = (
                applicable_programs._keep_only_most_interesting_auto_applied_global_discount_program()
            )
        ...
        invalid_lines = order.order_line.filtered(
            ...
            and line.product_id.id not in reward_product_ids
        )
        ...
        if programs_to_remove:
            ...
            if product_ids_to_remove:
                ...
                self.generated_coupon_ids.filtered(
                    lambda coupon: coupon.program_id.discount_line_product_id.id
                    in product_ids_to_remove
                ).write({"state": "expired"})
    ...
    def _get_applied_programs_with_rewards_on_next_order(self):
        return self.no_code_promo_program_ids.filtered(
...
class SaleOrderLine(models.Model):
    ...
    def unlink(self):
        ...
        for line in self.filtered(lambda line: line.is_reward_line):
            ...
            related_program = self.env["coupon.program"].search(
    ...
    def modified(self, fnames, *args, **kwargs):
        super().modified(fnames, *args, **kwargs)
                    ...
                    if program.discount_line_product_id in products:
                        self.env.cache.invalidate(
                            [
                                ...
                                (field_total_order_count, program.ids),
                            ]
                        )
