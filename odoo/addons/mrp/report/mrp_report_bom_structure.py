...
class ReportBomStructure(models.AbstractModel):
    ...
    def get_operations(self, product_id=False, bom_id=False, qty=0, level=0):
        ...
        lines = self._get_operation_line(
            ...
            bom,
            float_round(
                qty / bom.product_qty, precision_rounding=1, rounding_method="UP"
            ),
            level,
        )
        values = {
    ...
    def _get_bom(
        ...
        byproducts, byproduct_cost_portion = self._get_byproducts_lines(
            bom, bom_quantity, level, lines["total"]
        )
        lines["byproducts"] = byproducts
        lines["cost_share"] = float_round(
    ...
    def _get_operation_line(self, product, bom, qty, level):
        ...
        for operation in bom.operation_ids:
            ...
            operation_cycle = float_round(
                qty / operation.workcenter_id.capacity,
    ...
    def _get_sub_lines(
        self, bom, product_id, line_qty, line_id, level, child_bom_ids, unfolded
    ):
        data = self._get_bom(
            bom_id=bom.id,
            product_id=product_id,
    ...
    def _get_pdf_line(
        ...
        pdf_lines = self._get_sub_lines(
            bom, product_id, qty, False, 1, child_bom_ids, unfolded
        )
        data["components"] = []
