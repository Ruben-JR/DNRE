...
class ProductTemplate(models.Model):
    ...
    mrp_product_qty = fields.Float(
        "Manufactured", compute="_compute_mrp_product_qty", compute_sudo=False
    )
...
class ProductProduct(models.Model):
    ...
    def _compute_quantities_dict(
        ...
        res = super(ProductProduct, self - kits)._compute_quantities_dict(
    ...
    def _match_all_variant_values(self, product_template_attribute_value_ids):
        ...
        if not product_template_attribute_value_ids:
            return True
        for _, iter_ptav in groupby(
            product_template_attribute_value_ids.sorted("attribute_line_id"),
            lambda ptav: ptav.attribute_line_id,
        ):
            if not any(
                ptav in self.product_template_attribute_value_ids for ptav in iter_ptav
            ):
                return False
        return True
