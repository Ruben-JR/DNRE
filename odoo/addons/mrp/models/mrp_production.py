...
class MrpProduction(models.Model):
    ...
    date_start = fields.Datetime(
        "Start Date", copy=False, readonly=True, help="Date of the WO"
    )
    date_finished = fields.Datetime(
        "End Date", copy=False, readonly=True, help="Date when the MO has been close"
    )
    ...
    orderpoint_id = fields.Many2one(
        "stock.warehouse.orderpoint", "Orderpoint", index=True, copy=False
    )
    ...
    show_serial_mass_produce = fields.Boolean(
        "Display the serial mass product wizard action moves",
    ...
    def _compute_mrp_production_child_count(self):
        for production in self:
            production.mrp_production_child_count = len(
                production.procurement_group_id.stock_move_ids.created_production_id.procurement_group_id.mrp_production_ids
                - production
            )
    ...
    def _compute_mrp_production_source_count(self):
        for production in self:
            production.mrp_production_source_count = len(
                production.procurement_group_id.mrp_production_ids.move_dest_ids.group_id.mrp_production_ids
                - production
            )
    ...
    def _compute_delay_alert_date(self):
        delay_alert_date_data = self.env["stock.move"].read_group(
    ...
    def _compute_show_lot_ids(self):
        for order in self:
            order.show_lot_ids = order.state != "draft" and any(
                m.product_id.tracking == "serial" for m in order.move_raw_ids
            )
        ...
        for order in self:
            if (
                ...
                (
                    ...
                    dummy,
                    dummy,
                ) = order._check_serial_mass_produce_components()
    ...
    def _onchange_company_id(self):
        if self.company_id:
            ...
            if (
                ...
                and self.picking_type_id.company_id != self.company_id
            ):
                self.picking_type_id = (
                    self.env["stock.picking.type"]
                    .search(
        ...
        for workorder in self.workorder_ids:
            ...
            if workorder.date_planned_start and workorder.duration_expected:
                workorder.date_planned_finished = (
                    ...
                    + relativedelta(minutes=workorder.duration_expected)
                )
    ...
    def _onchange_bom_id(self):
        ...
        self.product_uom_id = (
            self.bom_id and self.bom_id.product_uom_id.id or self.product_id.uom_id.id
        )
        self.move_raw_ids = [
    ...
    def _onchange_move_finished(self):
        if self.product_id and self.product_qty > 0:
            ...
            self.move_finished_ids = [
                (2, move.id)
                for move in self.move_finished_ids.filtered(lambda m: m.bom_line_id)
            ]
    ...
    def _onchange_picking_type(self):
        if (
            not self.picking_type_id.default_location_src_id
            or not self.picking_type_id.default_location_dest_id.id
        ):
            company_id = (
                self.company_id.id
                if (self.company_id and self.company_id in self.env.companies)
        ...
        self.location_src_id = (
            self.picking_type_id.default_location_src_id.id or fallback_loc.id
        )
        self.location_dest_id = (
            self.picking_type_id.default_location_dest_id.id or fallback_loc.id
        )
    ...
    def _onchange_lot_producing(self):
        if self.product_id.tracking == "serial" and self.lot_producing_id:
            message, dummy = self.env["stock.quant"]._check_serial_number(
                self.product_id, self.lot_producing_id, self.company_id
            )
            if message:
    ...
    def write(self, vals):
        ...
        for production in self:
            ...
            if (
                ...
                if production.state == "done":
                    ...
                    production.move_raw_ids.filtered(
                        lambda move: move.additional
                        and move.date > production.date_planned_start
                    ).write(
                        {
                    ...
                    production.move_finished_ids.filtered(
                        lambda move: move.additional
                        and move.date > production.date_planned_finished
                    ).write(
                        {
                            "reference": production.name,
                            "date": production.date_planned_finished,
            ...
            if (
                not production.workorder_ids.operation_id
    ...
    def create(self, values):
        ...
        if values.get("move_finished_ids", False):
            values["move_finished_ids"] = list(
                filter(
                    lambda move: move[2]["byproduct_id"] is False,
                    values["move_finished_ids"],
                )
            )
        if values.get("move_byproduct_ids", False):
            values["move_finished_ids"] = (
                values.get("move_finished_ids", []) + values["move_byproduct_ids"]
            )
            del values["move_byproduct_ids"]
        if not values.get("name", False) or values["name"] == _("New"):
            picking_type_id = (
                values.get("picking_type_id") or self._get_default_picking_type()
            )
        ...
        (production.move_raw_ids | production.move_finished_ids).write(
            {"group_id": production.procurement_group_id.id, "origin": production.name}
        )
    ...
    def action_view_mrp_production_sources(self):
        self.ensure_one()
        mrp_production_ids = (
            self.procurement_group_id.mrp_production_ids.move_dest_ids.group_id.mrp_production_ids.ids
        )
        action = {
            "res_model": "mrp.production",
            "type": "ir.actions.act_window",
        ...
        self.lot_producing_id = self.env["stock.production.lot"].create(
            {
                ...
                "company_id": self.company_id.id,
                "name": self.env["stock.production.lot"]._get_next_serial(
    ...
    def _plan_workorders(self, replan=False):
        ...
        if replan:
            workorder_ids = self.workorder_ids.filtered(
                lambda wo: wo.state in ("pending", "waiting", "ready")
            )
    ...
    def _generate_backorder_productions(self, close_mo=True):
        ...
        for wo in wo_to_update:
            wo.state = (
                "ready"
                if wo.next_work_order_id.production_availability == "assigned"
                else "waiting"
            )
    ...
    def _split_productions(
        ...
        for production in self:
            ...
            elif total_amount > production.product_qty or production.state in [
                "done",
                "cancel",
            ]:
                raise UserError(
    ...
    def action_serial_mass_produce_wizard(self):
        ...
        ) = self._check_serial_mass_produce_components()
        message = ""
        if missing_components:
            message += _(
                "Make sure enough quantities of these components are reserved to carry on production:\n"
            )
        ...
        if multiple_lot_components:
            if message:
                message += "\n"
            message += _(
                "Component Lots must be unique for mass production. Please review consumption for:\n"
            )
            message += "\n".join(
                component.name for component in multiple_lot_components
            )
        if message:
            ...
            for move_line in move.move_line_ids:
                ...
                if duplicates:
                    ...
                    if not (
                        ...
                        and duplicates - duplicates_returned - removed == 0
                    ):
                        raise UserError(message)
    ...
    def _check_serial_mass_produce_components(self):
        ...
        for production in self:
            have_serial_components |= any(
                move.product_id.tracking == "serial" for move in production.move_raw_ids
            )
            have_lot_components |= any(
                move.product_id.tracking == "lot" for move in production.move_raw_ids
            )
            ...
            multiple_lot_components.update(
                [p for p, l in components.items() if len(l) != 1]
            )
        return (
            have_serial_components,
            have_lot_components,
    ...
    def _generate_backorder_productions_multi(
        self, serial_numbers, cancel_remaining_quantities=False
    ):
        warnings.warn(
            "Method '_generate_backorder_productions_multi()' is deprecated, use _split_productions() instead.",
            DeprecationWarning,
        )
        self._split_productions(
            {self: [1] * len(serial_numbers)}, cancel_remaining_quantities
        )
