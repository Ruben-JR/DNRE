...
class MrpWorkorder(models.Model):
    ...
    next_work_order_id = fields.Many2one(
        "mrp.workorder", "Next Work Order", check_company=True
    )
    ...
    def _compute_json_popover(self):
        ...
        for wo in self:
            ...
            if wo.state in ("pending", "waiting", "ready"):
                ...
                prev_finished = (
                    previous_wo and previous_wo["date_planned_finished"] or False
                )
                if (
                    wo.state == "pending"
                    and prev_start
    ...
    def _set_dates_planned(self):
        if self.leave_id and (
            not self[0].date_planned_start or not self[0].date_planned_finished
        ):
            raise UserError(
                _(
    ...
    def unlink(self):
        ...
        previous_wos = self.env["mrp.workorder"].search(
            [("next_work_order_id", "in", self.ids), ("id", "not in", self.ids)]
        )
            ...
            if delta_duration > 0:
                ...
                self.env["mrp.workcenter.productivity"].create(
                    order._prepare_timeline_vals(
                        delta_duration, date_start, datetime.now()
                    )
                )
            else:
    ...
    def _onchange_date_planned_start(self):
        if self.date_planned_start and self.duration_expected and self.workcenter_id:
            self.date_planned_finished = (
                self.workcenter_id.resource_calendar_id.plan_hours(
    ...
    def _action_confirm(self):
        ...
        for production, workorders in workorders_by_production.items():
            ...
            for workorder in workorders:
                ...
                moves.filtered(
                    lambda m: m.operation_id == workorder.operation_id
                ).write({"workorder_id": workorder.id})

            exploded_boms, dummy = production.bom_id.explode(
                production.product_id, 1, picking_type=production.bom_id.picking_type_id
            )
            ...
            for move in moves:
                ...
                while bom and bom not in workorders_by_bom:
                    ...
                    bom = (
                        bom_data.get("parent_line")
                        and bom_data["parent_line"].bom_id
                        or False
                    )
                if bom in workorders_by_bom:
                    move.write({"workorder_id": workorders_by_bom[bom][-1:].id})
                else:
                    move.write(
            ...
            for workorders in workorders_by_bom.values():
                ...
                if workorders[0].state == "pending":
                    workorders[0].state = (
                        "ready"
                        if workorders[0].production_availability == "assigned"
                        else "waiting"
                    )
                for workorder in workorders:
    ...
    def _start_nextworkorder(self):
        if self.state == "done":
            ...
            if next_order.state == "pending":
                next_order.state = (
                    "ready"
                    if next_order.production_availability == "assigned"
                    else "waiting"
                )
        ...
        for timeline in timeline_obj.search(domain, limit=None if doall else 1):
            ...
            if wo.duration_expected <= wo.duration:
                ...
                maxdate = fields.Datetime.from_string(
                    timeline.date_start
                ) + relativedelta(minutes=wo.duration_expected - wo.duration)
