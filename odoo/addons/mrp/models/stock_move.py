...
class StockMove(models.Model):
    ...
    created_production_id = fields.Many2one(
        "mrp.production", "Created Production Order", check_company=True
    )
    production_id = fields.Many2one(
        "mrp.production",
    ...
    order_finished_lot_ids = fields.Many2many(
        ...
        compute="_compute_order_finished_lot_ids",
    )
    ...
    product_qty_available = fields.Float(
        "Product On Hand Quantity", related="product_id.qty_available"
    )
    product_virtual_available = fields.Float(
        "Product Forecasted Quantity", related="product_id.virtual_available"
    )
    ...
    def _compute_order_finished_lot_ids(self):
        for move in self:
            move.order_finished_lot_ids = (
                move.raw_material_production_id.lot_producing_id
            )
    ...
    def _update_quantity_done(self, mo):
        self.ensure_one()
        new_qty = mo.product_uom_id._compute_quantity(
