...
class MrpProduction(models.Model):
    ...
    def _populate(self, size):
        ...
        def fill_mo_with_bom_info():
            ...
            _logger.info(
                "Create Raw moves of MO(s) with bom (%d)" % len(productions_with_bom)
            )
            self.env["stock.move"].create(productions_with_bom._get_moves_raw_values())
            _logger.info(
                "Create Finished moves of MO(s) with bom (%d)"
                % len(productions_with_bom)
            )
            self.env["stock.move"].create(
                productions_with_bom._get_moves_finished_values()
            )
            _logger.info(
                "Create Workorder moves of MO(s) with bom (%d)"
                % len(productions_with_bom)
            )
