...
class TestSaleOrder(TestSaleCommon):
    ...
    def test_sale_with_pricelist_discount_included(self):
        ...
        for line in self.sale_order.order_line:
            if line.product_id == self.company_data["product_order_no"]:
                self.assertEqual(
                    line.price_unit,
                    self.pricelist_discount_incl_item3.fixed_price,
                    "Price of product_order should be %s applied on the order line"
                    % (self.pricelist_discount_incl_item3.fixed_price,),
                )
            else:  # only services (service_order and service_deliver)
                for item in self.sale_order.pricelist_id.item_ids.filtered(
                    lambda l: l.product_tmpl_id == line.product_id.product_tmpl_id
                ):
                    ...
                    self.assertEqual(
                        ...
                        * 100,
                        "Pricelist of the SO should be applied on an order line %s"
    ...
    def test_sale_with_pricelist_discount_excluded(self):
        ...
        self.company_data["product_order_no"].write(
            {"categ_id": self.product_category_1.id}
        )
        self.company_data["product_delivery_no"].write(
            {"categ_id": self.product_category_1.id}
        )
        ...
        for line in self.sale_order.order_line:
            if (
                ...
                in self.sale_order.pricelist_id.item_ids.mapped("categ_id")
            ):  # reduction per category (consummable only)
                for item in self.sale_order.pricelist_id.item_ids.filtered(
                    lambda l: l.categ_id == line.product_id.categ_id
                ):
                    self.assertEqual(
                        line.discount,
                        item.price_discount,
                        "Discount should be displayed on order line %s since its category get some discount"
                        % (line.name,),
                    )
                self.assertEqual(
                    line.price_unit,
                    line.product_id.standard_price,
                    f"Price unit should be the cost price for product {line.name}",
                )
            else:
                if (
                    line.product_id == self.company_data["product_service_order"]
                ):  # reduction for this product
                    self.assertEqual(
                        line.discount,
                        20.0,
                        "Discount for product %s should be 20 percent with pricelist %s"
                        % (line.name, self.pricelist_discount_excl.name),
                    )
                    self.assertEqual(
                        line.price_unit,
                        line.product_id.list_price,
                    ...
                    self.assertEqual(
                        line.discount,
                        0.0,
                        "Pricelist of SO should not be applied on an order line",
                    )
                    self.assertEqual(
                        line.price_unit,
                        line.product_id.list_price,
    ...
    def test_sale_change_of_pricelists_excluded_value_discount_on_tax_included_price_mapped_to_tax_excluded_price(
        self,
    ):
        self.env.user.write(
            {"groups_id": [(4, self.env.ref("product.group_discount_per_so_line").id)]}
        ...
        fiscal_pos.tax_ids = [
            (0, None, {"tax_src_id": tax_a.id, "tax_dest_id": tax_b.id})
        ]
        ...
        my_product = self.env["product.product"].create(
            {"name": "my Product", "lst_price": 115, "taxes_id": [tax_a.id]}
        )
        ...
        self.assertEqual(
            sale_order.order_line.tax_id.id,
            tax_b.id,
            "Wrong tax applied for specified product & pricelist",
        )
