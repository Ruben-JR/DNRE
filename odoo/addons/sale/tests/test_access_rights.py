...
class TestAccessRights(TestSaleCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.company_data["default_user_salesman_2"] = (
            cls.env["res.users"]
            .with_context(no_reset_password=True)
            .create(
                {
                    "name": "default_user_salesman_2",
                    "login": "default_user_salesman_2.comp%s"
    ...
    def test_access_sales_person(self):
        ...
        self.order.with_user(self.company_data["default_user_salesman_2"]).write(
            {"team_id": self.company_data["default_sale_team"].id}
        )
    ...
    def test_access_portal_user(self):
        ...
        with self.assertRaises(AccessError):
            self.order.with_user(self.company_data["default_user_portal"]).write(
                {"team_id": self.company_data["default_sale_team"].id}
            )
    ...
    def test_access_employee(self):
        ...
        with self.assertRaises(AccessError):
            self.order.with_user(self.company_data["default_user_employee"]).write(
                {"team_id": self.company_data["default_sale_team"].id}
            )
