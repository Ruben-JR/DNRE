...
class TestOnchangeProductId(TransactionCase):
    ...
    def test_onchange_product_id(self):
        ...
        self.assertEqual(
            100,
            sale_order.order_line[0].price_unit,
    ...
    def test_pricelist_application(self):
        ...
        self.assertEqual(
            so.order_line[0].price_unit, 80, "First date pricelist rule not applied"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            so.order_line[0].price_unit, 50, "Second date pricelist rule not applied"
        )
        self.assertEqual(
            so.order_line[0].price_subtotal,
            so.order_line[0].price_unit * so.order_line[0].product_uom_qty,
    ...
    def test_pricelist_uom_discount(self):
        ...
        self.assertEqual(
            ...
            90,
            "Christmas discount pricelist rule not applied",
        )
        self.assertEqual(
            order_line.discount, 10, "Christmas discount not equalt to 10%"
        )
