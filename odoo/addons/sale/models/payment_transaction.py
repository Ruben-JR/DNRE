...
class PaymentTransaction(models.Model):
    ...
    def _compute_sale_order_reference(self, order):
        ...
        if self.acquirer_id.so_reference_type == "so_name":
            ...
            return "{}/{}".format("CUST", str(identification_number % 97).rjust(2, "0"))
    ...
    def _set_pending(self, state_message=None):
        ...
        super()._set_pending(state_message=state_message)

        for record in self:
            sales_orders = record.sale_order_ids.filtered(
    ...
    def _reconcile_after_done(self):
        ...
        ) and any(so.state in ("sale", "done") for so in self.sale_order_ids):
            self.filtered(
