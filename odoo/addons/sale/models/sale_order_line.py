...
class SaleOrderLine(models.Model):
    ...
    product_updatable = fields.Boolean(
        compute="_compute_product_updatable", string="Can Edit Product", default=True
    )
    ...
    qty_delivered_method = fields.Selection(
        [("manual", "Manual"), ("analytic", "Analytic From Expenses")],
    ...
    qty_delivered_manual = fields.Float(
        "Delivered Manually", copy=False, digits="Product Unit of Measure", default=0.0
    )
    ...
    def _onchange_suggest_packaging(self):
        ...
        if self.product_id and self.product_uom_qty and self.product_uom:
            self.product_packaging_id = self.product_id.packaging_ids.filtered(
                "sales"
            )._find_suitable_product_packaging(self.product_uom_qty, self.product_uom)
    ...
    def _onchange_product_packaging_qty(self):
        if self.product_packaging_id:
            ...
            product_uom_qty = packaging_uom._compute_quantity(
                self.product_packaging_qty * qty_per_packaging, self.product_uom
            )
            if (
                float_compare(
                    product_uom_qty,
        ...
        for line in self:
            if not line.analytic_tag_ids:
                default_analytic_account = (
                    line.env["account.analytic.default"]
                    .sudo()
                    .account_get(
    ...
    def name_get(self):
        ...
        for so_line in self.sudo():
            ...
            if so_line.order_partner_id.ref:
                name = f"{name} ({so_line.order_partner_id.ref})"
