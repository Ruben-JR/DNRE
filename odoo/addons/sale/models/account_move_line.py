...
class AccountMoveLine(models.Model):
    ...
    def _prepare_analytic_line(self):
        ...
        """
        values_list = super()._prepare_analytic_line()
    ...
    def _sale_determine_order(self):
        ...
        if analytic_accounts:  # first, search for the open sales order
            ...
            analytic_accounts_without_open_order = analytic_accounts.filtered(
                lambda account: not mapping.get(account.id)
            )
            if (
                analytic_accounts_without_open_order
            ):  # then, fill the blank with not open sales orders
                sale_orders = self.env["sale.order"].search(
                    [
                        (
                            "analytic_account_id",
                            "in",
                            analytic_accounts_without_open_order.ids,
                        )
                    ],
                    order="create_date DESC",
                )
        ...
        return {
            move_line.id: mapping.get(move_line.analytic_account_id.id)
            for move_line in self
        }
    ...
    def _sale_prepare_sale_line_values(self, order, price):
        ...
        fpos = order.fiscal_position_id or order.fiscal_position_id.get_fiscal_position(
            order.partner_id.id
        )
