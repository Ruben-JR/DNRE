...
class ResCompany(models.Model):
    ...
    siret = fields.Char(
        related="partner_id.siret", string="SIRET", size=14, readonly=False
    )
    ...
    def _is_accounting_unalterable(self):
        if not self.vat and not self.country_id:
            return False
        return (
            self.country_id and self.country_id.code in self._get_unalterable_country()
        )
            ...
            for seq_field in sequence_fields:
                if not company[seq_field]:
                    vals = {
                        ...
                        "code": f"FRSECURE{company.id}-{seq_field}",
