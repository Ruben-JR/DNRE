...
class ResourceCalendar(models.Model):
        ...
        for attendance in self.env["resource.calendar.attendance"].search(domain):
            for resource in resources_list:
                ...
                if self.two_weeks_calendar and attendance.week_type:
                    days = rrule(
                        WEEKLY, start, interval=2, until=until, byweekday=weekday
                    )
                else:
                ...
                for day in days:
                    ...
                    result[resource.id].append(
                        (
                            max(cache_dates[(tz, start_dt)], dt0),
                            min(cache_dates[(tz, end_dt)], dt1),
                            attendance,
                        )
                    )
    ...
    def _get_closest_work_time(
        self, dt, match_end=False, resource=None, search_range=None
    ):
...
class ResourceResource(models.Model):
    ...
    def _adjust_to_calendar(self, start, end):
        ...
        for resource in self:
            ...
            if (
                calendar_start
                and start.astimezone(tz).date() == end.astimezone(tz).date()
            ):
            ...
            calendar_end = resource.calendar_id._get_closest_work_time(
                end, match_end=True, resource=resource, search_range=search_range
            )
            result[resource] = (
