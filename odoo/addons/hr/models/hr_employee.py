...
class HrEmployeePrivate(models.Model):
    ...
    emergency_contact = fields.Char(
        "Emergency Contact", groups="hr.group_hr_user", tracking=True
    )
    emergency_phone = fields.Char(
        "Emergency Phone", groups="hr.group_hr_user", tracking=True
    )
    ...
    def _read(self, fields):
        ...
        for r in res:
            ...
            record._update_cache(
                {k: v for k, v in r.items() if k in fields}, validate=False
            )
    ...
    def _get_unusual_days(self, date_from, date_to=None):
        ...
        if not calendar:
            return {}
        dfrom = datetime.combine(fields.Date.from_string(date_from), time.min).replace(
            tzinfo=pytz.UTC
        )
        dto = datetime.combine(fields.Date.from_string(date_to), time.max).replace(
            tzinfo=pytz.UTC
        )
