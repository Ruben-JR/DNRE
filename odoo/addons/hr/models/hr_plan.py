...
class HrPlanActivityType(models.Model):
    ...
    def get_responsible_id(self, employee):
        if self.responsible == "coach":
            ...
            if not responsible:
                raise UserError(
                    _("User of coach of employee %s is not set.", employee.name)
                )
            ...
            if not responsible:
                raise UserError(
                    _("User of manager of employee %s is not set.", employee.name)
                )
        elif self.responsible == "employee":
            responsible = employee.user_id
            if not responsible:
                raise UserError(
                    _("User linked to employee %s is required.", employee.name)
                )
            ...
            if not responsible:
                raise UserError(
                    _(
                        "No specific user given on activity %s.",
                        self.activity_type_id.name,
                    )
                )
        return responsible
...
class HrPlan(models.Model):
    ...
    plan_activity_type_ids = fields.Many2many(
        "hr.plan.activity.type", string="Activities"
    )
