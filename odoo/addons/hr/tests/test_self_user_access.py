...
class TestSelfAccessProfile(TestHrCommon):
    ...
    def test_profile_view_fields(self):
        ...
        view_infos = (
            self.env["res.users"]
            .with_user(user_all_groups)
            .fields_view_get(view_id=view.id)
        )
    ...
    def test_access_my_profile_toolbar(self):
        ...
        view = self.env.ref("hr.res_users_view_form_profile")
        available_actions = james.fields_view_get(view_id=view.id, toolbar=True)[
            "toolbar"
        ]["action"]
        ...
        view = self.env.ref("hr.res_users_view_form_profile")
        available_actions = john.fields_view_get(view_id=view.id, toolbar=True)[
            "toolbar"
        ]["action"]
        self.assertTrue(
            any(x["id"] == change_password_action.id for x in available_actions)
...
class TestSelfAccessRights(TestHrCommon):
    def setUp(self):
        ...
        self.protected_fields_emp = OrderedDict(
            [
                (k, v)
                for k, v in self.env["hr.employee"]._fields.items()
