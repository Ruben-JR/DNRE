...
class MicrosoftSync(models.AbstractModel):
        ...
        for record in self.filtered("need_sync_m"):
            if need_delete and record.microsoft_id:
                ...
                record._microsoft_delete(
                    microsoft_service, record.microsoft_id, timeout=3
                )
                ...
                record._microsoft_patch(
                    microsoft_service, record.microsoft_id, values, timeout=3
                )
        return result
    ...
    def create(self, vals_list):
        ...
        for record in records_to_sync:
            record._microsoft_insert(
                microsoft_service,
                record._microsoft_values(self._get_microsoft_synced_fields()),
                timeout=3,
            )
        return records
    ...
    def _sync_recurrence_microsoft2odoo(self, microsoft_events: MicrosoftEvent):
        ...
        for recurrent_master_id in {x.seriesMasterId for x in recurrents}:
            recurrence_id = recurrences.filtered(
                lambda ev: ev.microsoft_id == recurrent_master_id
            )
            ...
            for recurrent_event in to_update:
                ...
                existing_event = recurrence_id.calendar_event_ids.filtered(
                    lambda e: e._range() == (value["start"], value["stop"])
                )
    ...
    def _update_microsoft_recurrence(self, recurrence_event, events):
        vals = dict(
            self.base_event_id._microsoft_to_odoo_values(recurrence_event, ()),
            need_sync_m=False,
        )
        ...
        for recurrent_event in events_to_update:
            if recurrent_event.type == "occurrence":
                ...
                event = (
                    ...
                    .browse(recurrent_event.odoo_id(self.env))
                    .exists()
                )
                if event:
                    event.with_context(
                        no_mail_to_attendees=True, mail_create_nolog=True
                    ).write(dict(value, need_sync_m=False))
            if value.get("start") and value.get("stop"):
                values[(self.id, value.get("start"), value.get("stop"))] = dict(
                    value, need_sync_m=False
                )

        if (self.id, vals.get("start"), vals.get("stop")) in values:
            base_event_vals = dict(vals)
            base_event_vals.update(
                values[(self.id, vals.get("start"), vals.get("stop"))]
            )
            self.base_event_id.write(base_event_vals)

        old_record = self._apply_recurrence(
            specific_values_creation=values, no_send_edit=True
        )
    ...
    @api.model
    def _sync_microsoft2odoo(
        self, microsoft_events: MicrosoftEvent, default_reminders=()
    ):
        ...
        odoo_values = [
            dict(
                self._microsoft_to_odoo_values(e, default_reminders, default_values),
                need_sync_m=False,
            )
            for e in (new - new_recurrent)
        ]
        new_odoo = self.with_context(dont_notify=True)._create_from_microsoft(
            new, odoo_values
        )
        ...
        recurrent_cancelled = self.env["calendar.recurrence"].search(
            [
                (
                    "microsoft_id",
                    "in",
                    (microsoft_events.cancelled() - cancelled).microsoft_ids(),
                )
            ]
        )
        ...
        synced_records = (
            new_odoo + cancelled_odoo + synced_recurrent_records.calendar_event_ids
        )

        for mevent in (existing - cancelled).filter(
            lambda e: e.lastModifiedDateTime and not e.seriesMasterId
        ):
            # Last updated wins.
            # This could be dangerous if microsoft server time and odoo server time are different
            if mevent.is_recurrence():
                odoo_record = self.env["calendar.recurrence"].browse(
            ...
            if updated >= odoo_record_updated:
                vals = dict(
                    odoo_record._microsoft_to_odoo_values(mevent, default_reminders),
                    need_sync_m=False,
                )
    ...
    @after_commit
    def _microsoft_delete(
        self, microsoft_service: MicrosoftCalendarService, microsoft_id, timeout=TIMEOUT
    ):
    ...
    def _microsoft_patch(
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                microsoft_service.patch(
                    microsoft_id, values, token=token, timeout=timeout
                )
                self.need_sync_m = False

    @after_commit
    def _microsoft_insert(
        self, microsoft_service: MicrosoftCalendarService, values, timeout=TIMEOUT
    ):
        if not values:
            return
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                self._ensure_attendees_have_email()
                microsoft_id = microsoft_service.insert(
    ...
    def _microsoft_attendee_answer(
        self,
        microsoft_service: MicrosoftCalendarService,
        microsoft_id,
        answer,
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                microsoft_service.answer(
                    microsoft_id, answer, params, token=token, timeout=timeout
                )
                self.write(
                    {
                        "need_sync_m": False,
    ...
    def _microsoft_to_odoo_values(
        self, microsoft_event: MicrosoftEvent, default_reminders=()
    ):
