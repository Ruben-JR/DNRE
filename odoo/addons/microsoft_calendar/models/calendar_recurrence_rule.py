...
class RecurrenceRule(models.Model):
    ...
    def _apply_recurrence(
        ...
        for event in events.filtered("microsoft_id"):
            if (
                ...
                and not event.recurrence_id.microsoft_id
            ):
                vals += [
                    {
                        "name": event.name,
    ...
    def _microsoft_to_odoo_values(
        self, microsoft_recurrence, default_reminders=(), default_values={}
    ):
    ...
    def _microsoft_values(self, fields_to_sync):
        events_outliers = self.calendar_event_ids.filtered(
            lambda e: not e.follow_recurrence
        )
        events = self.calendar_event_ids.sorted("start")
        normal_event = (events - events_outliers)[:1] or events[:1]
        if not normal_event:
            return {}
        values = normal_event._microsoft_values(
