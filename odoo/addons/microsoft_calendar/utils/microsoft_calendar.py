...
class MicrosoftCalendarService:
    ...
    def get_events(self, sync_token=None, token=None, timeout=TIMEOUT):
        ...
        if sync_token:
            ...
            params["startDateTime"] = fields.Datetime.subtract(
                fields.Datetime.now(), years=2
            ).strftime("%Y-%m-%dT00:00:00Z")
            params["endDateTime"] = fields.Datetime.add(
                fields.Datetime.now(), years=2
            ).strftime("%Y-%m-%dT00:00:00Z")
        try:
            status, data, time = self.microsoft_service._do_request(
        ...
        next_sync_token = (
            urls.url_parse(next_sync_token_url).decode_query().get("$deltatoken", False)
        )
    ...
    def insert(self, values, token=None, timeout=TIMEOUT):
        ...
        if not values.get("id"):
            values.pop("id", None)
        dummy, data, dummy = self.microsoft_service._do_request(
            url,
    ...
    def patch(self, event_id, values, token=None, timeout=TIMEOUT):
        ...
        self.microsoft_service._do_request(
            url,
            json.dumps(values, separators=(",", ":")),
            headers,
            method="PATCH",
    ...
    def answer(self, event_id, answer, values, token=None, timeout=TIMEOUT):
        url = f"/v1.0/me/calendar/events/{event_id}/{answer}"
        headers = {
            "Content-type": "application/json",
            "Authorization": "Bearer %s" % token,
        }
