...
class SaleOrder(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if "sale_order_template_id" in fields_list and not default_vals.get(
    ...
    def copy(self, default=None):
        if (
            ...
            and self.sale_order_template_id.number_of_days > 0
        ):
    ...
    def get_access_action(self, access_uid=None):
        ...
        user = (
            access_uid
            and self.env["res.users"].sudo().browse(access_uid)
            or self.env.user
        )

        if not self.sale_order_template_id or (
            not user.share and not self.env.context.get("force_website")
        ):
...
class SaleOrderLine(models.Model):
    ...
    def product_id_change(self):
        ...
        if self.product_id and self.order_id.sale_order_template_id:
            for (
                line
            ) in self.order_id.sale_order_template_id.sale_order_template_line_ids:
                if line.product_id == self.product_id:
                    self.name = (
                        line.with_context(lang=self.order_id.partner_id.lang).name
...
class SaleOrderOption(models.Model):
    ...
    quantity = fields.Float(
        "Quantity", required=True, digits="Product Unit of Measure", default=1
    )
