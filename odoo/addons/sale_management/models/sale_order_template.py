...
class SaleOrderTemplate(models.Model):
    ...
    sale_order_template_line_ids = fields.One2many(
        "sale.order.template.line", "sale_order_template_id", "Lines", copy=True
    )
        ...
        for lang in languages:
            for line in self.sale_order_template_line_ids:
                if (
                    ...
                    self.create_or_update_translations(
                        model_name="sale.order.template.line,name",
                        lang_code=lang.code,
                        res_id=line.id,
                        src=line.name,
            ...
            for option in self.sale_order_template_option_ids:
                if (
                    ...
                    self.create_or_update_translations(
                        model_name="sale.order.template.option,name",
                        lang_code=lang.code,
                        res_id=option.id,
                        src=option.name,
                        value=option.product_id.with_context(
                            lang=lang.code
                        ).get_product_multiline_description_sale(),
                    )
